; ------------------------------------------------------------------------------
; subrom.asm
; MSX SUBROM, SONY HB-F1 (version 2.0)
;
; Code copyright by ASCII and maybe others
; Source comments by Arjen Zeilemaker
;
; Source code supplied for STUDY ONLY
; Recreation NOT permitted without authorisation of the copyright holder(s)
;
; H.J. Berends:
; Converted sources to assemble with z88dk z80asm
; Restructured and reviewed comments
; ------------------------------------------------------------------------------

DEFINE	_BIOS_		; include BIOS symbol definitions
DEFINE	_SUBROM_	; avoid duplicate label definitions

		INCLUDE	"base.inc"
		INCLUDE "msx.inc"

		ORG	0000H

; Symbols defined in BIOS module
RDSLT		EQU	0000CH
WRSLT		EQU	00014H
OUTDO		EQU	00018H
KEYINT		EQU	00038H
GICINI		EQU	00090H
CHGET		EQU	0009FH
CKCNTC		EQU	000BDH
FNKSB		EQU	000C9H
SNSMAT		EQU	00141H
EOL		EQU	00168H

; Symbols defined in BASIC module
FDBINT		EQU	2F92H			; convert float (SGN or DBL) to integer
CHKSTR		EQU	3058H			; check if string
CONSUI		EQU	3236H			; convert unsigned integer to single real
LINPRT		EQU	3412H			; number to interpreter output
REDDY		EQU	3FD7H			; Ok message
SNERR		EQU	4055H			; syntax error
MOERR		EQU	406AH			; missing operand error
TMERR		EQU	406DH			; type mismatch error
ERROR		EQU	406FH			; basic error
NUMCON		EQU	46E6H			; basic text for constant handling
FCERR		EQU	475AH			; illegal function call error
FRMEVL		EQU	4C64H			; evaluate expression
SNGFLT		EQU	4FCFH			; byte to DAC
M51D7		EQU	51D7H			; memory disk helper routine for page 0 transfer
GETIN2		EQU	520FH			; evaluate integer operand
GETBYT		EQU	521CH			; evaluate byte operand
GETUIN		EQU	542FH			; evaluate address operand
FRQINT		EQU	5439H			; convert DAC to address operand
SCAN1		EQU	579CH			; evaluate graphic coordinatepair
SCAND		EQU	57ABH			; evaluate 2nd graphic coordinatepair
PTRGET		EQU	5EA4H			; locate variable
OMERR		EQU	6275H			; out of memory error
STRINI		EQU	6627H			; allocate temporary string
STROUT		EQU	6678H			; message to interpreter output
FRESTR		EQU	67D0H			; free temporary string with type check
FILEVL		EQU	6A0EH			; evaluate filespecification
CLSFIL		EQU	6B24H			; close I/O channel
DERNMF		EQU	6E6BH			; bad filename error
DERFAO		EQU	6E6EH			; file already open error
DERFNF		EQU	6E74H			; file not found error
DERSOO		EQU	6E86H			; sequential i/o only error
CRDONZ		EQU	7323H			; newline to OUTDO if not at start of line
CRDO		EQU	7328H			; newline to OUTDO
M7EE4		EQU	7EE4H			; msx version string
M7EF2		EQU	7EF2H			; msx basic string
M7FFD		EQU	7FFDH			; BDOS entry in mainrom

; MODE:
; b0    = SHIFT KANA mode
; b1-b2 = VRAM Size
; b7    = upcase status SHIFT KANA mode
;
; SLTWRK+0:
; b7,b3-b0 = slotid RAM memory disk
; b5       = memory disk disabled
; b6       = memory disk initialized

		DEFB	"CD"			; subROM extension indentifier
		DEFW	C0336			; subROM extension initialize
		DEFW	C1649			; subROM extension BASIC statement handler
		DEFW	C36D5			; subROM extension BASIC device handler

S_SYNCHR:	JP	J1D32
		DEFS	0000CH-$,0
S_RDSLT:	JP	C01FD
		DEFS	00010H-$,0
S_CHRGTR:	JP	J1D3A
		DEFS	00014H-$,0
S_WRSLT:	JP	C0218
		DEFS	00018H-$,0
S_OUTDO:	JP	J0577
		DEFS	0001CH-$,0
S_CALM01:	JP	C0262
		DEFS	00020H-$,0

; Subroutine S.DCOMPR
S_DCOMPR:	LD	A,H
		SUB	D
		RET	NZ
		LD	A,L
		SUB	E
		RET

		DEFS	00028H-$,0

; Subroutine evaluate byte operand
S_GETBYT:	JP	C060E
		DEFS	00030H-$,0

S_CALM0F:	JP	C0246
		DEFS	00038H-$,0

S_KEYINT:	EX	AF,AF'
		EXX
		PUSH	AF
		PUSH	BC
		PUSH	DE
		PUSH	HL
		EX	AF,AF'
		EXX
		PUSH	IX
		PUSH	IY
		LD	IX,KEYINT
		LD	IY,(EXPTBL+0-1)
		CALL	C058D
		POP	IY
		POP	IX
		EX	AF,AF'
		EXX
		POP	HL
		POP	DE
		POP	BC
		POP	AF
		EX	AF,AF'
		EXX
		EI
		RETI

		DEFS	00066H-$,0

S_NMI:
Q0066:		JP	H_NMI

S_PAINT:
Q0069:		EI
		JP	J2651

S_PSET:
Q006D:		EI
		JP	J2556

S_ATRSCN:
Q0071:		EI
		JP	C256F

S_GLINE:
Q0075:		EI
		JP	J2607

S_DOBOXF:
Q0079:		EI
		JP	J262A

S_DOLINE:
Q007D:		EI
		JP	J263F

S_BOXLIN:
Q0081:		EI
		JP	J2648

S_DOGRPH:
Q0085:		EI
		JP	C2823

S_GRPPRT:
Q0089:		EI
		JP	C110E

S_SCALXY:
Q008D:		EI
		JP	C12C4

S_MAPXYC:
Q0091:		EI
		JP	C1342

S_READC:
Q0095:		EI
		JP	C1384

S_SETATR:
Q0099:		EI
		JP	C13AD

S_SETC:
Q009D:		EI
		JP	C13B5

S_TRIGHT:
Q00A1:		EI
		JP	C1426

S_RIGHTC:
Q00A5:		EI
		JP	J1439

S_TLEFTC:
Q00A9:		EI
		JP	C13E7

S_LEFTC:
Q00AD:		EI
		JP	J144B

S_TDOWNC:
Q00B1:		EI
		JP	C13FC

S_DOWNC:
Q00B5:		EI
		JP	J148C

S_TUPC:
Q00B9:		EI
		JP	C1414

S_UPC:
Q00BD:		EI
		JP	J1498

S_SCANR:
Q00C1:		EI
		JP	C14DB

S_SCANL:
Q00C5:		EI
		JP	C158F

S_NVBXLN:
Q00C9:		EI
		JP	C27E7

S_NVBXFL:
Q00CD:		EI
		JP	C2898

S_CHGMOD:
Q00D1:		EI
		JP	J09C3

S_INITXT:
Q00D5:		EI
		JP	C09E5

S_INIT32:
Q00D9:		EI
		JP	C0A1A

S_INIGRP:
Q00DD:		EI
		JP	C0A48

S_INIMLT:
Q00E1:		EI
		JP	C0A98

S_SETTXT:
Q00E5:		EI
		JP	C0B42

S_SETT32:
Q00E9:		EI
		JP	C0B9C

S_SETGRP:
Q00ED:		EI
		JP	C0BD2

S_SETMLT:
Q00F1:		EI
		JP	C0C22

S_CLRSPR:
Q00F5:		EI
		JP	C06F5

S_CALPAT:
Q00F9:		EI
		JP	J0755

S_CALATR:
Q00FD:		EI
		JP	C076A

S_GSPSIZ:
Q0101:		EI
		JP	C077F

S_GETPAT:
Q0105:		EI
		JP	C07D9

S_WRTVRM:
Q0109:		EI
		JP	C08CB

S_RDVRM:
Q010D:		EI
		JP	C08D6

S_CHGCLR:
Q0111:		EI
		JP	C0953

S_CLS:
Q0115:		EI
		JP	C07FE

S_CLRTXT:
Q0119:		EI
		JP	C0864

S_DSPFNK:
Q011D:		EI
		JP	J0D52

S_DELLNO:
Q0121:		EI
		JP	J0E0F

S_INSLNO:
Q0125:		EI
		JP	J0E5D

S_PUTVRM:
Q0129:		EI
		JP	J0F16

S_WRTVDP:
Q012D:		EI
		JP	C0647

S_VDPSTA:
Q0131:		EI
		JP	C298B

S_KYKLOK:
Q0135:		EI
		JP	J3F04

S_PUTCHR:
Q0139:		EI
		JP	J3CC9

S_SETPAG:
Q013D:		EI
		JP	C06A8

S_INIPLT:
Q0141:		EI
		JP	J0F9F

S_RSTPLT:
Q0145:		EI
		JP	J0F7E

S_GETPLT:
Q0149:		EI
		JP	C0F6A

S_SETPLT:
Q014D:		EI
		JP	C0FD3

S_PUTSPR:
Q0151:		EI
		JP	J208F

S_COLOR:
Q0155:		EI
		JP	J1DF6

S_SCREEN:
Q0159:		EI
		JP	J1EE5

S_WIDTHS:
Q015D:		EI
		JP	J1FDF

S_VDP:
Q0161:		EI
		JP	J21C6

S_VDPF:
Q0165:		EI
		JP	J21E5

S_BASE:
Q0169:		EI
		JP	J2220

S_BASEF:
Q016D:		EI
		JP	J2297

S_VPOKE:
Q0171:		EI
		JP	J22F4

S_VPEEK:
Q0175:		EI
		JP	J2307

S_SETS:
Q0179:		EI
		JP	J1833

S_BEEP:
Q017D:		EI
		JP	J1A62

S_PROMPT:
Q0181:		EI
		JP	J1B4E

S_SDFSCR:
Q0185:		EI
		JP	J0480

S_SETSCR:
Q0189:		EI
		JP	J0470

S_SCOPY:
Q018D:		EI
		JP	J2342

S_BLTVV:
Q0191:		EI
		JP	J2EB9

S_BLTVM:
Q0195:		EI
		JP	J2F42

S_BLTMV:
Q0199:		EI
		JP	J2EDF

S_BLTVD:
Q019D:		EI
		JP	J30A8

S_BLTDV:
Q01A1:		EI
		JP	J31B4

S_BLTMD:
Q01A5:		EI
		JP	J306F

S_BLTDM:
Q01A9:		EI
		JP	J307C

S_NEWPAD:
Q01AD:		EI
		JP	J34DD

S_GETPUT:
Q01B1:		EI
		JP	J1683

S_CHGMDP:
Q01B5:		EI
		JP	C09BF

S_RESVI:
Q01B9:		EI
		JP	0

S_KNJPRT:
Q01BD:		EI
		JP	J1049

; Subroutine calslt mainrom on page 0, when subrom is in a subslot of slot 0
; Remark: The working of this code depends on the fact that the mainrom contains code
;         at 01D0H indentical to this code.
J01C1:		LD	A,B
		AND	3FH
		OUT	(0A8H),A
		LD	A,(D_FFFF)
		CPL
		LD	C,A
		AND	D
		LD	E,A
		LD	(D_FFFF),A

; from this point code continues to execute in mainrom
; This code is executed from mainrom when calling the subrom when it was a subslot of slot 0
Q01D0:		LD	A,B
		AND	D
		OUT	(0A8H),A
		LD	A,E
		LD	(SLTTBL+0),A
		PUSH	BC
		EXX
		EX	AF,AF'
		CALL	CLPRM1
		DI
		EX	AF,AF'
		EXX
		POP	BC
		LD	A,B
		AND	3FH
		OUT	(0A8H),A
		LD	A,C
		LD	(D_FFFF),A

; from this point code returns executing from the mainrom
; This code is executed when mainrom was called from the subrom when the subrom was in a subslot of slot 0
Q01EB:		LD	A,B
		OUT	(0A8H),A
		LD	A,C
		LD	(SLTTBL+0),A
		EX	AF,AF'
		EXX
		RET

S_REDCLK:
Q01F5:		EI
		JP	C1D08

S_WRTCLK:
Q01F9:		EI
		JP	C1D10


; Subroutine S.RDSLT
; Remark: crashes when S.RDSLT is used for a slotid in the same primary slot as the subrom, but only on page 0
; NOT USED ANYWHERE IN SUBROM
C01FD:		CALL	C02DD			; calculate masks
		JP	M,J020D			; expanded slot,
		IN	A,(0A8H)
		LD	D,A
		AND	C
		OR	B
		CALL	RDPRIM			; read from slot
		LD	A,E
		RET

J020D:		PUSH	HL
		CALL	C0302			; adjust secondary slotregister
		EX	(SP),HL
		PUSH	BC
		CALL	C01FD			; RDSLT (on primary slot)
		JR	J0233

; Subroutine S.WRSLT
; Remark: crashes when S.WRSLT is used for a slotid in the same primary slot as the subrom, but only on page 0
; NOT USED ANYWHERE IN SUBROM
C0218:		PUSH	DE
		CALL	C02DD			; calculate masks
		JP	M,J0228			; expanded slot,
		POP	DE
		IN	A,(0A8H)
		LD	D,A
		AND	C
		OR	B
		JP	WRPRIM			; write to slot

J0228:		EX	(SP),HL
		PUSH	HL
		CALL	C0302			; adjust secondary slotregister
		POP	DE
		EX	(SP),HL
		PUSH	BC
		CALL	C0218			; WRSLT (on primary slot)
J0233:		POP	BC
		EX	(SP),HL
		PUSH	AF
		LD	A,B
		AND	3FH
		OR	C
		OUT	(0A8H),A
		LD	A,L
		LD	(D_FFFF),A
		LD	A,B
		OUT	(0A8H),A
		POP	AF
		POP	HL
		RET

; Subroutine callfar biosrom (page 0)
; Remark: only usable for page 0 addresses
C0246:		EX	(SP),HL
		PUSH	AF
		PUSH	DE
		LD	A,(HL)
		PUSH	AF
		POP	IY
		INC	HL
		LD	E,(HL)
		INC	HL
		LD	D,(HL)
		INC	HL
		PUSH	DE
		POP	IX
		POP	DE
		POP	AF
		EX	(SP),HL

; Subroutine call biosrom (page 0)
; Remark: only usable for page 0 addresses
C0258:		EXX
		LD	D,0FCH			; page 0 only
		JR	J0265

; Subroutine call basicrom (page 1)
; Remark: only usable for page 1 addresses
; NOT USED ANYWHERE IN SUBROM
Q025D:		EXX
		LD	D,0F3H			; page 1 only
		JR	J0265

; Subroutine call mainrom (page 0 and page 1)
; Remark: only usable for page 0 or 1 addresses
C0262:		EXX
		LD	D,0F0H			; page 0 and 1
J0265:		EX	AF,AF'
		DI
		LD	A,(EXPTBL+0)
		AND	A			; slot 0 expanded ?
		IN	A,(0A8H)
		JP	M,J0276			; yep, take extra care
		PUSH	AF
		AND	D
		EXX
		JP	CLPRIM

J0276:		LD	B,A
		LD	A,(EXBRSA)
		AND	03H			; subrom in a subslot of slot 0 ?
		JR	NZ,J0284		; nope, handle it the normal way
		LD	A,D
		CP	0F3H			; page 1 only ?
		JP	NZ,J01C1		; nope, use a trick to make this work
J0284:		LD	A,B
		AND	3FH
		OUT	(0A8H),A
		LD	A,(D_FFFF)
		CPL
		LD	C,A
		AND	D
		LD	(D_FFFF),A
		LD	E,A
		LD	A,B
		OUT	(0A8H),A
		LD	A,E
		LD	(SLTTBL+0),A
		LD	A,B
		PUSH	BC
		LD	HL,I02A6
		PUSH	HL
		PUSH	AF
		AND	D
		EXX
		JP	CLPRIM

I02A6:		DI
		EXX
		EX	AF,AF'
		POP	BC
		LD	A,B
		AND	3FH
		OUT	(0A8H),A
		LD	A,C
		LD	(D_FFFF),A
		LD	A,B
		OUT	(0A8H),A
		LD	A,C
		LD	(SLTTBL+0),A
		EX	AF,AF'
		EXX
		RET

; Subroutine S.ENASLT
; Remark: crashes when address is in page 0
C02BD:		CALL	C02DD			; calculate masks
		JP	M,J02CA			; slot expanded,
		IN	A,(0A8H)
		AND	C
		OR	B
		OUT	(0A8H),A
		RET

J02CA:		PUSH	HL
		CALL	C0302			; adjust secondary slotregister
		LD	C,A
		LD	B,0
		LD	A,L
		AND	H
		OR	D
		LD	HL,SLTTBL
		ADD	HL,BC
		LD	(HL),A
		POP	HL
		LD	A,C
		JR	C02BD

; Subroutine calculate masks
C02DD:		DI
		PUSH	AF
		LD	A,H
		RLCA
		RLCA
		AND	03H
		LD	E,A
		LD	A,0C0H
J02E7:		RLCA
		RLCA
		DEC	E
		JP	P,J02E7
		LD	E,A
		CPL
		LD	C,A
		POP	AF
		PUSH	AF
		AND	03H
		INC	A
		LD	B,A
		LD	A,0ABH
J02F8:		ADD	A,55H
		DJNZ	J02F8
		LD	D,A
		AND	E
		LD	B,A
		POP	AF
		AND	A
		RET

; Subroutine adjust secondary slotregister
C0302:		PUSH	AF
		LD	A,D
		AND	0C0H
		LD	C,A
		POP	AF
		PUSH	AF
		LD	D,A
		IN	A,(0A8H)
		LD	B,A
		AND	3FH
		OR	C
		OUT	(0A8H),A
		LD	A,D
		RRCA
		RRCA
		AND	03H
		LD	D,A
		LD	A,0ABH
J031A:		ADD	A,55H
		DEC	D
		JP	P,J031A
		AND	E
		LD	D,A
		LD	A,E
		CPL
		LD	H,A
		LD	A,(D_FFFF)
		CPL
		LD	L,A
		AND	H
		OR	D
		LD	(D_FFFF),A
		LD	A,B
		OUT	(0A8H),A
		POP	AF
		AND	03H
		RET

; Subroutine subrom init routine
C0336:		LD	HL,I29F6
		LD	DE,RG0SAV
		LD	BC,8
		LDIR				; initialize vdp register 0-7 defaults
		LD	DE,RG8SAV
		LD	BC,16
		LDIR				; initialize vdp register 8-23 defaults
		XOR	A
		LD	(DPPAGE),A		; vdp display page=0
		LD	(ACPAGE),A		; vdp active page=0
		CALL	C0512			; get my slotid
		LD	(EXBRSA),A
		LD	H,A
		LD	L,0F7H
		LD	(H_ERRP+0),HL
		LD	HL,I3C11
		LD	(H_ERRP+2),HL		; expand error messages
		LD	A,(EXPTBL+0)
		LD	(D_FFF7),A
		LD	A,2CH
		LD	(XSAVE+1),A		; initialize X border skip lightpen
		LD	A,26H
		LD	(YSAVE+1),A		; initialize Y border skip lightpen
		LD	A,0D3H			; no external audio mixing, video input 21 pin, internal sync, AV control=1, Ym control=full, S1=0, S0=0
		OUT	(0F7H),A		; initialize digitizer hardware
	IF CNTRY = 0
		LD	(AVCSAV),A
	ELSE
		CALL	C3CC3			; patch code european version
	ENDIF
		XOR	A
	IFNDEF FSA1FM	
		LD	C,0FFH			; assume internal devices should be enabled
		ELSE
		LD	C,0FDH
	ENDIF
		OUT	(0D8H),A
		LD	A,2
		OUT	(0D9H),A
		LD	HL,I050A
		LD	B,8
J0387:		IN	A,(0D9H)
		CP	(HL)
		JR	NZ,J0391
		INC	HL
		DJNZ	J0387
		RES	0,C			; JIS-1 rom found, disable internal JIS-1
J0391:		IN	A,(0C0H)
		CP	0FFH
		JR	Z,J0399
		RES	2,C			; MSX-AUDIO found, disable internal MSX-AUDIO
J0399:		IN	A,(0F7H)
		AND	77H			; b6-b4, b2-b0 are outputs which are readback, b7 and b3 are I/O with different input function
		CP	77H
		JR	Z,J03A3
		RES	3,C			; superimpose found, disable internal superimpose
J03A3:		IN	A,(0C8H)
		CP	0FFH
		JR	Z,J03AB
		RES	4,C			; MSX-MOTOR found, disable internal MSX-MOTOR interface
J03AB:		XOR	A
		OUT	(81H),A
		PUSH	AF
		POP	AF
		OUT	(81H),A
		PUSH	AF
		POP	AF
		OUT	(81H),A
		PUSH	AF
		POP	AF
		LD	A,40H
		OUT	(81H),A
		EX	(SP),HL
		EX	(SP),HL
		IN	A,(81H)
		AND	3FH
		CP	05H
		JR	NZ,J03C8
		RES	5,C			; MSX-RS232 found, disable internal MSX-RS232 interface
J03C8:		IN	A,(0BBH)
		CP	0FFH
		JR	Z,J03D0
		RES	6,C			; lightpen found, disable internal lightpen interface
J03D0:		LD	A,C
		OUT	(0F5H),A		; enable internal devices
		CALL	C1CF7			; select RP-501 bank 2
		LD	B,0
		CALL	C1CD2			; read RP-501 register 0
		CP	0AH
		JR	Z,J03E2
J03DF:		CALL	C0431			; initalize RP-501
J03E2:		CALL	C1CF7			; select RP-501 bank 2
		LD	B,3
		CALL	C1CD2			; read RP-501 register 3
		AND	01H
		PUSH	AF
		CALL	C1CDB			; read byte from RP-501 registers 4/5
		POP	BC
		CP	80+1			; screenwidth >80 ?
		JP	NC,J03DF		; yep, initialize
		OR	A			; screenwidth <1 ?
		JP	Z,J03DF			; yep, initialize
		DEC	B			; screenmode 1 ?
		JR	NZ,J0402		; nope,
		CP	32+1			; screenwidth >32 ?
		JP	NC,J03DF		; yep, initialize
J0402:		CALL	C1CF7			; select RP-501 bank 2
		LD	B,1
		CALL	C1CD2			; read RP-501 register 1
		LD	D,A
		CALL	C1CD2			; read RP-501 register 2
		LD	E,A
		CALL	C17F8			; setup screenposition
		CALL	C2A0E			; display msx startscreen
		LD	B,2
		LD	HL,0
J041A:		DEC	HL
		LD	A,L
		OR	H
		JR	NZ,J041A
		DJNZ	J041A			; wait
		CALL	C063C			; disable screendisplay
		XOR	A
		LD	BC,04000H
		CALL	C0977			; fill VRAM (active page)
		CALL	C0A1A			; setup screen 1 (INIT32)
		JP	J0F9F			; initialize pallette (INIPLT)

; Subroutine initialize bank 2 and 3 clockchip
C0431:
	IF CNTRY = 0
		LD	HL,I04FF
	ELSE
		LD	HL,I3CEC
	ENDIF
		LD	C,2			; 2 banks
J0436:		LD	B,13			; bank register
		LD	A,4
		SUB	C			; bank 2 or 3, alarm output OFF, CLOCK count stop
		CALL	C1A59			; write nibble to RP-501 (register 13)
J043E:		LD	B,13
J0440:		LD	A,13
		SUB	B
		OUT	(0B4H),A		; select RP-501 register
		LD	A,(HL)
		INC	HL
		CP	0FFH
		JR	NZ,J044D
		DEC	HL
		XOR	A
J044D:		OUT	(0B5H),A		; write RP-501 register
		DJNZ	J0440			; next register
		DEC	C
		JR	NZ,J0436		; next bank
		LD	B,13
		LD	A,08H			; bank 0, alarm output OFF, CLOCK count start
		CALL	C1A59			; write nibble to RP-501 (register 13)
		XOR	A			; all tests disabled
		CALL	C1A59			; write nibble to RP-501 (register 14)
		LD	A,0DH			; reset alarm registers,	leave fractions, 16 Hz pulse off, 1 Hz pulse off
		CALL	C1A59			; write nibble to RP-501 (register 15)
		LD	A,1
		CALL	C1CF9			; select RP-501 bank 1
		LD	B,10
		LD	A,1			; 24-hour clock
		JP	C1A59			; write nibble to RP-501 (register 10)

; Subroutine S.SETSCR (setup screen for basicstart)
J0470:		SCF				; set functionkey display according to startup value
		CALL	C0481			; restore saved settings
		LD	HL,M7EF2
		CALL	C05F6			; message to interpreter output
		LD	HL,M7EE4
		JP	C05F6			; message to interpreter output

; Subroutine S.SDFSCR (setup screen with functionkeys display disabled)
J0480:		XOR	A			; force functionkey display off

; Subroutine initialize settings stored in clockchip
; Input:  Cx set for normal functionkeys display, Cx reset for disabled functionkey display
C0481:		PUSH	AF
		CALL	C1CF7			; select RP-501 bank 2
		LD	B,3
		CALL	C1CD2			; read RP-501 register 3
		LD	C,A
		AND	01H			; startup screenmode
		POP	DE
		PUSH	AF
		PUSH	DE
		PUSH	BC
		LD	A,C
		RLCA
		RLCA
		AND	08H
		LD	C,A			; startup interlace mode in b3
		LD	A,(RG9SAV)
		AND	0F7H
		OR	C
		LD	B,A
		LD	C,9
		CALL	C0647			; write vdp register 9
		POP	BC
		CALL	C1CDB			; read byte from RP-501 registers 4/5
		LD	E,A			; startup screenwidth
		POP	AF
		PUSH	DE
		PUSH	AF
		CALL	C1CD2			; read RP-501 register 6
		LD	(FORCLR),A		; set foreground color
		LD	(ATRBYT),A		; set attribute color
		CALL	C1CD2			; read RP-501 register 7
		LD	(BAKCLR),A		; set background color
		CALL	C1CD2			; read RP-501 register 8
		LD	(BDRCLR),A		; set border color
		CALL	C1CD2			; read RP-501 register 9
		LD	C,A
		RRC	C			; functionkey display on/off
		SBC	A,A
		LD	B,A
		POP	AF			; functionkey display forced off ?
		JR	NC,J04CC		; yep,
		LD	A,B
J04CC:		LD	(CNSDFG),A
		RRC	C			; keyclick on/off
		SBC	A,A
		LD	(CLIKSW),A
	IF CSPRFIX = 1
		BIT	1,C			; cassette speed
		PUSH	BC
		LD	BC,5
		LD	HL,CS1200
		JR	Z,J04E3
		LD	HL,CS2400
J04E3:		LD	DE,LOW_
		LDIR
		POP	BC
		RRC	C			; printermode
		SBC	A,A
		LD	(NTMSXP),A
	ELSE
		RRC	C			; cassette speed (BUG, it is the printermode)
		PUSH	BC
		LD	BC,5
		LD	HL,CS1200
		JR	NC,J04E3
		LD	HL,CS2400
J04E3:		LD	DE,LOW_
		LDIR
		POP	BC
		RRC	C			; printermode (BUG, it is the cassette speed)
		SBC	A,A
		LD	(NTMSXP),A
	ENDIF
		POP	DE
		POP	AF			; startup screenmode
		LD	(OLDSCR),A
		LD	(SCRMOD),A
		PUSH	DE
		CALL	C09BF			; change screenmode, initialize pallette
		POP	DE
		JP	J1FFF

; Initial RP-501 data
I04FF:		DEFB	00AH			; id
		DEFB	000H			; adjust X
		DEFB	000H			; adjust Y
		DEFB	001H			; no interlace, screen 1
		DEFB	29 % 16
		DEFB	29 / 16			; screen width
		DEFB	00FH			; foreground color
		DEFB	004H			; background color
		DEFB	007H			; border color
		DEFB	003H			; cassette speed 1200, printer mode msx, key click on, function keys on
						; beep tone 0, volume 0
						; title color 0
						; native code 0
		DEFB	0FFH
		
; JIS1 test pattern
I050A:		DEFB	00H,40H,20H,10H,08H,04H,02H,01H

; Subroutine get my slotid
C0512:		PUSH	BC
		PUSH	HL
		IN	A,(0A8H)
		AND	03H
		LD	C,A
		LD	B,00H
		LD	HL,EXPTBL
		ADD	HL,BC
		OR	(HL)
		LD	C,A
		INC	HL
		INC	HL
		INC	HL
		INC	HL
		LD	A,(HL)
		AND	03H
		RLCA
		RLCA
		OR	C
		POP	HL
		POP	BC
		RET

J052E:		LD	IX,DERNMF
		JR	J0562

J0534:		LD	IX,DERSOO
		JR	J0562

J053A:		LD	IX,DERFAO
		JR	J0562

J0540:		LD	IX,SNERR
		JR	J0562

J0546:		LD	IX,FCERR
		JR	J0562

J054C:		LD	IX,MOERR
		JR	J0562

J0552:		LD	IX,TMERR
		JR	J0562

J0558:		LD	IX,OMERR
		JR	J0562

J055E:		LD	IX,DERFNF
J0562:		JP	J0618

C0565:		LD	IX,CHGET
		JR	C058D

C056B:		LD	IX,GICINI
		JR	C058D

J0571:		LD	IX,FNKSB
		JR	C058D

J0577:		LD	IX,OUTDO
		JR	C058D

C057D:		LD	IX,RDSLT
		JR	C058D

C0583:		LD	IX,WRSLT
		JR	C058D

C0589:		LD	IX,CKCNTC

C058D:		JP	C0258

C0590:		LD	IX,SNSMAT
		JR	C058D

J0596:		LD	IX,EOL
		JR	C058D

C059C:		LD	IX,SCAN1
		JR	J0618

C05A2:		LD	IX,SCAND
		JR	J0618

J05A8:		LD	IX,CRDO
		JR	J05F4

C05AE:		LD	IX,CRDONZ
		JR	J05F4

C05B4:		LD	IX,FRQINT
		JR	J05F4

C05BA:		LD	IX,FILEVL
		JR	J05F4

C05C0:		LD	IX,CLSFIL
		JR	J05F4

J05C6:		LD	IX,ERROR
		JR	J05F4

C05CC:		LD	IX,LINPRT
		JR	J05F4

C05D2:		LD	IX,CHKSTR
		JR	J05F4

C05D8:		LD	IX,PTRGET
		JR	J05F4

C05DE:		LD	IX,CONSUI
		JR	J05F4

C05E4:		LD	IX,SNGFLT
		JR	J05F4

C05EA:		LD	IX,STRINI
		JR	J05F4

J05F0:		LD	IX,FDBINT
J05F4:		JR	J0618

C05F6:		LD	IX,STROUT
		JR	J05F4

C05FC:		LD	IX,FRMEVL
		JR	J0618

C0602:		LD	IX,GETUIN
		JR	J0618

; Subroutine evaluate integer operand
C0608:		LD	IX,GETIN2
		JR	J0618

; Subroutine S.GETBYT
C060E:		LD	IX,GETBYT
		JR	J0618

C0614:		LD	IX,FRESTR
J0618:		CALL	C0262
		EI
		RET

; Subroutine change functionkey displaystatus and check if in textmode
; Output: Cx set when in textmode
C061D:		LD	(CNSDFG),A

; Subroutine check if in screenmode 2
C0620:		LD	A,(SCRMOD)
		CP	2
		RET

; Subroutine check if in screenmode 4
C0626:		LD	A,(SCRMOD)
		CP	4
		RET

; Subroutine check if new style MSX2 screen
C062C:		LD	A,(SCRMOD)
		CP	5
		RET

; Subroutine enable vdp displayoutput
; Remark: Beats me why the IE0 bit is explicitly set
J0632:		CALL	C1034			; wait for VR to pass
		LD	A,(RG1SAV)
		OR	60H			; IE0=1, BL=1 (enable VR interrupts, enable screen)
		JR	J0644

; Subroutine disable vdp display output
; Remark: Beats me why the b7 bit is explicitly reset
C063C:		CALL	C1034			; wait for VR to pass
		LD	A,(RG1SAV)
		AND	3FH			; b7=0, BL=0 (reset unused bit (TMS9918->4K/16K), disable screen)
J0644:		LD	B,A
		LD	C,1			; vdp register 1

; Subroutine S.WRTVDP
; Input: C = VDP register, B = data
C0647:		LD	A,I
		PUSH	AF			; store IFF2 flag
		PUSH	HL
		LD	A,C
		AND	A			; vdp register 0 ?
		JR	NZ,J0680		; nope, change vdp register
		LD	A,B
		LD	HL,RG0SAV
		XOR	(HL)
		AND	01H			; b0 (TMS 9918: external VDP input) changed ?
		JR	Z,J0680			; nope, change vdp register
		LD	HL,RG9SAV
		LD	A,(HL)
		AND	0CFH
		LD	(HL),A			; reset b5,b4 (S1,S0) -> normal MSX-VIDEO
		LD	A,B
		AND	01H
		RRCA
		RRCA
		RRCA
		RRCA
		OR	(HL)			; if TMS 9918 external VDP input then select Superimpose MSX-VIDEO (transparent parts)
		PUSH	BC
		LD	B,A			; new value register 9
		LD	C,9			; vdp register 9
		LD	HL,AVCSAV
		LD	A,(HL)
		AND	3FH
		LD	(HL),A			; reset b7,b6
		LD	A,B
		RLCA
		RLCA
		CPL
		AND	0C0H
		OR	(HL)
		OUT	(0F7H),A
		LD	(HL),A
		CALL	C0647			; write vdp register 9
		POP	BC
J0680:		LD	A,B
		DI
		OUT	(99H),A
		LD	A,C
		OR	80H
		OUT	(99H),A
		PUSH	BC
		PUSH	DE
		LD	D,B
		LD	A,C
		LD	B,0
		CP	08H
		JR	NC,J0698
		LD	HL,RG0SAV
		JR	J069F

J0698:		CP	18H
		JR	NC,J06A1
		LD	HL,RG8SAV-8
J069F:		ADD	HL,BC
		LD	(HL),D
J06A1:		POP	DE
		POP	BC
		POP	HL
		POP	AF			; restore IFF2 flag
		RET	PO			; maskable interrupts where disabled, quit
		EI				; enable maskable interrupts
		RET

; Subroutine S.SETPAG (setup display and active page)
C06A8:		LD	A,(SCRMOD)
		PUSH	AF
		PUSH	BC
		PUSH	DE
		PUSH	HL
		DI
		LD	HL,I06B8
		CALL	C09C9
		JR	J06F0

I06B8:		DEFW	C0B42			; SETTXT
		DEFW	C0B9C			; SETT32
		DEFW	C0BD2			; SETGRP
		DEFW	C0C22			; SETMLT
		DEFW	C0BD9			; setup for GRAPHIC 3 mode (screenmode 4)
		DEFW	C0C56			; setup for GRAPHIC 4 mode (screenmode 5)
		DEFW	C0C80			; setup for GRAPHIC 5 mode (screenmode 6)
		DEFW	C0C97			; setup for GRAPHIC 6 mode (screenmode 7)
		DEFW	C0CC7			; setup for GRAPHIC 7 mode (screenmode 8)

; Subroutine setup VDP registers and digitizehardware
J06CA:		PUSH	AF
		PUSH	BC
		PUSH	DE
		PUSH	HL
		DI
		XOR	A
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer to 0, autoincrement
		CALL	C1034			; wait for VR to pass
		LD	HL,RG0SAV
		LD	B,8
J06DE:		LD	A,(HL)
		INC	HL
		OUT	(9BH),A
		DJNZ	J06DE
		LD	HL,RG8SAV
		LD	B,16
J06E9:		LD	A,(HL)
		INC	HL
		OUT	(9BH),A
		DJNZ	J06E9
		EI
J06F0:		POP	HL
		POP	DE
		POP	BC
		POP	AF
		RET

; Subroutine S.CLRSPR
C06F5:		CALL	C0701			; initialize sprites
		PUSH	HL
		LD	HL,0
		CALL	C08E3			; set VRAM pointer for writing (active page)
		POP	HL
		RET

; Subroutine clear spritepatterntable, initialize planes and colortable
C0701:		LD	A,(RG1SAV)
		LD	B,A
		LD	C,1
		CALL	C0647			; write vdp register 1
		LD	HL,(PATBAS)
		LD	BC,256*8
		XOR	A
		CALL	C0977			; clear sprite patterns (fill VRAM (active page))

; Subroutine initialize spriteplanes and spritecolortable
C0714:		LD	A,(FORCLR)
		LD	E,A
		LD	HL,(ATRBAS)
		LD	BC,02000H		; 32 sprites, start with sprite 0
J071E:		CALL	C0626			; MSX1 screenmode ?
		LD	A,209
		JR	C,J0727			; yep,
		LD	A,217
J0727:		CALL	C08CB			; set sprite Y-coord (write VRAM (active page))
		INC	HL
		INC	HL			; leave sprite X-coord alone
		LD	A,C
		CALL	C08CB			; set sprite pattern (write VRAM (active page))
		INC	HL
		INC	C
		LD	A,(RG1SAV)
		RRCA
		RRCA				; 16*16 sprites ?
		JR	NC,J073C		; nope,
		INC	C
		INC	C
		INC	C			; yep, sprite+4
J073C:		LD	A,E
		CALL	C08CB			; set sprite color (write VRAM (active page))
		INC	HL
		DJNZ	J071E
		CALL	C0626			; MSX1 screenmode ?
		RET	C			; yep, quit
		LD	HL,(ATRBAS)
		LD	BC,512
		SBC	HL,BC
		LD	A,(FORCLR)
		JP	C0977			; initialize sprite colortable (fill VRAM (active page))

; Subroutine S.CALPAT
; Input:  A=patternnumber
; Output: HL=address of pattern
J0755:		LD	L,A
		LD	H,0
		ADD	HL,HL
		ADD	HL,HL
		ADD	HL,HL
		CALL	C077F			; get spritesize
		CP	8			; 16*16 sprites ?
		JR	Z,J0764			; nope,
		ADD	HL,HL
		ADD	HL,HL
J0764:		EX	DE,HL
		LD	HL,(PATBAS)
		ADD	HL,DE
		RET

; Subroutine S.CALATR
; Input:  A=planenumber
; Output: HL=address of plane

C076A:		LD	L,A
		LD	H,0
		ADD	HL,HL
		ADD	HL,HL
		EX	DE,HL
		LD	HL,(ATRBAS)
		ADD	HL,DE
		RET

; Subroutine get plane colorentry
; Input:  A=planenumber
C0775:		ADD	A,A
		ADD	A,A
		CALL	C076A			; get spritelane address
		LD	DE,-512
		ADD	HL,DE
		RET

; Subroutine S.GSPSIZ
C077F:		LD	A,(RG1SAV)
		RRCA
		RRCA
		LD	A,8
		RET	NC
		LD	A,32
		RET

; Subroutine initialize patterngenerator with msx characterset
C078A:		CALL	H_INIP
		LD	HL,(CGPBAS)
		CALL	C08E3			; set VRAM pointer for writing (active page)
		LD	A,(CGPNT+0)		; slotid charset
		LD	HL,(CGPNT+1)		; address charset
		LD	BC,256*8
		PUSH	AF
J079D:		POP	AF
		PUSH	AF
		PUSH	BC
		DI
		CALL	C057D			; call RDSLT mainrom
		EI
		POP	BC
		OUT	(98H),A
		INC	HL
		DEC	BC
		LD	A,C
		OR	B
		JR	NZ,J079D
		POP	AF
		RET

; Subroutine transfer from VRAM to RAM
; Input:  HL=VRAM base address, DE=RAM address, BC=size
; Remark: NOT USED
Q07B0:		CALL	C08F1			; set VRAM pointer for reading (active page)
		LD	A,C
		OR	A
		LD	A,B
		LD	B,C
		LD	C,98H
		EX	DE,HL
		JR	Z,J07BD
		INC	A			; setup for INIR
J07BD:		INIR
		DEC	A
		JR	NZ,J07BD
		EX	DE,HL
		RET

; Subroutine transfer from RAM to VRAM
; Input:  HL=RAM address, DE=VRAM base address, BC=size
; Remark: UNUSED CODE
Q07C4:		EX	DE,HL
		CALL	C08E3			; set VRAM pointer for writing (active page)
		EX	DE,HL
		LD	A,C
		OR	A
		LD	A,B
		LD	B,C
		LD	C,98H
		JR	Z,J07D2
		INC	A			; setup for OTIR
J07D2:		OTIR
		DEC	A
		JR	NZ,J07D2
		EX	DE,HL
		RET

; Subroutine S.GETPAT (copy characterpattern to PATWRK)
; Input:  A=charcode
C07D9:		LD	H,0
		LD	L,A
		ADD	HL,HL
		ADD	HL,HL
		ADD	HL,HL			; *8
		EX	DE,HL
		LD	HL,(CGPNT+1)
		ADD	HL,DE
		LD	DE,PATWRK
		LD	B,8
		LD	A,(CGPNT+0)
J07EC:		PUSH	AF
		PUSH	HL
		PUSH	DE
		PUSH	BC
		CALL	C057D			; call RDSLT mainrom
		EI
		POP	BC
		POP	DE
		POP	HL
		LD	(DE),A
		INC	DE
		INC	HL
		POP	AF
		DJNZ	J07EC
		RET

; Subroutine S.CLS
C07FE:		CALL	C0626			; vdp in MSX1 style screenmode ?
		JP	Z,C0895			; screenmode 4, clear screen in screenmode 2/4
		JR	NC,C0811		; new style MSX2 screenmode, clear screen in new style MSX2 screenmode
		CALL	C0620			; vdp in textmode ?
		JP	Z,C0895			; graphics mode 2, clear screen in screenmode 2/4
		JP	NC,C08AD		; graphics mode 3, clear screen in screenmode 3
		JR	C0864			; clear screen in textmode

; Subroutine clear screen for new style MSX2 screenmodes
C0811:		PUSH	AF
		PUSH	BC
		PUSH	DE
		PUSH	HL
		CALL	C2980			; wait for vdp command to complete
		LD	A,(BAKCLR)
		CALL	C084B			; convert color to colorregister value
		CALL	C29EB			; write colorregister
		XOR	A
		LD	H,A
		LD	L,A
		CALL	C29A5			; write DX,DY registers
		XOR	A
		CALL	C29DB			; write ARG register
		LD	A,(SCRMOD)
		AND	02H			; screenmode 6 or 7 ?
		LD	DE,256
		JR	Z,J0838			; nope, screenwidth 256 pixels
		LD	DE,512			; yep, screenwidth 512 pixels
J0838:		LD	HL,212
		CALL	C29C4			; write NX,NY registers
		LD	A,0C0H			; HMMV (high speed move) command
		CALL	C29E3			; execute vdp command
		CALL	C2980			; wait for vdp command to complete
		POP	HL
		POP	DE
		POP	BC
		POP	AF
		RET

; Subroutine calculate colorbyte
; Input:  A=color
; Output: A=colorbyte

C084B:		LD	B,A
		LD	A,(SCRMOD)
		CP	8			; screenmode 8 ?
		LD	A,B
		RET	Z			; yep, use all 8 bits and quit
		RLCA
		RLCA
		RLCA
		RLCA
		OR	B
		LD	B,A			; copy 4 lower bits in 4 higher bits
		LD	A,(SCRMOD)
		CP	6			; screenmode 6 ?
		LD	A,B
		RET	NZ			; nope, quit
		RRCA
		RRCA
		OR	B			; copy 2 lower bits in b2-b3, b4-b5 and b6-b7
		RET

; Subroutine S.CLRTXT (clear screen for textscreenmodes)
C0864:		LD	A,(SCRMOD)
		AND	A			; screenmode 0 ?
		LD	HL,(NAMBAS)
		JR	NZ,J087E		; nope, clear in screenmode 1
		LD	A,(LINLEN)
		CP	40+1
		JR	NC,J0879
		LD	BC,40*24
		JR	J0881

J0879:		LD	BC,80*24
		JR	J0881

J087E:		LD	BC,32*24
J0881:		LD	A,20H
		CALL	C0977			; fill VRAM (active page)
		CALL	C08C1			; cursorposition to home
		LD	HL,LINTTB
		LD	B,24
J088E:		LD	(HL),B
		INC	HL
		DJNZ	J088E			; initialize LINTTB
		JP	J0571			; handle functionkey display

; Subroutine clear screen for screenmode 2 and 4
C0895:		CALL	C099C			; write bordercolor register
		LD	BC,1800H
		PUSH	BC
		LD	HL,(GRPCOL)
		LD	A,(BAKCLR)
		CALL	C0977			; fill VRAM (active page)
		LD	HL,(GRPCGP)
		POP	BC
		XOR	A
J08AA:		JP	C0977			; fill VRAM (active page)

; Subroutine clear screen for screenmode 3
C08AD:		CALL	C099C			; write bordercolor register
		LD	HL,BAKCLR
		LD	A,(HL)
		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
		OR	(HL)
		LD	HL,(MLTCGP)
		LD	BC,0600H
		JR	J08AA


; Subroutine cursorposition to home
C08C1:		LD	HL,00101H
		LD	(CSRY),HL
		RET

; Subroutine MSX1 compatible WRTVRM
; Remark: UNUSED CODE
Q08C8:		CALL	C08DC			; adjust to 16 KB range

; Subroutine S.WRTVRM
; Input:  HL=VRAM base address
C08CB:		PUSH	AF
		CALL	C08E3			; set VRAM pointer for writing (active page)
		POP	AF
		OUT	(98H),A
		RET

; Subroutine MSX1 compatible RDVRM
C08D3:		CALL	C08DC			; adjust to 16 KB range

; Subroutine S.RDVRM
; Input:  HL=VRAM base address
C08D6:		CALL	C08F1			; set VRAM pointer for reading (active page)
		IN	A,(98H)
		RET

; Subroutine VRAM address in 16 Kb range
C08DC:		PUSH	AF
		LD	A,H
		AND	3FH
		LD	H,A
		POP	AF
		RET

; Subroutine setup vdp addressregister for active screenpage (write)
C08E3:		PUSH	BC
		PUSH	DE
		PUSH	HL
		EX	DE,HL
		CALL	C0914			; convert to absolute VRAM address
		LD	A,H
		AND	3FH
		OR	40H
		JR	J08FB

; Subroutine setup vdp addressregister for active screenpage (read)
C08F1:		PUSH	BC
		PUSH	DE
		PUSH	HL
		EX	DE,HL
		CALL	C0914			; convert to absolute VRAM address
		LD	A,H
		AND	3FH
J08FB:		PUSH	AF
		LD	A,H
		AND	0C0H
		OR	D
		RLCA
		RLCA
		DI
		OUT	(99H),A
		LD	A,14+128
		OUT	(99H),A
		LD	A,L
		OUT	(99H),A
		POP	AF
		OUT	(99H),A
		EI
		POP	HL
		POP	DE
		POP	BC
		RET

; Subroutine convert page base address to VRAM address
; Input:  DE=base address
; Output: DHL=VRAM address

C0914:		LD	A,(SCRMOD)
		LD	C,A			; current screenmode
		LD	A,(ACPAGE)
		AND	A			; active page is 0 ?
		LD	HL,I0925
		JP	NZ,J09CA		; nope,
		EX	DE,HL
		LD	D,A			; D=0 (b23-b16) VRAM address
		RET

I0925:		DEFW	C0937			; screenmode 0
		DEFW	C0945			; screenmode 1, 16 Kb per page
		DEFW	C0945			; screenmode 2, 16 Kb per page
		DEFW	C0945			; screenmode 3, 16 Kb per page
		DEFW	C0945			; screenmode 4, 16 Kb per page
		DEFW	C0944			; screenmode 5, 32 Kb per page
		DEFW	C0944			; screenmode 6, 32 Kb per page
		DEFW	C0943			; screenmode 7, 64 Kb per page
		DEFW	C0943			; screenmode 8, 64 Kb per page

C0937:		LD	A,(LINLEN)
		CP	40+1			; 40 col mode ?
		LD	A,(ACPAGE)
		JR	C,J0947			; yep, 4 Kb per page
		JR	J0946			; nope, 8 Kb per page

C0943:		ADD	A,A
C0944:		ADD	A,A
C0945:		ADD	A,A
J0946:		ADD	A,A
J0947:		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
		LD	H,A
		LD	A,0
		LD	L,A			; b7-b0 = 0
		ADC	A,A			; b16
		ADD	HL,DE
		LD	D,A			; b23-b16
		RET

; Subroutine S.CHGCLR
C0953:		CALL	C062C			; new style screenmode ?
		JP	NC,J0997		; yep,
		CALL	C09A5			; adjust forground and background color to 4 bits
		DEC	A			; screenmode 0 ?
		JP	M,J098A			; yep,
		PUSH	AF
		CALL	C099C			; write bordercolor register
		POP	AF
		RET	NZ			; screenmode 2 or 3, quit
		LD	HL,BAKCLR
		LD	A,(FORCLR)
		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
		OR	(HL)
		LD	HL,(T32COL)
		LD	BC,32

; Subroutine fill VRAM
; Input:  HL=VRAM base address, BC=size, A=fillbyte
C0977:		PUSH	AF
		CALL	C08E3			; set VRAM pointer for writing (active page)
		LD	A,C
		OR	A
		JR	Z,J0980
		INC	B
J0980:		POP	AF
J0981:		OUT	(98H),A
		DEC	C
		JP	NZ,J0981
		DJNZ	J0981
		RET

J098A:		LD	HL,BAKCLR
		LD	A,(FORCLR)
		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
		OR	(HL)
		JR	J099F

J0997:		CP	8
		CALL	NZ,C09A5		; nope, adjust forground and background color to 4 bits

; Subroutine setup bordercolor
C099C:		LD	A,(BDRCLR)
J099F:		LD	B,A
		LD	C,7
		JP	C0647			; write vdp register 7

; Subroutine update FORCLR and BAKCLR to 4 bit colorvalues



C09A5:		PUSH	AF
		LD	A,(BAKCLR)
		AND	0FH
		LD	(BAKCLR),A
		LD	A,(FORCLR)
		AND	0FH
		LD	(FORCLR),A
		POP	AF
		RET

; Subroutine CLS statement
; Remark: UNUSED CODE
Q09B8:		RET	NZ
		PUSH	HL
		CALL	C07FE			; clear screen
		POP	HL
		RET

; Subroutine S.CHGMDP (setup screenmode with paletteinit)
C09BF:		LD	HL,J0F9F
		PUSH	HL			; initialize pallette afterwards

; Subroutine call screenmode specific routine
; Input:  HL=jumptable
; Output: A=screenmode
J09C3:		CP	9			; screenmode valid ?
		RET	NC			; nope, quit
		LD	HL,I09D3

; Subroutine __________________________



C09C9:		LD	C,A
J09CA:		LD	B,0
		ADD	HL,BC
		ADD	HL,BC
		LD	C,(HL)
		INC	HL
		LD	H,(HL)
		LD	L,C
		JP	(HL)

I09D3:		DEFW	C09E5			; INITXT
		DEFW	C0A1A			; INIT32
		DEFW	C0A48			; INIGRP
		DEFW	C0A98			; INIMLT
		DEFW	C0A71			; initialize screenmode 4
		DEFW	C0AD6			; initialize screenmode 5
		DEFW	C0AE3			; initialize screenmode 6
		DEFW	C0AF0			; initialize screenmode 7
		DEFW	C0AFD			; initialize screenmode 8

; Subroutine S.INITXT (screenmode 0)
C09E5:		XOR	A
		LD	(OLDSCR),A		; force to textmode reverts to screenmode 0
		CALL	C0B22			; setup for screenmode change
		LD	A,(LINL40)
		LD	(LINLEN),A
		LD	HL,0800H
		LD	(TXTCOL),HL
		LD	HL,(TXTNAM)
		LD	(NAMBAS),HL
		LD	HL,(TXTCGP)
		CP	40+1
		JR	C,J0A08
		LD	HL,1000H
J0A08:		LD	(CGPBAS),HL
		CALL	C0B42			; SETTXT
J0A0E:		CALL	C0953			; change colors
		CALL	C0864			; clear screen in textmode
		CALL	C078A			; initialize characterpatterns
		JP	J0632			; enable screendisplay

; Subroutine S.INIT32 (screenmode 1)
C0A1A:		LD	A,1
		LD	(OLDSCR),A		; force to textmode reverts to screenmode 1
		CALL	C0B22			; setup for screenmode change
		LD	A,(LINL32)
		LD	(LINLEN),A
		LD	HL,(T32NAM)
		LD	(NAMBAS),HL
		LD	HL,(T32CGP)
		LD	(CGPBAS),HL
		LD	HL,(T32PAT)
		LD	(PATBAS),HL
		LD	HL,(T32ATR)
		LD	(ATRBAS),HL
		CALL	C0B9C			; SETT32
		CALL	C0714			; initialize spriteplanes and spritecolor table
		JR	J0A0E

; Subroutine S.INIGRP (screenmode 2)
C0A48:		LD	A,2
		CALL	C0B22			; setup for screenmode change
		LD	HL,(GRPATR)
		CALL	C0A82			; initialize ATRBAS,PATBAS,CGPBAS and NAMBAS
		PUSH	HL
		CALL	C0BD2			; SETGRP
J0A57:		POP	HL
		CALL	C08E3			; set VRAM pointer for writing (active page)
		XOR	A
		LD	B,3
J0A5E:		OUT	(98H),A
		INC	A
		JR	NZ,J0A5E
		DJNZ	J0A5E
		CALL	C0895			; clear screen in screenmode 2/4
J0A68:		CALL	C0953			; change colors
		CALL	C0714			; initialize spriteplanes and spritecolor table
		JP	J0632			; enable screendisplay

; Subroutine screenmode 4
C0A71:		LD	A,4
		CALL	C0B22			; setup for screenmode change
		LD	HL,1E00H
		CALL	C0A82			; initialize ATRBAS,PATBAS,CGPBAS and NAMBAS
		PUSH	HL
		CALL	C0BD9			; setup for GRAPHIC 3 mode (screenmode 4)
		JR	J0A57

; Subroutine setup ATRBAS,PATBAS,CGPBAS and NAMBAS
C0A82:		LD	(ATRBAS),HL
		LD	HL,(GRPPAT)
		LD	(PATBAS),HL
		LD	HL,(GRPCGP)
		LD	(CGPBAS),HL
		LD	HL,(GRPNAM)
		LD	(NAMBAS),HL
		RET

; Subroutine S.INIMLT (screenmode 3)
C0A98:		LD	A,3
		CALL	C0B22			; setup for screenmode change
		LD	HL,(MLTPAT)
		LD	(PATBAS),HL
		LD	HL,(MLTATR)
		LD	(ATRBAS),HL
		LD	HL,(MLTCGP)
		LD	(CGPBAS),HL
		LD	HL,(MLTNAM)
		LD	(NAMBAS),HL
		PUSH	HL
		CALL	C0C22			; SETMLT
		POP	HL
		CALL	C08E3			; set VRAM pointer for writing (active page)
		LD	DE,6
J0AC0:		LD	C,4
J0AC2:		LD	A,D
		LD	B,32
J0AC5:		OUT	(98H),A
		INC	A
		DJNZ	J0AC5
		DEC	C
		JR	NZ,J0AC2
		LD	D,A
		DEC	E
		JR	NZ,J0AC0
		CALL	C08AD			; clear screen in screenmode 3
		JR	J0A68

; Subroutine screenmode 5
C0AD6:		LD	A,5
		LD	DE,07876H
		CALL	C0B14			; initialize PATBAS,ATRBAS and NAMBAS, setup for screenmode change
		CALL	C0C56			; setup for GRAPHIC 4 mode (screenmode 5)
		JR	J0B08

; Subroutine screenmode 6
C0AE3:		LD	A,6
		LD	DE,07876H
		CALL	C0B14			; initialize PATBAS,ATRBAS and NAMBAS, setup for screenmode change
		CALL	C0C80			; setup for GRAPHIC 5 mode (screenmode 6)
		JR	J0B08

; Subroutine screenmode 7
C0AF0:		LD	A,7
		LD	DE,0F0FAH
		CALL	C0B14			; initialize PATBAS,ATRBAS and NAMBAS, setup for screenmode change
		CALL	C0C97			; setup for GRAPHIC 6 mode (screenmode 7)
		JR	J0B08

; Subroutine screenmode 8
C0AFD:		LD	A,8
		LD	DE,0F0FAH
		CALL	C0B14			; initialize PATBAS,ATRBAS and NAMBAS, setup for screenmode change
		CALL	C0CC7			; setup for GRAPHIC 7 mode (screenmode 8)
J0B08:		CALL	C0953			; change colors
		CALL	C0811
		CALL	C0714			; initialize spriteplanes and spritecolor table
		JP	J0632			; enable screendisplay


; Subroutine setup PATBAS,ATRBAS and NAMBAS, disable display, scroll and MSX2+ colormodes
; Input:  E = highbyte ATRBAS, D = highbyte PATBAS
C0B14:		LD	L,0
		LD	H,D
		LD	(PATBAS),HL
		LD	H,E
		LD	(ATRBAS),HL
		LD	H,L
		LD	(NAMBAS),HL

; Subroutine disable display
C0B22:		LD	(SCRMOD),A		; set current screenmode
		XOR	A
		LD	(DPPAGE),A		; display page=0
		LD	(ACPAGE),A		; active page=0
		LD	HL,256
		LD	(ASPCT1),HL
		LD	(ASPCT2),HL		; 1:1 ratio for circle
		CALL	C2980			; wait for vdp command to complete
		CALL	C063C			; disable screendisplay
		LD	B,0
		LD	C,23
		JP	C0647			; write vdp register 23 (reset vertical scroll)

; Subroutine S.SETTXT
C0B42:		CALL	C0D2D			; set vertical screensize to 192 lines
		LD	A,(LINLEN)
		CP	40+1
		LD	B,00H
		JR	C,J0B50
		LD	B,04H
J0B50:		LD	C,10H
		CALL	C0D3F			; change mode bits
		LD	A,(LINLEN)
		CP	40+1
		LD	A,(DPPAGE)
		JR	NC,J0B72
		ADD	A,A
		LD	HL,(TXTCGP)
		LD	B,0
		CALL	C0CE6			; set pattern generator table address
		ADD	A,A
		LD	HL,(TXTNAM)
		CALL	C0CDA			; set pattern name table address
		JP	J06CA			; update vdp registers from backup

J0B72:		LD	A,(DPPAGE)
		PUSH	AF
		ADD	A,A
		ADD	A,A
		LD	B,00H
		LD	HL,(CGPBAS)
		CALL	C0CE6			; set pattern generator table address
		ADD	A,A
		LD	B,3
		LD	HL,(TXTNAM)
		CALL	C0CDA			; set pattern name table address
		POP	AF
		LD	HL,(TXTCOL)
		LD	E,00H
		SRL	A
		RR	E
		LD	D,A
		LD	B,07H
		CALL	C0CF4			; set color table address
		JP	J06CA			; update vdp registers from backup

; Subroutine S.SETT32
C0B9C:		CALL	C0D2D			; set vertical screensize to 192 lines
		LD	BC,0000H
		CALL	C0D3F			; change mode bits
		LD	A,(DPPAGE)
		LD	C,A
		LD	B,00H
		LD	HL,(T32CGP)
		ADD	A,A
		ADD	A,A
		ADD	A,A
		CALL	C0CE6			; set pattern generator table address
		LD	HL,(T32PAT)
		CALL	C0D20			; set sprite pattern generator table address
		LD	HL,(T32NAM)
		ADD	A,A
		CALL	C0CDA			; set pattern name table address
		LD	E,B
		LD	D,C
		LD	HL,(T32COL)
		CALL	C0CF4			; set color table address
		SRL	D
		RR	E
		LD	HL,(T32ATR)
		JR	J0C1C			; set sprite attribute table address and update vdp registers from backup

; Subroutine S.SETGRP
C0BD2:		XOR	A
		PUSH	AF
		LD	BC,0200H
		JR	J0BE0

; Subroutine setup vdp for screenmode 4
C0BD9:		LD	A,3
		OR	A
		PUSH	AF
		LD	BC,0400H
J0BE0:		CALL	C0D3F			; change mode bits
		CALL	C0D2D			; set vertical screensize to 192 lines
		LD	A,(DPPAGE)
		LD	C,A
		LD	B,3
		LD	HL,(GRPCGP)
		ADD	A,A
		ADD	A,A
		ADD	A,A
		CALL	C0CE6			; set pattern generator table address
		LD	HL,(GRPPAT)
		CALL	C0D20			; set sprite pattern generator table address
		LD	HL,(GRPNAM)
		ADD	A,A
		LD	B,0
		CALL	C0CDA			; set pattern name table address
		LD	E,B
		LD	D,C
		LD	B,7FH
		LD	HL,(GRPCOL)
		CALL	C0CF4			; set color table address
		SRL	D
		RR	E
		POP	AF
		LD	B,A
		LD	HL,(GRPATR)
		JR	Z,J0C1C			; screenmode 2, set sprite attribute table address and update vdp registers from backup
		LD	HL,(ATRBAS)
J0C1C:		CALL	C0D0B			; set sprite attribute table address
		JP	J06CA			; update vdp registers from backup

; Subroutine S.SETMLT
C0C22:		CALL	C0D2D			; set vertical screensize to 192 lines
		LD	BC,0008H
		CALL	C0D3F			; change mode bits
		LD	A,(DPPAGE)
		LD	C,A
		LD	B,00H
		LD	HL,(MLTCGP)
		ADD	A,A
		ADD	A,A
		ADD	A,A
		CALL	C0CE6			; set pattern generator table address
		LD	HL,(MLTPAT)
		CALL	C0D20			; set sprite pattern generator table address
		LD	HL,(MLTNAM)
		ADD	A,A
		LD	B,0
		CALL	C0CDA			; set pattern name table address
		LD	E,B
		LD	D,C
		SRL	D
		RR	E
		LD	B,00H
		LD	HL,(MLTATR)
		JR	J0C1C			; set sprite attribute table address and update vdp registers from backup

; Subroutine setup vdp for screenmode 5
C0C56:		CALL	C0D36			; set vertical screensize to 212 lines
		LD	BC,0600H
		CALL	C0D3F			; change mode bits
J0C5F:		LD	A,(DPPAGE)
		LD	C,A
		LD	B,00H
		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
		CALL	C0D1D			; set sprite pattern generator table address to PATBAS
		LD	HL,(NAMBAS)
		ADD	A,A
		LD	B,1FH
		CALL	C0CDA			; set pattern name table address
		LD	D,C
J0C76:		LD	E,00H
		LD	B,03H
		CALL	C0D08			; set sprite attribute table address to ATRBAS
		JP	J06CA			; update vdp registers from backup

; Subroutine setup vdp for screenmode 6
C0C80:		CALL	C0D36			; set vertical screensize to 212 lines
		LD	BC,0800H
		CALL	C0D3F			; change mode bits
		LD	HL,512
		LD	(ASPCT1),HL
		LD	HL,128
		LD	(ASPCT2),HL		; 2:1 ratio for circle
		JR	J0C5F

; Subroutine setup vdp for screenmode 7
C0C97:		CALL	C0D36			; set vertical screensize to 212 lines
		LD	BC,0A00H
		CALL	C0D3F			; change mode bits
		LD	HL,512
		LD	(ASPCT1),HL
		LD	HL,128
		LD	(ASPCT2),HL		; 2:1 ratio for circle
J0CAC:		LD	A,(DPPAGE)
		LD	C,A
		LD	B,00H
		RRCA
		RRCA
		RRCA
		AND	0E0H
		CALL	C0D1D			; set sprite pattern generator table address to PATBAS
		LD	HL,(NAMBAS)
		LD	B,1FH
		CALL	C0CDA			; set pattern name table address
		LD	A,C
		ADD	A,A
		LD	D,A
		JR	J0C76

; Subroutine setup vdp for screenmode 8
C0CC7:		CALL	C0D36			; set vertical screensize to 212 lines
		LD	BC,0E00H
		CALL	C0D3F			; change mode bits
		LD	A,(RG2SAV)
		SRL	A
		LD	(RG2SAV),A		; ????
		JR	J0CAC

; Subroutine setup vdp patternnametable address
C0CDA:		LD	L,A
		SRL	H
		SRL	H
		ADD	A,H
		OR	B
		LD	(RG2SAV),A
		LD	A,L
		RET

; Subroutine setup vdp patterngenerator address
C0CE6:		LD	L,A
		SRL	H
		SRL	H
		SRL	H
		ADD	A,H
		OR	B
		LD	(RG4SAV),A
		LD	A,L
		RET

; Subroutine setup vdp colortable address
C0CF4:		PUSH	AF
		XOR	A
		ADD	HL,HL
		ADC	A,A
		ADD	HL,HL
		ADC	A,A
		LD	L,H
		LD	H,A
		ADD	HL,DE
		LD	A,L
		OR	B
		LD	(RG3SAV),A
		LD	A,H
		LD	(RG10SA),A
		POP	AF
		RET

; Subroutine setup vdp spriteattribute address with ATRBAS
C0D08:		LD	HL,(ATRBAS)

; Subroutine setup vdp spriteattribute address
C0D0B:		PUSH	AF
		XOR	A
		ADD	HL,HL
		ADC	A,A
		LD	L,H
		LD	H,A
		ADD	HL,DE
		LD	A,L
		OR	B
		LD	(RG5SAV),A
		LD	A,H
		LD	(RG11SA),A
		POP	AF
		RET

; Subroutine setup vdp spritepattern address with PATBAS
C0D1D:		LD	HL,(PATBAS)

; Subroutine setup vdp spritepattern address
; Input:  A = ?, HL = address
C0D20:		LD	L,A
		SRL	H
		SRL	H
		SRL	H
		ADD	A,H
		LD	(RG6SAV),A
		LD	A,L
		RET

; Subroutine set 192 lines
C0D2D:		LD	A,(RG9SAV)
		AND	7FH
		LD	(RG9SAV),A
		RET

; Subroutine set 212 lines
C0D36:		LD	A,(RG9SAV)
		OR	80H
		LD	(RG9SAV),A
		RET

; Subroutine set VDP mode bits
C0D3F:		LD	A,(RG0SAV)
		AND	0F1H
		OR	B
		LD	(RG0SAV),A
		LD	A,(RG1SAV)
		AND	0E7H
		OR	C
		LD	(RG1SAV),A
		RET

; Subroutine S.DSPFNK
J0D52:		LD	A,0FFH
		CALL	C061D			; set functionkey display flag and check if in textmode
		RET	NC			; not in textmode, quit
		PUSH	HL
		LD	A,(CSRY)
		LD	HL,CRTCNT
		CP	(HL)			; cursor on last line ?
		LD	A,10
		JR	NZ,J0D65
		RST	S_OUTDO			; yep, give linefeed to create empty line
J0D65:		LD	A,(NEWKEY+6)
		RRCA				; SHIFT key pressed ?
		LD	HL,FNKSTR+0*16
		LD	A,1
		JR	C,J0D74			; nope, display F1-F5
		LD	HL,FNKSTR+5*16
		XOR	A			; yep, display F6-F10
J0D74:		LD	(FNKSWI),A
		CALL	C0DE2			; empty line in LINWRK
		LD	C,5			; 5 functionkeys
		LD	A,(LINLEN)
		CP	40+1			; screenwidth >40 ?
		JR	NC,J0DA2		; yep, do functionkey display in two parts
		CALL	C0DF0			; calculate size for one functionkey
		JR	C,J0D9A			; no space, display nothing
J0D88:		PUSH	BC
		LD	C,0
J0D8B:		CALL	C0DFE			; put char in LINWRK if printable
		DJNZ	J0D8B
		LD	A,16
		SUB	C
		LD	C,A
		ADD	HL,BC			; to next functionkey definition
		INC	DE			; space between functionkeys
		POP	BC
		DEC	C
		JR	NZ,J0D88		; next functionkey
J0D9A:		LD	HL,(CRTCNT)
		CALL	C0ED7			; write line from LINWRK to VRAM
		POP	HL
		RET

J0DA2:		PUSH	AF
		CALL	C0DF0			; calculate size for one functionkey
		POP	AF
		SUB	40
J0DA9:		PUSH	BC
		LD	C,0
J0DAC:		EX	AF,AF'
		CALL	C0DFE			; put char in LINWRK if printable
		EX	AF,AF'
		DEC	A			; first part of line full ?
		CALL	Z,C0DD2			; yep, write line from LINWRK to VRAM (first part), clear LINWRK
		DJNZ	J0DAC
		DEC	A			; first part of line full ?
		JR	NZ,J0DBE
		CALL	C0DD2			; yep, write line from LINWRK to VRAM (first part), clear LINWRK
		DEC	DE
J0DBE:		EX	AF,AF'
		LD	A,16
		SUB	C
		LD	C,A
		ADD	HL,BC			; to next functionkey definition
		EX	AF,AF'
		INC	DE			; space between functionkeys
		POP	BC
		DEC	C
		JR	NZ,J0DA9		; next functionkey
		LD	HL,(CRTCNT)
		CALL	C0EF2			; write line from LINWRK to VRAM, second part
		POP	HL
		RET

; Subroutine copy LINWRK to first part of line and clear LINWRK
C0DD2:		PUSH	BC
		PUSH	HL
		LD	HL,(CRTCNT)
		CALL	C0ED8			; write line from LINWRK to VRAM, first part
		CALL	C0DE2			; empty line in LINWRK
		POP	HL
		POP	BC
		LD	A,0FFH
		RET

; Subroutine fill LINWRK with spaces
C0DE2:		LD	DE,LINWRK
		PUSH	DE
		LD	B,40
		LD	A,' '
J0DEA:		LD	(DE),A
		INC	DE
		DJNZ	J0DEA
		POP	DE
		RET

; Subroutine calculate space for functionkey to display
; Input:  A=screenwidth
C0DF0:		SUB	4
		RET	C
		LD	B,-1
J0DF5:		INC	B
		SUB	5
		JR	NC,J0DF5
		LD	A,B
		SUB	1
		RET

; Subroutine put char in LINWRK if printable
C0DFE:		LD	A,(HL)
		INC	HL
		INC	C
		CALL	C12A5			; check if graphical header char
		JR	NC,C0DFE		; graphic header char, next char
		JR	NZ,J0E0C		; graphic char, ok
		CP	20H			; control char ?
		JR	C,J0E0D			; yep,
J0E0C:		LD	(DE),A
J0E0D:		INC	DE
		RET

; Subroutine S.DELLNO
; Input:  L = linenumber
J0E0F:		CALL	C0F00			; get number of screen lines
		SUB	L			; linenumber valid ?
		RET	C			; nope, quit
		JP	Z,J0EA9			; last line, clear line and quit
		PUSH	HL
		PUSH	AF
		LD	C,A
		LD	B,0
		CALL	C0F0A			; get LINTTB entry of line
		LD	L,E
		LD	H,D
		INC	HL
		LDIR
		LD	HL,FSTPOS
		DEC	(HL)
		POP	AF
		POP	HL
		PUSH	AF
		LD	A,(LINLEN)
		CP	40+1			; screenwidth >40 ?
		JR	NC,J0E43		; yep, do it in two parts
		POP	AF
J0E33:		PUSH	AF
		INC	L
		CALL	C0EAE			; read line from VRAM to LINWRK
		DEC	L
		CALL	C0ED7			; write line from LINWRK to VRAM
		INC	L
		POP	AF
		DEC	A
		JR	NZ,J0E33		; next line
		JR	J0E5A			; clear line and quit

J0E43:		POP	AF
J0E44:		PUSH	AF
		INC	L
		CALL	C0EAF			; read line from VRAM to LINWRK, first part
		DEC	L
		CALL	C0ED8			; write line from LINWRK to VRAM, first part
		INC	L
		CALL	C0EC8			; read line from VRAM to LINWRK, second part
		DEC	L
		CALL	C0EF2			; write line from LINWRK to VRAM, second part
		INC	L
		POP	AF
		DEC	A
		JR	NZ,J0E44		; next line
J0E5A:		JP	J0EA9			; clear line and quit

; Subroutine S.INSLNO
J0E5D:		CALL	C0F00			; get number of screen lines
		LD	H,A
		SUB	L			; linenumber valid ?
		RET	C			; nope, quit
		JP	Z,J0EA9			; last line, clear line and quit
		LD	L,H
		PUSH	HL
		PUSH	AF
		LD	C,A
		LD	B,0
		CALL	C0F0A			; get LINTTB entry of line
		LD	L,E
		LD	H,D
		PUSH	HL
		DEC	HL
		LDDR
		POP	HL
		LD	(HL),H
		POP	AF
		POP	HL
		PUSH	AF
		LD	A,(LINLEN)
		CP	40+1			; screenwidth >40 ?
		JR	NC,J0E92		; yep, do it in two parts
		POP	AF
J0E82:		PUSH	AF
		DEC	L
		CALL	C0EAE			; read line from VRAM to LINWRK
		INC	L
		CALL	C0ED7			; write line from LINWRK to VRAM
		DEC	L
		POP	AF
		DEC	A
		JR	NZ,J0E82		; next line
		JR	J0EA9			; clear line and quit

J0E92:		POP	AF
J0E93:		PUSH	AF
		DEC	L
		CALL	C0EAF			; read line from VRAM to LINWRK, first part
		INC	L
		CALL	C0ED8			; write line from LINWRK to VRAM, first part
		DEC	L
		CALL	C0EC8			; read line from VRAM to LINWRK, second part
		INC	L
		CALL	C0EF2			; write line from LINWRK to VRAM, second part
		DEC	L
		POP	AF
		DEC	A
		JR	NZ,J0E93		; next line
J0EA9:		LD	H,1			; column 1
		JP	J0596			; clear to end of line

; Subroutine copy line to LINWRK
C0EAE:		DEFB	0F6H			; OR xx, skip next instruction

; Subroutine copy line to LINWRK, first part
C0EAF:		XOR	A
		PUSH	HL
		PUSH	AF
		LD	H,1			; column = 1
		CALL	C0F35			; convert coordinate to VRAM address
		POP	AF
		LD	A,(LINLEN)
		JR	NZ,J0EBF
		SUB	40
J0EBF:		LD	B,A
J0EC0:		LD	DE,LINWRK
		CALL	C0F22			; read string from VRAM
		POP	HL
		RET

; Subroutine copy second part line to LINWRK
C0EC8:		PUSH	HL
		LD	A,(LINLEN)
		SUB	40
		INC	A
		LD	H,A			; column
		CALL	C0F35			; convert coordinate to VRAM address
		LD	B,40
		JR	J0EC0			; read string from VRAM in LINWRK

; Subroutine copy LINWRK to line
C0ED7:		DEFB	0F6H			; OR xx, skip next instruction

; Subroutine copy LINWRK to first part of line
C0ED8:		XOR	A
		PUSH	HL
		PUSH	AF
		LD	H,1			; column = 1
		CALL	C0F35			; convert coordinate to VRAM address
		POP	AF
		LD	A,(LINLEN)
		JR	NZ,J0EE8
		SUB	40
J0EE8:		LD	B,A
J0EE9:		EX	DE,HL
		LD	HL,LINWRK
		CALL	C0F2B			; write string to VRAM
		POP	HL
		RET

; Subroutine copy LINWRK to second part of line
C0EF2:		PUSH	HL
		LD	A,(LINLEN)
		SUB	40-1
		LD	H,A			; column
		CALL	C0F35			; convert coordinate to VRAM address
		LD	B,40
		JR	J0EE9			; write string in LINWRK to VRAM

; Subroutine get last linenumber
C0F00:		LD	A,(CNSDFG)
		PUSH	HL
		LD	HL,CRTCNT
		ADD	A,(HL)
		POP	HL
		RET

; Subroutine get LINTTB entry
; Input:  L=linenumber (1 based)
C0F0A:		PUSH	HL
		LD	DE,LINTTB-1
		LD	H,0
		ADD	HL,DE
		LD	A,(HL)
		EX	DE,HL
		POP	HL
		AND	A
		RET

; Subroutine S.PUTVRM (put char on display)
; Input:  HL=coords, C=char
J0F16:		PUSH	HL
		CALL	C0F35			; convert coordinate to VRAM address
		CALL	C08E3			; set VRAM pointer for writing (active page)
		LD	A,C
		OUT	(98H),A
		POP	HL
		RET

; Subroutine copy vram to ram
; Input:  HL=vram base address, DE=ram address, B=size
C0F22:		CALL	C08F1			; set VRAM pointer for reading (active page)
		EX	DE,HL
		LD	C,98H
		INIR
		RET

; Subroutine copy ram to vram
; Input:  HL=ram address, DE=vram base address, B=size
C0F2B:		EX	DE,HL
		CALL	C08E3			; set VRAM pointer for writing (active page)
		EX	DE,HL
		LD	C,98H
		OTIR
		RET

; Subroutine convert screenlocation to VRAM address
; Input:  H=colnumber,L=linenumber
C0F35:		PUSH	BC
		DEC	H
		DEC	L
		LD	E,H
		LD	H,0
		LD	D,H
		ADD	HL,HL
		ADD	HL,HL
		ADD	HL,HL
		LD	C,L
		LD	B,H			; *8
		ADD	HL,HL
		ADD	HL,HL			; *32
		LD	A,(SCRMOD)
		AND	A			; screenmode 0 ?
		LD	A,(LINLEN)
		JR	Z,J0F50			; yep,
		SUB	32+2
		JR	J0F5D

J0F50:		CP	40+1			; 40 colum mode ?
		JR	C,J0F5A			; yep, *40
		ADD	HL,BC
		ADD	HL,HL			; *80
		SUB	80+2
		JR	J0F5D

J0F5A:		ADD	HL,BC
		SUB	40+2
J0F5D:		ADD	HL,DE
		CPL
		AND	A
		RRA
		LD	E,A
		ADD	HL,DE
		EX	DE,HL
		LD	HL,(NAMBAS)
		ADD	HL,DE
		POP	BC
		RET

; Subroutine S.GETPLT (get palette values)
; Input:  A=palletenumber
C0F6A:		PUSH	HL
		LD	D,A			; color
		CALL	C0FF9			; get pallette table address in VRAM
		CALL	C08F1			; set VRAM pointer for reading (active page)
		EX	(SP),HL
		EX	(SP),HL			; wait
		IN	A,(98H)
		LD	B,A
		NOP
		NOP
		IN	A,(98H)
		LD	C,A
		POP	HL
		RET

; Subroutine S.RSTPLT (restore all palettes)
J0F7E:		PUSH	HL
		LD	D,0			; color 0
		CALL	C0FF9			; get pallette table address in VRAM
		CALL	C08F1			; set VRAM pointer for reading (active page)
		LD	B,D
		LD	C,16
		CALL	C0647			; write vdp register 16
		LD	B,16			; 16 colors
J0F8F:		IN	A,(98H)			; read VRAM
		NOP				; wait
		OUT	(9AH),A			; write pallette register
		NOP				; wait
		IN	A,(98H)			; read VRAM
		NOP				; wait
		OUT	(9AH),A			; write pallette register
		NOP				; wait
		DJNZ	J0F8F			; next color
		POP	HL
		RET

; Subroutine S.INIPLT (initialize all palettes)
J0F9F:		PUSH	HL
		LD	HL,I0FB3
		LD	B,16			; 16 colors
		LD	D,0			; starting with color 0
J0FA7:		LD	A,(HL)
		INC	HL
		LD	E,(HL)
		INC	HL
		CALL	C0FD3			; set pallette values of color (SETPLT)
		INC	D
		DJNZ	J0FA7			; next color
		POP	HL
		RET

I0FB3:		DEFB	00H,00H
		DEFB	00H,00H
		DEFB	11H,06H
		DEFB	33H,07H
		DEFB	17H,01H
		DEFB	27H,03H
		DEFB	51H,01H
		DEFB	27H,06H
		DEFB	71H,01H
		DEFB	73H,03H
		DEFB	61H,06H
		DEFB	64H,06H
		DEFB	11H,04H
		DEFB	65H,02H
		DEFB	55H,05H
		DEFB	77H,07H

; Subroutine S.SETPLT (setup palette)
; Input:  D=palettenumber
C0FD3:		PUSH	BC
		PUSH	HL
		LD	B,D
		LD	C,16
		PUSH	AF
		CALL	C0647			; write vdp register 16
		POP	AF
		LD	C,9AH
		OUT	(C),A
		PUSH	AF
		POP	AF
		OUT	(C),E
		CALL	C0FF9
		PUSH	AF
		CALL	C08E3			; set VRAM pointer for writing (active page)
		EX	(SP),HL
		EX	(SP),HL			; wait
		POP	AF
		OUT	(98H),A			; write VRAM
		PUSH	AF
		POP	AF			; wait
		LD	A,E
		OUT	(98H),A			; write VRAM
		POP	HL
		POP	BC
		RET

; Subroutine get palletetable address in vram
C0FF9:		PUSH	AF
		LD	A,(SCRMOD)
		LD	HL,I1020
		PUSH	BC
		INC	A
		LD	C,A
		LD	B,0			; screenmode, 1 based
		DEC	A			; screenmode 0 ?
		JR	NZ,J1010		; nope,
		LD	A,(LINLEN)
		CP	40+1			; 40 column mode ?
		JR	NC,J1010		; nope,
		DEC	C			; yep, use first entry of table
J1010:		ADD	HL,BC
		ADD	HL,BC
		POP	BC
		LD	A,(HL)
		INC	HL
		LD	H,(HL)
		LD	L,A
		PUSH	DE
		LD	E,D
		LD	D,0
		ADD	HL,DE
		ADD	HL,DE
		POP	DE
		POP	AF
		RET

I1020:		DEFW	00400H			; screenmode 0, 40 cols
		DEFW	00F00H			; screenmode 0, 80 cols
		DEFW	02020H			; screenmode 1
		DEFW	01B80H			; screenmode 2
		DEFW	02020H			; screenmode 3
		DEFW	01B80H			; screenmode 4
		DEFW	07680H			; screenmode 5
		DEFW	07680H			; screenmode 6
		DEFW	0FA80H			; screenmode 7
		DEFW	0FA80H			; screenmode 8

; Subroutine wait for start of Vertikal Retrace
C1034:		LD	A,2
		CALL	C298B			; read vdp status register 2
		AND	40H			; in VR ?
		JR	NZ,C1034		; nope, wait
J103D:		LD	A,2
		CALL	C298B			; read vdp status register 2
		AND	40H			; VR passed ?
		JR	Z,J103D			; nope, wait
		RET

J1047:		POP	AF
		RET

; Subroutine S.KNJPRT
; Input:  BC=JIS code, A=outputmode
J1049:		PUSH	AF
		LD	A,B
		CP	21H
		JR	C,J1047
		CP	28H
		JR	C,J105B
		CP	30H
		JR	C,J1047
		CP	50H
		JR	NC,J1047
J105B:		LD	A,C
		CP	21H
		JR	C,J1047
		CP	7FH
		JR	NC,J1047
		POP	AF
		PUSH	HL
		PUSH	DE
		PUSH	BC
		PUSH	AF
		LD	A,C
		SUB	32
		LD	E,A
		LD	D,00H
		LD	A,B
		SUB	32
		LD	L,A
		LD	H,D
		LD	C,L
		LD	B,H
		ADD	HL,HL
		ADD	HL,BC
		ADD	HL,HL
		ADD	HL,HL
		ADD	HL,HL
		ADD	HL,HL
		ADD	HL,HL
		ADD	HL,DE
		CP	10H
		JR	C,J1084
		DEC	H
		DEC	H
J1084:		LD	A,L
		OUT	(0D8H),A
		ADD	HL,HL
		ADD	HL,HL
		LD	A,H
		OUT	(0D9H),A
		LD	BC,020D9H
		LD	HL,LINWRK
		INIR
		POP	AF
		PUSH	AF
		AND	A
		LD	C,2
		JR	Z,J10C5
		LD	HL,LINWRK
		LD	E,L
		LD	D,H
		DEC	A
		JR	Z,J10A4
		INC	HL
J10A4:		LD	C,2
J10A6:		LD	B,8
J10A8:		LD	A,(HL)
		LD	(DE),A
		INC	HL
		INC	HL
		INC	DE
		LD	A,B
		CP	5
		JR	NZ,J10B8
		PUSH	BC
		LD	BC,8
		ADD	HL,BC
		POP	BC
J10B8:		DJNZ	J10A8
		PUSH	BC
		LD	BC,-16
		ADD	HL,BC
		POP	BC
		DEC	C
		JR	NZ,J10A6
		LD	C,1
J10C5:		LD	HL,(GRPACX)
		PUSH	HL
		LD	HL,(GRPACY)
		PUSH	HL
		LD	DE,LINWRK
J10D0:		LD	HL,(GRPACY)
		PUSH	HL
		LD	HL,(GRPACX)
		PUSH	HL
		LD	B,2
J10DA:		PUSH	BC
		EX	DE,HL
		LD	DE,PATWRK
		LD	BC,8
		LDIR
		PUSH	HL
		CALL	C112D
		POP	DE
		POP	BC
		DJNZ	J10DA
		POP	HL
		LD	(GRPACX),HL
		POP	HL
		PUSH	BC
		LD	BC,8
		ADD	HL,BC
		POP	BC
		LD	(GRPACY),HL
		DEC	C
		JR	NZ,J10D0
		POP	HL
		LD	(GRPACY),HL
		POP	HL
		LD	BC,16
		ADD	HL,BC
		LD	(GRPACX),HL
I1109:		POP	AF
		POP	BC
		POP	DE
		POP	HL
		RET

; Subroutine S.GRPPRT
C110E:		PUSH	HL
		PUSH	DE
		PUSH	BC
		PUSH	AF
		LD	HL,I1109
		PUSH	HL
		CALL	C12A5			; check if graphical header char
		RET	NC			; graphic header char, quit
		JR	NZ,J1124		; graphic char, go
		CP	0DH
		JP	Z,J1210			; CR,
		CP	20H
		RET	C			; other control char, quit
J1124:		CALL	C07D9			; copy characterpattern to PATWRK
		LD	A,(FORCLR)
		LD	(ATRBYT),A

; Subroutine pattern to screen with coords handling
C112D:		LD	HL,(GRPACY)
		EX	DE,HL
		LD	BC,(GRPACX)
		CALL	C12C4
		RET	NC
		CALL	C1342
		LD	DE,PATWRK
		CALL	C2980			; wait for vdp command to complete
		PUSH	DE
		XOR	A
		CALL	C29DB			; write ARG register
		CALL	C1376
		LD	E,A
		CALL	C29A5			; write DX,DY registers
		LD	A,E
		SUB	0CDH
		LD	L,08H
		JR	C,J115A
		LD	L,A
		LD	A,07H
		SUB	L
		LD	L,A
J115A:		LD	B,L
		LD	DE,8
		LD	H,D
		CALL	C29C4			; write NX,NY registers
		POP	DE
		LD	A,(ATRBYT)
		LD	L,A
		LD	A,(BAKCLR)
		LD	H,A
		LD	A,(DE)
		ADD	A,A
		LD	C,A
		LD	A,H
		JR	NC,J1172
		LD	A,L
J1172:		CALL	C29EB			; write colorregister
		LD	A,(LOGOPR)
		AND	0FH
		OR	0B0H			; LMMC (logical move) command
		CALL	C29E3			; execute vdp command
		DI
		LD	A,44+128
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer to 44, non-autoincrement
		LD	A,C
		LD	C,9BH
		DI
		JR	J11A3

J118E:		DI
		LD	A,44+128
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer to 44, non-autoincrement
		INC	DE
		LD	A,(DE)
		RLCA
		JR	NC,J11A1
		OUT	(C),L
		JP	J11A3

J11A1:		OUT	(C),H
J11A3:		RLCA
		JR	NC,J11AB
		OUT	(C),L
		JP	J11AD

J11AB:		OUT	(C),H
J11AD:		RLCA
		JR	NC,J11B5
		OUT	(C),L
		JP	J11B7

J11B5:		OUT	(C),H
J11B7:		RLCA
		JR	NC,J11BF
		OUT	(C),L
		JP	J11C1

J11BF:		OUT	(C),H
J11C1:		RLCA
		JR	NC,J11C9
		OUT	(C),L
		JP	J11CB

J11C9:		OUT	(C),H
J11CB:		RLCA
		JR	NC,J11D3
		OUT	(C),L
		JP	J11D5

J11D3:		OUT	(C),H
J11D5:		RLCA
		JR	NC,J11DD
		OUT	(C),L
		JP	J11DF

J11DD:		OUT	(C),H
J11DF:		RLCA
		JR	NC,J11E7
		OUT	(C),L
		JP	J11E9

J11E7:		OUT	(C),H
J11E9:		EI
		DJNZ	J118E
		LD	A,(SCRMOD)
		AND	0FEH
		CP	6			; screen mode 6 or 7 ?
		JR	Z,J1200			; yep,
		LD	A,(GRPACX)
		ADD	A,8
		JR	C,J1210
		LD	(GRPACX),A
		RET

J1200:		LD	HL,(GRPACX)
		LD	BC,8
		ADD	HL,BC
		LD	A,H
		AND	0FEH
		JR	NZ,J1210
		LD	(GRPACX),HL
		RET

J1210:		LD	HL,0
		LD	(GRPACX),HL
		LD	A,(GRPACY)
		ADD	A,8
		CP	212
		JR	C,J1220
		XOR	A
J1220:		LD	(GRPACY),A
		RET

; Subroutine pattern to screen for startupscreen
; Input:  A=char
C1224:		PUSH	HL
		PUSH	DE
		PUSH	BC
		PUSH	AF
		LD	HL,I1109
		PUSH	HL
		CALL	C12A5			; check if graphic header char
		RET	NC			; graphic header char, quit
		JR	NZ,J123A		; graphic char, ok
		CP	07H
		JP	Z,J1A62			; BELL,
		CP	20H
		RET	C			; other control char, quit
J123A:		CALL	C07D9			; copy characterpattern to PATWRK
		LD	DE,PATWRK
		PUSH	DE
		CALL	C2980			; wait for vdp command to complete
		XOR	A
		CALL	C29DB			; write ARG register
		CALL	C1376
		CALL	C29A5			; write DX,DY registers
		LD	HL,8
		LD	DE,16
		CALL	C29C4			; write NX,NY registers
		POP	DE
		LD	HL,0103H
		LD	A,(DE)
		ADD	A,A
		LD	C,A
		LD	A,H
		JR	NC,J1262
		LD	A,L
J1262:		PUSH	AF
		CALL	C29EB			; write colorregister
		LD	A,0B0H			; LMMC (logical move) command
		CALL	C29E3			; execute vdp command
		DI
		LD	A,44+128
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer to 44, non-autoincrement
		POP	AF
		OUT	(9BH),A
		LD	A,C
		LD	C,9BH
		LD	B,8
		PUSH	BC
		DEC	B
		JR	J1285

J1280:		INC	DE
		LD	A,(DE)
		PUSH	BC
		LD	B,8
J1285:		RLCA
		JR	NC,J1290
		OUT	(C),L
		PUSH	AF
		POP	AF
		OUT	(C),L
		JR	J1296

J1290:		OUT	(C),H
		PUSH	AF
		POP	AF
		OUT	(C),H
J1296:		DJNZ	J1285
		POP	BC
		DJNZ	J1280
		LD	HL,(CLOC)
		LD	BC,16
		ADD	HL,BC
		JP	J1372

; Subroutine convert character
; Output: Cx reset if graphic header, 
C12A5:		PUSH	HL
		PUSH	AF
		LD	HL,GRPHED
		XOR	A
		CP	(HL)
		LD	(HL),A
		JR	Z,J12BC
		POP	AF
		SUB	40H
		CP	20H
		JR	C,J12BA			; graphic char, Reset Zx, Set Cx
		ADD	A,40H
J12B8:		CP	A			; Set Zx
		SCF				; Set Cx
J12BA:		POP	HL
		RET

J12BC:		POP	AF
		CP	01H
		JR	NZ,J12B8
		LD	(HL),A
		POP	HL			; Set Zx, Reset Cx
		RET

; Subroutine S.SCALXY
C12C4:		CALL	C062C			; new style screenmode ?
		JP	NC,J1305		; yep,
		PUSH	HL
		PUSH	BC
		LD	B,1 
		EX	DE,HL
		LD	A,H
		ADD	A,A
		JR	NC,J12D8
		LD	HL,0
		JR	J12E0

J12D8:		LD	DE,192
		RST	S_DCOMPR
		JR	C,J12E2
		EX	DE,HL
		DEC	HL
J12E0:		LD	B,00H
J12E2:		EX	(SP),HL
		LD	A,H
		ADD	A,A
		JR	NC,J12EC
		LD	HL,0
		JR	J12F4

J12EC:		LD	DE,256
		RST	S_DCOMPR
		JR	C,J12F6
		EX	DE,HL
		DEC	HL
J12F4:		LD	B,00H
J12F6:		POP	DE
		SRL	L
		SRL	L
		SRL	E
		SRL	E
		LD	A,B
		RRCA
		LD	C,L
		LD	B,H
		POP	HL
		RET

J1305:		PUSH	HL
		PUSH	BC
		LD	B,1
		EX	DE,HL
		LD	A,H
		ADD	A,A
		JR	NC,J1313
		LD	HL,0
		JR	J131B

J1313:		LD	DE,212
		RST	S_DCOMPR
		JR	C,J131D
		EX	DE,HL
		DEC	HL
J131B:		LD	B,00H
J131D:		EX	(SP),HL
		LD	A,H
		ADD	A,A
		JR	NC,J1327
		LD	HL,0
		JR	J1339

J1327:		LD	A,(SCRMOD)
		AND	02H
		LD	DE,512
		JR	NZ,J1334
		LD	DE,256
J1334:		RST	S_DCOMPR
		JR	C,J133B
		EX	DE,HL
		DEC	HL
J1339:		LD	B,00H
J133B:		LD	A,B
		RRCA
		LD	B,H
		LD	C,L
		POP	DE
		POP	HL
		RET

; Subroutine S.MAPXYC
C1342:		CALL	C062C			; new style screenmode ?
		JR	C,J134F			; nope,
		LD	H,B
		LD	L,C
		LD	A,E
		LD	(CMASK),A
		JR	J1372

J134F:		PUSH	BC
		LD	A,C
		RRCA
		LD	A,0F0H
		JR	NC,J1358
		LD	A,0FH
J1358:		LD	(CMASK),A
		LD	A,C
		ADD	A,A
		ADD	A,A
		AND	0F8H
		LD	C,A
		LD	A,E
		AND	07H
		OR	C
		LD	C,A
		LD	A,E
		RRCA
		RRCA
		RRCA
		AND	07H
		LD	B,A
		LD	HL,(MLTCGP)
		ADD	HL,BC
		POP	BC
J1372:		LD	(CLOC),HL
		RET

; Subroutine get coords
; Output: A=current y, HL=current x
C1376:		LD	A,(CMASK)
		LD	HL,(CLOC)
		RET

; Subroutine set coords
; Input:  A=current y, HL=current x
C137D:		LD	(CMASK),A
		LD	(CLOC),HL
		RET

; Subroutine S.READC
C1384:		CALL	C062C			; new style screenmode ?
		JP	NC,J13A1		; yep,
		PUSH	BC
		PUSH	HL
		CALL	C1376
		LD	B,A
		CALL	C08D3			; read VRAM limit 16 Kb (active page)
		INC	B
		DEC	B
		JP	P,J139C
		RRCA
		RRCA
		RRCA
		RRCA
J139C:		AND	0FH
		POP	HL
		POP	BC
		RET

J13A1:		PUSH	HL
		LD	HL,(CLOC)
		LD	A,(CMASK)
		CALL	C2949
		POP	HL
		RET

; Subroutine S.SETATR
C13AD:		CALL	C1E47
		RET	C
		LD	(ATRBYT),A
		RET

; Subroutine S.SETC
C13B5:		PUSH	BC
		PUSH	DE
		PUSH	HL
		CALL	C062C			; new style screenmode ?
		LD	A,(CMASK)
		LD	HL,(CLOC)
		JR	C,J13CA			; nope,
		CALL	C2961
		POP	HL
		POP	DE
		POP	BC
		RET

J13CA:		LD	B,A
		CALL	C08D6			; read VRAM (active page)
		LD	C,A
		LD	A,B
		CPL
		AND	C
		LD	C,A
		LD	A,(ATRBYT)
		INC	B
		DEC	B
		JP	P,J13DF
		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
J13DF:		OR	C
		CALL	C08CB			; write VRAM (active page)
		POP	HL
		POP	DE
		POP	BC
		RET

; Subroutine S.TLEFTC
C13E7:		PUSH	HL
		CALL	C062C			; new style screenmode ?
		CALL	C1376
		JP	C,J1466			; nope,
		DEC	HL
		LD	A,H
		OR	A
		JP	M,J1471
		LD	(CLOC),HL
		POP	HL
		RET

; Subroutine S.TDOWNC
C13FC:		PUSH	HL
		CALL	C062C			; new style screenmode ?
		CALL	C1376
		JP	C,J1474			; nope,
		LD	A,(CMASK)
		CP	212-1
		JR	NC,J1471
		INC	A
		LD	(CMASK),A
		POP	HL
		AND	A
		RET

; Subroutine S.TUPC
C1414:		CALL	C062C			; new style screenmode ?
		JP	C,J14AE			; nope,
		LD	A,(CMASK)
		OR	A
		SCF
		RET	Z
		DEC	A
		LD	(CMASK),A
		OR	A
		RET

; Subroutine S.TRIGHT
C1426:		PUSH	HL
		CALL	C1376
		AND	A
		LD	A,0FH
		JP	M,J1460
		LD	A,L
		AND	0F8H
		CP	0F8H
		JR	NZ,J1443
		JR	J1471

; Subroutine S.RIGHTC
J1439:		PUSH	HL
		CALL	C1376
		AND	A
		LD	A,0FH
		JP	M,J1460
J1443:		PUSH	DE
		LD	DE,8
		LD	A,0F0H
		JR	J145B

; Subroutine S.LEFTC
J144B:		PUSH	HL
		CALL	C1376
		AND	A
		LD	A,0F0H
		JP	P,J1460
J1455:		PUSH	DE
		LD	DE,-8
		LD	A,0FH
J145B:		ADD	HL,DE
		LD	(CLOC),HL
		POP	DE
J1460:		LD	(CMASK),A
		AND	A
		POP	HL
		RET

J1466:		AND	A
		LD	A,0F0H
		JP	P,J1460
		LD	A,L
		AND	0F8H
		JR	NZ,J1455
J1471:		SCF
		POP	HL
		RET

J1474:		PUSH	DE
		PUSH	HL
		LD	HL,(MLTCGP)
		LD	DE,0500H
		ADD	HL,DE
		EX	DE,HL
		POP	HL
		RST	S_DCOMPR
		JR	C,J148E
		LD	A,L
		INC	A
		AND	07H
		JR	NZ,J148E
		SCF
		POP	DE
		POP	HL
		RET

; Subroutine S.DOWNC
J148C:		PUSH	HL
		PUSH	DE
J148E:		CALL	C1376
		INC	HL
		LD	A,L
		LD	DE,00F8H
		JR	J14A2

; Subroutine S.UPC
J1498:		PUSH	HL
		PUSH	DE
J149A:		CALL	C1376
		LD	A,L
		DEC	HL
		LD	DE,0FF08H
J14A2:		AND	07H
		JR	NZ,J14A7
		ADD	HL,DE
J14A7:		LD	(CLOC),HL
		AND	A
		POP	DE
		POP	HL
		RET

J14AE:		PUSH	HL
		PUSH	DE
		LD	HL,(MLTCGP)
		LD	DE,256
		ADD	HL,DE
		EX	DE,HL
		LD	HL,(CLOC)
		RST	S_DCOMPR
		JR	NC,J149A
		LD	A,L
		AND	07H
		JR	NZ,J149A
		SCF
		POP	DE
		POP	HL
		RET

C14C7:		AND	A
		PUSH	AF
		LD	A,(SCRMOD)
		CP	8			; screenmode 8 ?
		JR	Z,J14D8			; yep, use all 256 colors
		POP	AF
		CP	16
		CCF
J14D4:		LD	(BDRATR),A
		RET

J14D8:		POP	AF
		JR	J14D4

; Subroutine S.SCANR
C14DB:		LD	HL,0
		LD	C,L
		CALL	C062C			; new style screenmode ?
		JP	C,J1566			; nope,
		PUSH	HL
		PUSH	BC
		PUSH	DE
		LD	HL,(CLOC)
		LD	A,(CMASK)
		LD	E,A
		LD	D,00H
		LD	A,(BDRATR)
		CALL	C28E0
		JR	C,J14FF
		POP	DE
J14FA:		POP	BC
		POP	HL
		LD	D,H
		LD	E,H
		RET

J14FF:		POP	DE
		PUSH	HL
		LD	HL,(CLOC)
		ADD	HL,DE
		POP	DE
		OR	A
		SBC	HL,DE
		JR	C,J14FA
		JR	Z,J14FA
		POP	BC
		POP	BC
		PUSH	HL
		EX	DE,HL
		LD	(CSAVEA),HL
		LD	A,(CMASK)
		LD	(CSAVEM),A
		LD	E,A
		LD	D,00H
		LD	A,(BDRATR)
		CALL	C28E4
		PUSH	AF
		JR	C,J1533
		LD	A,(SCRMOD)
		AND	02H
		LD	HL,01FFH
		JR	NZ,J1533
		LD	HL,00FFH
J1533:		LD	(CLOC),HL
		POP	AF
		JR	C,J153A
		INC	HL
J153A:		PUSH	HL
		EX	DE,HL
		LD	HL,(CSAVEA)
		EX	DE,HL
		OR	A
		SBC	HL,DE
		EX	(SP),HL
		PUSH	HL
		EX	DE,HL
		LD	A,(CSAVEM)
		LD	E,A
		LD	D,00H
		LD	A,(ATRBYT)
		CALL	C28E0
		POP	DE
		JR	NC,J1561
		RST	S_DCOMPR
		JR	NC,J1561
		POP	DE
		LD	HL,(CSAVEA)
		CALL	C15DB
		POP	DE
		RET

J1561:		LD	C,00H
		POP	HL
		POP	DE
		RET

J1566:		CALL	C1633
		JR	NC,J1578
		DEC	DE
		LD	A,D
		OR	E
		RET	Z
		CALL	C1426
		JR	NC,J1566
		LD	DE,0
		RET

J1578:		CALL	C1376
		LD	(CSAVEA),HL
		LD	(CSAVEM),A
		LD	HL,0
J1584:		INC	HL
		CALL	C1426
		RET	C
		CALL	C1633
		JR	NC,J1584
		RET

; Subroutine S.SCANL
C158F:		LD	HL,0
		LD	C,L
		CALL	C062C			; new style screenmode ?
		JP	C,J1626			; nope,
		CALL	C13E7
		RET	C
		LD	HL,(CLOC)
		PUSH	HL
		LD	A,(CMASK)
		LD	E,A
		LD	D,00H
		LD	A,(BDRATR)
		CALL	C28D8
		PUSH	AF
		JR	C,J15B3
		LD	HL,-1
J15B3:		INC	HL
		LD	(CLOC),HL
		EX	DE,HL
		POP	HL
		EX	(SP),HL
		PUSH	HL
		OR	A
		SBC	HL,DE
		INC	HL
		EX	(SP),HL
		PUSH	DE
		LD	A,(CMASK)
		LD	E,A
		LD	D,00H
		LD	A,(ATRBYT)
		CALL	C28DC
		JR	NC,J161B
		POP	DE
		POP	BC
		POP	AF
		PUSH	BC
		PUSH	DE
		JR	NC,J15D9
		RST	S_DCOMPR
		JR	C,J1621
J15D9:		POP	HL
		POP	DE

; Input:  HL = dx, DE = nx
C15DB:		LD	A,36
		DI
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer to 36, autoincrement
		LD	A,L
		OUT	(9BH),A
		LD	A,H
		OUT	(9BH),A
		LD	A,(CMASK)
		OUT	(9BH),A
		LD	A,(ACPAGE)
		OUT	(9BH),A
		LD	A,E
		OUT	(9BH),A
		LD	A,D
		OUT	(9BH),A
		LD	A,01H
		OUT	(9BH),A
		XOR	A
		OUT	(9BH),A
		LD	A,(ATRBYT)
		OUT	(9BH),A
		XOR	A
		OUT	(9BH),A
		LD	A,(SCRMOD)
		CP	8
		LD	A,80H
		JR	NZ,J1614
		LD	A,0C0H
J1614:		OUT	(9BH),A
		EI
		EX	DE,HL
		LD	C,0FFH
		RET

J161B:		POP	DE
		POP	HL
		POP	DE
		LD	C,0
		RET

J1621:		POP	DE
		POP	HL
		LD	C,0
		RET

J1626:		CALL	C13E7
		RET	C
		CALL	C1633
		JP	C,J1439
		INC	HL
		JR	J1626

C1633:		CALL	C1384
		LD	B,A
		LD	A,(BDRATR)
		SUB	B
		SCF
		RET	Z
		LD	A,(ATRBYT)
		CP	B
		RET	Z
		CALL	C13B5
		LD	C,1
		AND	A
		RET

; Subroutine BASIC CALL handler
C1649:		EI
		LD	DE,I1660
		PUSH	HL
		LD	HL,PROCNM
		CALL	C16AD
		POP	HL
		RET	C
		SCF
		RET	NZ
		CALL	C165E
		EI
		AND	A
		RET

C165E:		PUSH	DE
		RET

I1660:		DEFB	"MEMINI",0
		DEFW	C3702
		DEFB	"MKILL",0
		DEFW	C38EF
		DEFB	"MNAME",0
		DEFW	C3946
		DEFB	"MFILES",0
		DEFW	C381B
		DEFB	0FFH

; Subroutine S.PUTGET
J1683:		PUSH	AF
		PUSH	HL
		LD	DE,I1699
		CALL	C16AD
		JR	C,J1695
		POP	AF
		POP	AF
		DEC	HL
		CALL	C165E
		AND	A
		RET

J1695:		POP	HL
		POP	AF
		SCF
		RET

I1699:		DEFB	0CBH,0
		DEFW	C16DB
		DEFB	"DATE",0
		DEFW	C16E1
		DEFB	"KANJI",0
		DEFW	C2195
		DEFB	0FFH

C16AD:		PUSH	HL
		LD	A,(DE)
		INC	A
		JR	Z,J16C6
		CALL	C16C9
		JR	Z,J16BD
		INC	DE
		INC	DE
		INC	DE
		POP	HL
		JR	C16AD

J16BD:		LD	A,(HL)
		EX	(SP),HL
		EX	DE,HL
		INC	HL
		LD	E,(HL)
		INC	HL
		LD	D,(HL)
		AND	A
		DEFB	038H
J16C6:		SCF
		POP	HL
		RET

C16C9:		LD	A,(DE)
		OR	A
		RET	Z
		CP	(HL)
		JR	NZ,J16D3
		INC	DE
		INC	HL
		JR	C16C9

J16D3:		LD	A,(DE)
		AND	A
		INC	DE
		JR	NZ,J16D3
		DEC	DE
		DEC	A
		RET

; Subroutine GET TIME statement
C16DB:		DEC	B
		LD	BC,I174B
		JR	J16E5

; Subroutine GET DATE statement
C16E1:		DEC	B
		LD	BC,I1767
J16E5:		JP	P,J0546			; illegal function call error
		RST	S_CHRGTR
		PUSH	BC
		CALL	C05D8			; locate variable
		CALL	C05D2			; check for string
		DEC	HL
		RST	S_CHRGTR
		JR	Z,J16FE
		RST	S_SYNCHR
		DEFB	','
		RST	S_SYNCHR
		DEFB	"A"
		EX	(SP),HL
		LD	BC,14
		ADD	HL,BC
		EX	(SP),HL
J16FE:		POP	BC
		PUSH	HL
		PUSH	DE
		PUSH	BC
		LD	A,8
		CALL	C05EA			; allocate temporary string
		LD	HL,(DSCTMP+1)
		POP	DE
		PUSH	HL
		LD	A,(DE)
		RLCA
		AND	01H
		LD	B,A
		LD	C,04H
		CALL	C194B			; change RP-501 register 13
		LD	C,2
J1718:		LD	B,2
J171A:		LD	A,(DE)
		INC	A
		JR	Z,J1723
		DEC	A
		OUT	(0B4H),A
		IN	A,(0B5H)
J1723:		INC	DE
		EX	DE,HL
		ADD	A,(HL)
		EX	DE,HL
		DEC	DE
		DAA
		AND	0FH
		OR	'0'
		LD	(HL),A
		INC	DE
		INC	DE
		INC	HL
		DJNZ	J171A
		INC	C
		DEC	C
		JR	Z,J173E
		LD	A,(DE)
		LD	(HL),A
		INC	DE
		INC	HL
		DEC	C
		JR	J1718

J173E:
	IF CNTRY = 0
		CALL	C1947
	ELSE
		CALL	C3CA7
	ENDIF

		POP	DE
		POP	HL
		LD	(HL),8
		INC	HL
		LD	(HL),E
		INC	HL
		LD	(HL),D
		POP	HL
		RET

I174B:		DEFB	5,0
		DEFB	4,0
		DEFB	":"
		DEFB	3,0
		DEFB	2,0
		DEFB	":"
		DEFB	1,0
		DEFB	0,0

		DEFB	085H,0
		DEFB	084H,0
		DEFB	":"
		DEFB	083H,0
		DEFB	082H,0
		DEFB	":"
		DEFB	081H,0
		DEFB	080H,0

I1767:
	IF DATFMT = 0
		DEFB	12,8
		DEFB	11,0
		DEFB	"/"
		DEFB	10,0
		DEFB	9,0
		DEFB	"/"
		DEFB	8,0
		DEFB	7,0

		DEFB	0FFH,0
		DEFB	0FFH,0
		DEFB	"/"
		DEFB	0FFH,0
		DEFB	0FFH,0
		DEFB	"/"
		DEFB	088H,0
		DEFB	087H,0
	ENDIF
	IF DATFMT = 1
		DEFB	10,0
		DEFB	9,0
		DEFB	"/"
		DEFB	8,0
		DEFB	7,0
		DEFB	"/"
		DEFB	12,8
		DEFB	11,0

		DEFB	0FFH,0
		DEFB	0FFH,0
		DEFB	"/"
		DEFB	088H,0
		DEFB	087H,0
		DEFB	"/"
		DEFB	0FFH,0
		DEFB	0FFH,0
	ENDIF
	IF DATFMT = 2
		DEFB	8,0
		DEFB	7,0
		DEFB	"/"
		DEFB	10,0
		DEFB	9,0
		DEFB	"/"
		DEFB	12,8
		DEFB	11,0

		DEFB	088H,0
		DEFB	087H,0
		DEFB	"/"
		DEFB	0FFH,0
		DEFB	0FFH,0
		DEFB	"/"
		DEFB	0FFH,0
		DEFB	0FFH,0
	ENDIF

; Subroutine SET SCREEN statement
J1783:		RST	S_CHRGTR
		PUSH	HL			; store BASIC pointer
		CALL	C1CF7			; select RP-501 bank 2
		LD	B,3			; register = 3
		LD	A,(OLDSCR)
		LD	C,A
		LD	A,(RG9SAV)
		RRCA
		RRCA
		AND	02H
		OR	C
		CALL	C1A59			; write nibble to RP-501 (register 3)
		LD	A,(LINLEN)
		CALL	C1A50			; write byte to RP-501 (register 4 and 5)
		LD	A,(FORCLR)
		CALL	C1A59			; write nibble to RP-501 (register 6)
		LD	A,(BAKCLR)
		CALL	C1A59			; write nibble to RP-501 (register 7)
		LD	A,(BDRCLR)
		CALL	C1A59			; write nibble to RP-501 (register 8)
		LD	C,0
		LD	A,(CNSDFG)
		OR	A
		JR	Z,J17BB
		SET	0,C
J17BB:		LD	A,(CLIKSW)
		OR	A
		JR	Z,J17C3
		SET	1,C
J17C3:		LD	A,(NTMSXP)
		OR	A
		JR	Z,J17CB
		SET	2,C
J17CB:		LD	A,(CS1200)
		LD	HL,LOW_
		CP	(HL)
		JR	Z,J17D6
		SET	3,C
J17D6:		LD	A,C
		JP	J1A8C			; restore BASIC and write nibble to RP-501

; Subroutine SET ADJUST statement
C17DA:		RST	S_SYNCHR
		DEFB	"("
		CALL	C180D
		LD	A,E
		PUSH	AF
		RST	S_SYNCHR
		DEFB	','
		CALL	C180D
		RST	S_SYNCHR
		DEFB	")"
		POP	AF
		AND	0FH
		LD	D,A
		CALL	C17F8
		CALL	C1CF7			; select RP-501 bank 2
		LD	B,1
		LD	A,D
		JP	C1A50			; write byte to RP-501 (register 1 and 2)

C17F8:		LD	A,E
		RLCA
		RLCA
		RLCA
		RLCA
		AND	0F0H
		OR	D
		LD	D,A
		LD	(RG18SA),A
		DI
		OUT	(99H),A
		LD	A,18+128
		EI
		OUT	(99H),A
		RET

; Subroutine evaluate adjust parameter
C180D:		CALL	C0608			; evaluate integer operand
		PUSH	HL
		EX	DE,HL
		CALL	C25DC
		EX	DE,HL
		LD	HL,7
		RST	S_DCOMPR
		JR	NC,J1823
		LD	HL,-9
		RST	S_DCOMPR
		JP	NC,J0546		; illegal function call error
J1823:		POP	HL
		RET

; Subroutine get string parameter
; Output: A=size,DE=address of string
C1825:		CALL	C05FC			; evaluate expression
		PUSH	HL
		CALL	C0614			; free temporary string with type check
		LD	A,(HL)
		INC	HL
		LD	E,(HL)
		INC	HL
		LD	D,(HL)
		POP	HL
		RET

; Subroutine S.SETS (SET statement handler)
J1833:		CP	0C5H
		JP	Z,J1783
		CP	0CBH
		JP	Z,J188A
		CP	0C0H
		JP	Z,J19A6
		LD	DE,I184F
		CALL	C16AD
		JP	C,J0540			; syntax error
		DEC	HL
		RST	S_CHRGTR
		PUSH	DE
		RET

I184F:		DEFB	"PAGE",0
		DEFW	C1FB1
		DEFB	"VIDEO",0
		DEFW	C35AF
		DEFB	"ADJUST",0
		DEFW	C17DA
		DEFB	"TITLE",0
		DEFW	C1980
		DEFB	"PASSW",0F7H,"D",0
		DEFW	C19DD
		DEFB	"PROMPT",0
		DEFW	C19E1
		DEFB	"DATE",0
		DEFW	C188E
		DEFB	0FFH

; Subroutine SET TIME statement
J188A:		RST	S_CHRGTR
		LD	A,':'
		DEFB	1

; Subroutine SET DATE statement
C188E:		LD	A,'/'
		PUSH	AF
		CALL	C1825			; evaluate string expression
		POP	IX
		CP	8			; size of string exactly 8 ?
		JP	NZ,J0540		; nope, syntax error
		PUSH	HL
		EX	DE,HL
		CALL	C195E
	IF DATFMT = 0
		JR	Z,J18A6
		CP	24H
		JR	NC,J18C0
J18A6:		LD	B,A
		CALL	C1956
		JR	Z,J18B0
		CP	60H
		JR	J18B2

J18B0:		CP	13H
J18B2:		JR	NC,J18C0
		LD	C,A
		CALL	C1956
		JR	Z,J18BE
		CP	60H
		JR	J18C0
J18BE:		CP	32H
J18C0:		JP	NC,J0546		; illegal function call error
		LD	D,A
		POP	HL
		PUSH	IX
		DEC	HL
		RST	S_CHRGTR
		POP	IX
		LD	A,00H
		JR	Z,J18D9
		PUSH	IX
		RST	S_SYNCHR
		DEFB	','
		RST	S_SYNCHR
		DEFB	"A"
		POP	IX
		OR	01H 
	ENDIF
	IF DATFMT = 1
		LD	E,24H
		JR	NZ,J18A7
		LD	D,A
		LD	E,13H
J18A7:		CP	E
		JR	NC,J18C4
		LD	B,A
		CALL	C1956
		LD	E,32H
		JR	Z,J18B4
		LD	E,60H
J18B4:		CP	E
		JR	NC,J18C4
		LD	C,A
		CALL	C1956
		JR	NZ,J18C2
		LD	B,A
		LD	A,C
		LD	C,D
		JR	J18C7

J18C2:		CP	60H
J18C4:		JP	NC,J0546		; illegal function call error
J18C7:		LD	D,A
		POP	HL
		PUSH	IX
		DEC	HL
		RST	S_CHRGTR
		LD	A,00H
		JR	Z,J18D7
		RST	S_SYNCHR
		DEFB	','
		RST	S_SYNCHR
		DEFB	"A"
		OR	01H 
J18D7:		POP	IX
	ENDIF
	IF DATFMT = 2
		LD	E,24H
		JR	NZ,J18A7
		LD	D,A
		LD	E,32H
J18A7:		CP	E
		JR	NC,J18C3
		LD	B,A
		CALL	C1956
		LD	E,13H
		JR	Z,J18B4
		LD	E,60H
J18B4:		CP	E
		JR	NC,J18C3
		LD	C,A
		CALL	C1956
		JR	NZ,J18C0
		LD	B,A
		JR	J18C6

J18C0:		LD	D,A
		CP	60H
J18C3:		JP	NC,J0546		; illegal function call error
J18C6:		POP	HL
		PUSH	IX
		DEC	HL
		RST	S_CHRGTR
		LD	A,00H
		JR	Z,J18D5
		RST	S_SYNCHR
		DEFB	','
		RST	S_SYNCHR
		DEFB	"A"
		OR	01H
J18D5:		POP	IX
		NOP
		NOP
	ENDIF
		
J18D9:		PUSH	HL
		PUSH	AF
		PUSH	BC
		PUSH	DE
		JR	NZ,J18EA
		LD	BC,0004H
		CALL	C194B			; change RP-501 register 13
		LD	BC,0E00H		; reset alarm registers,	leave fractions, 16 Hz pulse off, 1 Hz pulse off
		JR	J18F8

J18EA:		LD	BC,0108H
		CALL	C194B			; change RP-501 register 13
		CALL	C196C
		JR	Z,J18FD
		LD	BC,0D00H		; reset alarm registers,	leave fractions, 16 Hz pulse off, 1 Hz pulse off
J18F8:		LD	A,15
		CALL	C194D			; change RP-501 register 15
J18FD:		POP	DE
		LD	B,0
		CALL	C196C
		JR	NZ,J1907
		LD	B,7
J1907:		LD	A,D
		CALL	C1A50			; write byte to RP-501
		POP	DE
		POP	AF
		PUSH	AF
		JR	Z,J1915
		CALL	C196C
		JR	Z,J1941
J1915:		LD	A,E
		CALL	C1A50			; write byte to RP-501
		LD	A,D
		CALL	C196C
		JR	NZ,J193E
		PUSH	AF
		PUSH	BC
		BIT	4,A
		JR	Z,J1927
		ADD	A,2
J1927:		AND	03H
		PUSH	AF
		LD	A,1
		CALL	C1CF9			; select RP-501 bank 1
		POP	AF
		LD	B,11
		CALL	C1A59			; write nibble to RP-501 (register 11)
		XOR	A
		CALL	C1CF9			; select RP-501 bank 0
		POP	BC
		POP	AF
		SUB	80H
		DAA
J193E:		CALL	C1A50			; write byte to RP-501
J1941:		POP	AF
		POP	HL
		LD	B,04H
		JR	NZ,J1949

; Subroutine put clockchip in running state
C1947:		LD	B,08H
J1949:		LD	C,0FH

; Subroutine update clockchip setup register
C194B:		LD	A,13

; Subroutine update clockchip register
; Input:  C=AND mask, B=OR mask
C194D:		OUT	(0B4H),A
		IN	A,(0B5H)
		AND	C
		OR	B
		OUT	(0B5H),A
		RET

; Subroutine check for seperator and get digitpair
; Output: A=digitpair (BCD)
C1956:		PUSH	IX
		POP	AF
		CP	(HL)
		INC	HL
		JP	NZ,J0540		; syntax error

; Subroutine get digitpair
C195E:		CALL	C1974
		RLCA
		RLCA
		RLCA
		RLCA
		AND	0F0H
		LD	E,A
		CALL	C1974
		OR	E

; Subroutine check if date
C196C:		PUSH	BC
		PUSH	IX
		POP	BC
		BIT	4,B
		POP	BC
		RET

; Subroutine get digit
C1974:		LD	A,(HL)
		INC	HL
		SUB	'0'
		JR	C,J197D
		CP	9+1
		RET	C
J197D:		JP	J0546			; illegal function call error

; Subroutine SET TITLE statement
C1980:		CP	','			; title argument specified ?
		JR	Z,J198B			; nope,
		XOR	A			; select title
		CALL	C19EC			; store title in clockchip
		DEC	HL
		RST	S_CHRGTR
		RET	Z
J198B:		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
		RET	NZ
		DEC	A
		CP	4			; value 1-4 ?
		JR	NC,J197D		; nope, illegal function call error
		PUSH	AF
		CALL	C1CF7			; select RP-501 bank 2
		LD	B,11
		CALL	C1CD2			; read RP-501 register 11
		AND	0CH
		LD	C,A			; leave b3,b2
		POP	AF
		OR	C			; color in b1,b0
		DEC	B
		JP	C1A59			; write nibble to RP-501 (register 11)

; Subroutine SET BEEP statement
J19A6:		CALL	C1CF7			; select RP-501 bank 2
		LD	B,10
		CALL	C1CD2			; read RP-501 register 10
		PUSH	AF
		RST	S_CHRGTR
		CP	','
		JR	Z,J19C8
		RST	S_GETBYT		; evaluate byte operand
		DEC	A
		CP	4
		JR	NC,J19CE
		RLCA
		RLCA
		AND	0CH
		LD	B,A
		POP	AF
		AND	03H
		OR	B
		PUSH	AF
		DEC	HL
		RST	S_CHRGTR
		JR	Z,J19D7
J19C8:		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
		DEC	A
		CP	4
J19CE:		JP	NC,J0546		; illegal function call error
		LD	B,A
		POP	AF
		AND	0CH
		OR	B
		PUSH	AF
J19D7:		LD	B,10
		POP	AF
		JP	C1A59			; write nibble to RP-501 (register 10)

; Subroutine SET PASSWORD statement
C19DD:		LD	A,1			; select password
		JR	J19E3

; Subroutine SET PROMPT statement
C19E1:		LD	A,2			; select prompt
J19E3:		CALL	C19EC			; store title or prompt in clockchip
		DEC	HL
		RST	S_CHRGTR
		JP	NZ,J0540		; syntax error
		RET

; Subroutine get string parameter and write bank 3
; Input:  A = type (0=title,1=password,2=prompt)
C19EC:		PUSH	AF
		CALL	C1825			; evaluate string expression
		LD	C,A			; size of string
		CALL	C1CF3			; select RP-501 bank 3
		LD	B,12			; start with register 12
		XOR	A
J19F7:		CALL	C1A59			; write nibble to RP-501
		DEC	B
		DJNZ	J19F7			; clear registers 1-12
		POP	AF			; title/password/prompt
		CALL	C1A59			; write nibble to RP-501 (clear register 0)
		PUSH	HL
		EX	DE,HL
		DEC	A			; password ?
		JR	Z,J1A17			; yep, stored otherwise
		LD	E,6			; 6 characters maximum
J1A08:		DEC	C
		INC	C			; end of string ?
		JR	Z,J1A15			; yep, quit
		LD	A,(HL)
		INC	HL
		CALL	C1A50			; write byte to RP-501
		DEC	C
		DEC	E
		JR	NZ,J1A08		; next
J1A15:		POP	HL
		RET

J1A17:		LD	A,21H
		CALL	C1A50			; write byte to RP-501
		LD	A,03H
		CALL	C1A59			; write nibble to RP-501
		LD	A,C			; size of password string
		PUSH	AF
		CALL	C1CA5			; calculate checksum
		LD	B,4			; register 4
		LD	A,E
		CALL	C1A50			; write byte to RP-501 (register 4,5)
		LD	A,D
		CALL	C1A50			; write byte to RP-501 (register 6,7)
		POP	AF
		LD	C,A			; size of password string
		XOR	A
		OUT	(7FH),A
		IN	A,(7FH)
		CP	0AAH			; keycard reader found ?
		JR	NZ,J1A15		; nope, quit
		LD	A,C
		OR	A			; empty password string ?
		LD	A,1
		JR	Z,J1A42			; yep, use indicator 1
		INC	A			; nope, use indicator 2
J1A42:		CALL	C1A59			; write nibble to RP-501 (register 8)
		PUSH	BC
		CALL	C1C97			; read keycard
		POP	BC
		LD	A,E
		CALL	C1A50			; write byte to RP-501 (register 9,10)
		LD	A,D
		POP	HL
						; write byte to RP-501 (register 11,12)

; Subroutine write byte to clockchip
; Input:  A=data,B=pos
; Output: B=pos+2
C1A50:		PUSH	AF
		CALL	C1A59			; write nibble to RP-501
		POP	AF
		RRCA
		RRCA
		RRCA
		RRCA
						; write nibble to RP-501

; Subroutine write nibble to clockchip
; Input:  A=data,B=pos
; Output: B=pos+1
C1A59:		PUSH	AF
		LD	A,B
		OUT	(0B4H),A
		INC	B
		POP	AF
		OUT	(0B5H),A
		RET

; Subroutine S.BEEP
J1A62:		PUSH	HL
		CALL	C1AD0
		PUSH	AF
		DEC	B
		JR	Z,J1A90
		DEC	B
		JR	Z,J1AAD
		DEC	B
		JR	Z,J1ABC

; timbre 0 is a simple beep
		LD	E,55H
		CALL	C1B11
		CALL	C1B1A
		CALL	C1B32
		LD	A,8
		CALL	C1B1E
		LD	BC,07D0H
		CALL	C1B28
J1A86:		CALL	C056B
		POP	AF
		LD	B,13			; register 13
J1A8C:		POP	HL
		JP	C1A59			; write nibble to RP-501

; timbre 1 is a polyphonic beep
J1A90:		LD	HL,I1B3E
		LD	E,0B8H
		CALL	C1B1C
		LD	E,0A7H
		LD	A,02H
		CALL	C1B12
		LD	E,0A8H
		LD	A,04H
		CALL	C1B12
		LD	E,0A6H
J1AA8:		CALL	C1AEA
		JR	J1A86

; timbre 2 is a 2 tones beep
J1AAD:		LD	HL,I1B3E
		CALL	C1B1A
		LD	E,2AH
		CALL	C1AEA
		LD	E,35H
		JR	J1AA8

; timbre 3 is a 3 tones beep
J1ABC:		LD	HL,I1B46
		CALL	C1B1A
		LD	E,6BH
		CALL	C1AEA
		LD	E,47H
		CALL	C1AEA
		LD	E,55H
		JR	J1AA8

C1AD0:		LD	B,13
		CALL	C1CD2			; read RP-501 register 13
		PUSH	AF
		CALL	C1CF7			; select RP-501 bank 2
		LD	B,10
		CALL	C1CD2			; read RP-501 register 10
		LD	C,A
		RRCA
		RRCA
		AND	03H
		LD	B,A
		LD	A,C
		AND	03H
		LD	C,A
		POP	AF
		RET

C1AEA:		CALL	C1B11
		PUSH	BC
		PUSH	HL
		CALL	C1B32
		POP	HL
		PUSH	HL
		ADD	HL,BC
		ADD	HL,BC
		LD	C,(HL)
		INC	HL
		LD	B,(HL)
J1AF9:		LD	A,8
		CALL	C1B1E
		LD	A,9
		CALL	C1B1E
		LD	A,10
		CALL	C1B1E
		CALL	C1B28
		DEC	E
		JR	NZ,J1AF9
		POP	HL
		POP	BC
		RET

; Subroutine setup channel A frequency
C1B11:		XOR	A

; Subroutine setup channel frequency
; Input:  A=0,2 or 4
C1B12:		CALL	C1B1E
		LD	E,00H
		INC	A
		JR	C1B1E

; Subroutine enable tone generator on channel A
C1B1A:		LD	E,0BEH

; Subroutine write psg configregister
C1B1C:		LD	A,7

; Subroutine write psg register
; Input:  A = register, E = data
C1B1E:		DI
		OUT	(0A0H),A
		PUSH	AF
		LD	A,E
		EI
		OUT	(0A1H),A
		POP	AF
		RET

; Subroutine wait for ms
; Input:  BC = number
C1B28:		PUSH	BC
J1B29:		DEC	BC
		EX	(SP),HL
		EX	(SP),HL
		LD	A,B
		OR	C
		JR	NZ,J1B29
		POP	BC
		RET

; Subroutine get amplitude value
; Input:  C=volume id (0-3)
; Output: E=amplitude value
C1B32:		LD	B,0
		LD	HL,I1B3A
		ADD	HL,BC
		LD	E,(HL)
		RET

; amplitude table
I1B3A:		DEFB	04H,07H,0AH,0FH

; timbre 2 table
I1B3E:		DEFW	010D8H
		DEFW	009A0H
		DEFW	006BDH
		DEFW	0047EH

; timbre 3 table
I1B46:		DEFW	0059DH
		DEFW	00335H
		DEFW	0023FH
		DEFW	0017FH

; Subroutine S.PROMPT
J1B4E:		CALL	C05AE			; fresh line to interpreter output
		CALL	C1CF3			; select RP-501 bank 3
		CALL	C1CD0			; read RP-501 register 0
		CP	2
		LD	HL,REDDY
		JP	NZ,C05F6		; message to interpreter output
J1B5F:		CALL	C1CDB			; read byte from RP-501 registers
		JR	Z,J1B6A
		RST	S_OUTDO
		LD	A,B
		CP	13
		JR	C,J1B5F
J1B6A:		JP	J05A8			; newline to interpreter output

J1B6D:		CALL	C1C73			; read VRAM address 01FFFFH
		LD	C,A
		CPL
		CALL	C1C74			; write VRAM address 01FFFFH
		CALL	C1C73			; read VRAM address 01FFFFH
		CPL
		CP	C			; address has VRAM ?
		LD	A,C
		CALL	C1C74			; write VRAM address 01FFFFH
		LD	HL,I1C64
		LD	C,04H
		JR	Z,J1B8A			; yep, system has 128 KB VRAM (4 pages for screenmode 5 and 6)
		LD	HL,I1C68
		LD	C,02H			; nope, system has 64 KB VRAM (2 pages for screenmode 5 and 6)
J1B8A:		PUSH	HL
		LD	A,(MODE)
		AND	0F9H
		OR	C			; VRAM size in b2,b1
		LD	(MODE),A
		LD	HL,00090H
		LD	A,0B4H
		CALL	C1D2B
		LD	HL,I1C5E
		CALL	C1C4B			; print vram message on graphic screen
		POP	HL
		CALL	C1C4B			; print vram size message on graphic screen
		LD	HL,I1C6C
		CALL	C1C4B			; print kb message on graphic screen
		XOR	A
		OUT	(99H),A
		OUT	(99H),A			; VRAM address pointer=0
		OUT	(99H),A
		LD	A,14+128
		OUT	(99H),A			; vdp register 14=0
		CALL	C1CF3			; select RP-501 bank 3
		CALL	C1CD0			; read RP-501 register 0
		JR	Z,J1C35
		DEC	A
		RET	NZ
		CALL	C1CD2			; read RP-501 register 1
		CP	01H
		RET	NZ
		CALL	C1CD2			; read RP-501 register 2
		CP	02H
		RET	NZ
		CALL	C1CD2			; read RP-501 register 3
		CP	03H
		RET	NZ
		LD	B,8
		CALL	C1CD2			; read RP-501 register 8
		JR	Z,J1BEF
		XOR	A
		OUT	(7FH),A
		IN	A,(7FH)
		CP	0AAH
		JR	NZ,J1BEF
		PUSH	BC
		CALL	C1C97			; read keycard
		POP	BC
		CALL	C1CEA			; read word from RP-501 registers 9-12
		RST	S_DCOMPR
		RET	Z
		JR	J1C00

J1BEF:		LD	A,6
		CALL	C0590			; read keyboard row 6
		AND	04H
		JR	NZ,J1C00
		LD	A,7
		CALL	C0590			; read keyboard row 7
		AND	10H
		RET	Z
J1C00:		LD	HL,0032H
		LD	A,0D0H
		CALL	C1D2B
		LD	HL,I1C54
		CALL	C1C4B			; print message on graphic screen
		LD	B,8
		CALL	C1CD2			; read RP-501 register 8
		DEC	A
J1C14:		JR	Z,J1C14
J1C16:		LD	HL,BUF			; use BUF for inputline
		LD	B,255
J1C1B:		CALL	C0565			; get keyboard input
		CP	0DH
		JR	Z,J1C26
		LD	(HL),A
		INC	HL
		DJNZ	J1C1B
J1C26:		LD	A,255
		SUB	B			; size of line
		CALL	C1CA2			; calculate checksum of BUF
		LD	B,4
		CALL	C1CEA			; read word from RP-501 registers 4-7
		RST	S_DCOMPR
		RET	Z
		JR	J1C16

J1C35:		LD	HL,00C8H
		LD	A,L
		CALL	C1D2B
J1C3C:		CALL	C1CDB			; read byte from RP-501 registers
		RET	Z
		CALL	C1224			; print char on graphic screen
		LD	A,B
		CP	0DH
		JR	C,J1C3C
		JP	C0565			; get keyboard input


; Subroutine print message on graphic screen
C1C4B:		LD	A,(HL)
		INC	HL
		OR	A
		RET	Z
		CALL	C1224			; print char on graphic screen
		JR	C1C4B

I1C54:		DEFB	"Password:"
		DEFB	0

I1C5E:		DEFB	"VRAM:"
		DEFB	0

I1C64:		DEFB	"128"
		DEFB	0

I1C68:		DEFB	" 64"
		DEFB	0

I1C6C:		DEFB	"Kbytes"
		DEFB	0

C1C73:		DEFB	0F6H			; OR xx, skip next instruction

C1C74:		SCF
		PUSH	AF
		LD	A,07H
		OUT	(99H),A
		LD	A,14+128
		OUT	(99H),A
		LD	A,0FFH
		OUT	(99H),A
		POP	AF
		PUSH	AF
		LD	A,7FH
		JR	C,J1C8A
		LD	A,3FH
J1C8A:		OUT	(99H),A
		EX	(SP),HL
		EX	(SP),HL
		POP	AF
		JR	C,J1C94
		IN	A,(98H)
		RET

J1C94:		OUT	(98H),A
		RET

C1C97:		IN	A,(7FH)
		LD	L,A
		IN	A,(7FH)
		LD	H,A
		LD	(BUF+0),HL
		LD	A,2			; size of "line" = 2

C1CA2:		LD	HL,BUF

; Subroutine calculate checksum
C1CA5:		LD	DE,0
		LD	C,A
		OR	A
		RET	Z
		SCF
J1CAC:		PUSH	AF
J1CAD:		LD	B,8
J1CAF:		POP	AF
		PUSH	AF
		JR	NC,J1CB5
		RLC	(HL)
J1CB5:		RL	E
		RL	D
		JR	NC,J1CC3
		LD	A,D
		XOR	80H
		LD	D,A
		LD	A,E
		XOR	05H
		LD	E,A
J1CC3:		DJNZ	J1CAF
		INC	HL
		DEC	C
		JR	NZ,J1CAD
		POP	AF
		RET	NC
		OR	A
		LD	C,02H
		JR	J1CAC

; Subroutine read nibble from clockchip at pos 0
C1CD0:		LD	B,0

; Subroutine read nibble from clockchip
; Input:  B = pos nr
; Output: A = nibble, B = pos nr (updated)
C1CD2:		LD	A,B
		OUT	(0B4H),A
		INC	B
		IN	A,(0B5H)
		AND	0FH
		RET

; Subroutine read byte from clockchip
; Input:  B = pos nr
; Output: A = byte, B = pos nr (updated)
C1CDB:		PUSH	DE
		CALL	C1CD2			; read RP-501 register
		LD	D,A
		CALL	C1CD2			; read RP-501 register
		RLCA
		RLCA
		RLCA
		RLCA
		OR	D
		POP	DE
		RET

; Subroutine read word from clockchip
; Input:  B = pos nr
; Output: HL = word, B = pos nr (updated)
C1CEA:		CALL	C1CDB			; read byte from RP-501 registers
		LD	L,A
		CALL	C1CDB			; read byte from RP-501 registers
		LD	H,A
		RET

; Subroutine select bank 3 clockchip
C1CF3:		LD	A,3
		JR	C1CF9			; select RP-501 bank 3

; Subroutine select bank 2 clockchip
C1CF7:		LD	A,2

; Subroutine select bank clockchip
; Input:  A = bank nr
C1CF9:		PUSH	BC
		PUSH	AF
		LD	B,13
		CALL	C1CD2			; read RP-501 register 13
		AND	0CH
		POP	BC
		OR	B
		OUT	(0B5H),A
		POP	BC
		RET

; Subroutine S.REDCLK
; Input:  C = pos nr (b3-b0), bank nr (b5-b4)
C1D08:		CALL	C1D1A
		IN	A,(0B5H)
		AND	0FH
		RET

; Subroutine S.WRTCLK
; Input:  C = pos nr (b3-b0), bank nr (b5-b4)
C1D10:		PUSH	AF
		CALL	C1D1A
		POP	AF
		AND	0FH
		OUT	(0B5H),A
		RET

; Subroutine select bank and pos clockchip
; Input:  C = pos nr (b3-b0), bank nr (b5-b4)
C1D1A:		LD	A,C
		PUSH	AF
		AND	30H
		RRCA
		RRCA
		RRCA
		RRCA
		CALL	C1CF9			; select RP-501 bank
		POP	AF
		AND	0FH
		OUT	(0B4H),A
		RET

C1D2B:		LD	(CLOC),HL
		LD	(CMASK),A
		RET

; Subroutine S.SYNCHR
; Input:  HL = BASIC pointer
; Output: Zx set=end of statement
J1D32:		LD	A,(HL)
		EX	(SP),HL
		CP	(HL)
		JP	NZ,J0540		; syntax error
		INC	HL
		EX	(SP),HL

; Subroutine S.CHRGTR
J1D3A:		CALL	H_CHRG
		INC	HL
J1D3E:		LD	A,(HL)
		CP	':'
		RET	NC
		CP	20H
		JR	Z,J1D3A
		JR	NC,J1DB1
		OR	A
		RET	Z
		CP	0BH
		JR	C,J1DAC
		CP	1EH
		JR	NZ,J1D57
		LD	A,(CONSAV)
		OR	A
		RET

J1D57:		CP	10H
		JR	Z,J1D8F
		PUSH	AF
		INC	HL
		LD	(CONSAV),A
		SUB	1CH
		JR	NC,J1D94
		SUB	0F5H
		JR	NC,J1D6E
		CP	0FEH
		JR	NZ,J1D82
		LD	A,(HL)
		INC	HL
J1D6E:		LD	(CONTXT),HL
		LD	H,0
J1D73:		LD	L,A
		LD	(CONLO),HL
		LD	A,2
		LD	(CONTYP),A
		LD	HL,NUMCON
		POP	AF
		OR	A
		RET

J1D82:		LD	A,(HL)
		INC	HL
		INC	HL
		LD	(CONTXT),HL
		DEC	HL
		LD	H,(HL)
		JR	J1D73

; Subroutine get numeric constant (in DAC)
; Remark: UNUSED CODE
;         Was accidently copied with CHRGTR code from mainrom
Q1D8C:		CALL	C1DB7

J1D8F:		LD	HL,(CONTXT)
		JR	J1D3E

J1D94:		INC	A
		RLCA
		LD	(CONTYP),A
		PUSH	DE
		PUSH	BC
		LD	DE,CONLO
		CALL	C1DF0
		POP	BC
		POP	DE
		LD	(CONTXT),HL
		POP	AF
		LD	HL,NUMCON
		OR	A
		RET

J1DAC:		CP	9
		JP	NC,J1D3A
J1DB1:		CP	'0'
		CCF
		INC	A
		DEC	A
		RET

; Subroutine get numeric constant (in DAC)
; Remark: NOT USED ANYWHERE
;         Was accidently copied with CHRGTR code from mainrom
C1DB7:		LD	A,(CONSAV)
		CP	0FH
		JR	NC,J1DD1
		CP	0DH
		JR	C,J1DD1
		LD	HL,(CONLO)
		JR	NZ,J1DCE
		INC	HL
		INC	HL
		INC	HL
		LD	E,(HL)
		INC	HL
		LD	D,(HL)
		EX	DE,HL
J1DCE:		JP	C05DE			; convert to SGN and put in DAC

J1DD1:		LD	A,(CONTYP)
		LD	(VALTYP),A
		CP	2
		JR	NZ,J1DE1
		LD	HL,(CONLO)
		LD	(DAC+2),HL
J1DE1:		LD	HL,CONLO
		LD	DE,DAC
		LD	A,(VALTYP)
		CP	4
		JR	NC,C1DF0
		INC	DE
		INC	DE

; Subroutine copy
; Input:  HL=source,DE=destination,A=size
C1DF0:		LD	C,A
		LD	B,0
		LDIR
		RET

; Subroutine S.COLOR
J1DF6:		JP	Z,J1ED5
		CP	0EFH
		JP	Z,J1ECA
		CP	0C7H
I1E00:		JP	Z,J2040
		LD	DE,(FORCLR)
		PUSH	DE
		CP	','
		JR	Z,J1E17
		RST	S_GETBYT		; evaluate byte operand
		CALL	C1E40
		POP	DE
		LD	E,A
		PUSH	DE
		DEC	HL
		RST	S_CHRGTR
		JR	Z,J1E33
J1E17:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J1E28
		RST	S_GETBYT		; evaluate byte operand
		CALL	C1E40
		POP	DE
		LD	D,A
		PUSH	DE
		DEC	HL
		RST	S_CHRGTR
		JR	Z,J1E33
J1E28:		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
		CALL	C1E40
		LD	(BDRCLR),A
		DEC	HL
		RST	S_CHRGTR
J1E33:		EX	(SP),HL
		LD	(FORCLR),HL
		LD	A,L
		LD	(ATRBYT),A
		CALL	C0953			; change colors
		POP	HL
		RET

; Subroutine check colorattribute
C1E40:		CALL	C1E47
		RET	NC
		JP	J0546			; illegal function call error

; Subroutine validate colorattribute
C1E47:		PUSH	AF
		LD	A,(SCRMOD)
		CP	6
		JR	Z,J1E5B
		CP	8
		JR	Z,J1E58
		POP	AF
		CP	15+1
		CCF
		RET

J1E58:		POP	AF
		AND	A
		RET

J1E5B:		POP	AF
		CP	20H
		CCF
		RET	C
		CP	10H
		JR	C,J1E67
		AND	0FH
		RET

J1E67:		AND	03H
		PUSH	BC
		LD	B,A
		ADD	A,A
		ADD	A,A
		ADD	A,B
		POP	BC
		RET

; Subroutine COLOR=(a,b,c,d,e) statement
J1E70:		RST	S_CHRGTR
		LD	A,0FH
		CALL	C1EDC
		PUSH	AF
		CALL	C0F6A			; get pallette values of color (GETPLT)
		PUSH	BC
		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J1E98
		LD	A,7
		CALL	C1EDC
		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
		LD	E,A
		POP	BC
		LD	A,B
		AND	0FH
		OR	E
		LD	B,A
		PUSH	BC
		DEC	HL
		RST	S_CHRGTR
		CP	40+1
		JR	Z,J1EBA
J1E98:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J1EAC
		LD	A,7
		CALL	C1EDC
		POP	BC
		LD	C,A
		PUSH	BC
		DEC	HL
		RST	S_CHRGTR
		CP	40+1
		JR	Z,J1EBA
J1EAC:		RST	S_SYNCHR
		DEFB	','
		LD	A,7
		CALL	C1EDC
		POP	BC
		LD	A,B
		AND	0F0H
		OR	E
		LD	B,A
		PUSH	BC
J1EBA:		RST	S_SYNCHR
		DEFB	")"
		POP	BC
		LD	A,B
		LD	E,C
		POP	BC
		LD	D,B
		PUSH	AF
		CALL	C1034			; wait for VR to pass
		POP	AF
		CALL	C0FD3			; set pallette values of color (SETPLT)
		RET

; Subroutine COLOR= statement
J1ECA:		RST	S_CHRGTR
		CP	'('
		JR	Z,J1E70
		CP	8CH
		JR	Z,J1ED8
		RST	S_SYNCHR
		DEFB	094H

; Subroutine COLOR=NEW statement
J1ED5:		JP	J0F9F			; initialize pallette (INIPLT)

; Subroutine COLOR=RESTORE statement
J1ED8:		RST	S_CHRGTR
		JP	J0F7E			; restore pallette (RSTPLT)

; Subroutine evaluate operand
; Input:  E=max value
C1EDC:		PUSH	AF
		RST	S_GETBYT		; evaluate byte operand
		POP	AF
		CP	E
		LD	A,E
		RET	NC
		JP	J0546			; illegal function call error

J1EE5:		CP	','
		JR	Z,J1F1D
		RST	S_GETBYT		; evaluate byte operand
		CP	8+1
		JP	NC,J0546		; illegal function call error
		CP	5			; new style MSX2 screenmode ?
		JR	C,J1F0E			; nope, 16 Kb VRAM is always enough
		PUSH	AF
		LD	A,(MODE)
		RRCA
		AND	03H			; VRAM size (number of pages for screenmode 5 and 6)
		LD	C,A
		POP	AF
		CP	7			; screenmode 7 or 8 ?
		PUSH	AF
		LD	A,C
		JR	NC,J1F08		; yep, check for VRAM size 128 Kb
		CP	1			; VRAM size 16 Kb ?
		JR	C,J1F0A			; yep, illegal function call error
		JR	J1F0D

J1F08:		CP	2			; VRAM size 128 Kb ?
J1F0A:		JP	C,J0546			; nope, illegal function call error
J1F0D:		POP	AF
J1F0E:		PUSH	HL
		CALL	C09BF			; change screenmode, initialize pallette
		LD	A,(LINLEN)
		LD	E,A
		CALL	C2033
		POP	HL
		DEC	HL
		RST	S_CHRGTR
		RET	Z
J1F1D:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J1F3A
		RST	S_GETBYT		; evaluate byte operand
		CP	3+1
		JP	NC,J0546		; illegal function call error
		LD	A,(RG1SAV)
		AND	0FCH
		OR	E
		LD	(RG1SAV),A
		PUSH	HL
		CALL	C06F5			; clear sprites
		POP	HL
		DEC	HL
		RST	S_CHRGTR
		RET	Z
J1F3A:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J1F45
		RST	S_GETBYT		; evaluate byte operand
		LD	(CLIKSW),A
		RET	Z
J1F45:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J1F51
		CALL	C1F7A
		DEC	HL
		RST	S_CHRGTR
		RET	Z
J1F51:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J1F5C
		RST	S_GETBYT		; evaluate byte operand
		LD	(NTMSXP),A
		RET	Z
J1F5C:		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
		CP	3+1
		JP	NC,J0546		; illegal function call error
		ADD	A,A
		BIT	1,A
		JR	Z,J1F6B
		SET	3,A
J1F6B:		AND	0CH
		LD	B,A
		LD	A,(RG9SAV)
		AND	0F3H
		OR	B
		LD	C,9			; vdp register 9
J1F76:		LD	B,A
		JP	C0647			; write vdp register

C1F7A:		RST	S_GETBYT		; evaluate byte operand
		DEC	A
		CP	1+1
		JP	NC,J0546		; illegal function call error
		PUSH	HL
		LD	BC,5
		AND	A
		LD	HL,CS1200
		JR	Z,J1F8C
		ADD	HL,BC
J1F8C:		LD	DE,LOW_
		LDIR
		POP	HL
		RET

; Subroutine check screenpage is valid
C1F93:		PUSH	BC
		LD	B,A
		CALL	C062C			; new style screenmode ?
		JR	C,J1FAE			; nope,
		CP	7			; screenmode 7 or 8 ?
		LD	A,2-1
		JR	NC,J1FAD		; yep, 2 pages
		LD	A,(MODE)
		AND	06H
		CP	04H			; VRAM size 128 Kb ?
		LD	A,2-1
		JR	C,J1FAD			; nope, 2 pages (screenmode 5 or 6)
		LD	A,4-1			; yep, 4 pages (screenmode 5 or 6)
J1FAD:		CP	B
J1FAE:		LD	A,B
		POP	BC
		RET

; Subroutine SET PAGE statement
C1FB1:		LD	DE,(DPPAGE)
		CP	','
		JR	Z,J1FC7
		PUSH	DE
		RST	S_GETBYT		; evaluate byte operand
		POP	DE
		CALL	C1F93
J1FBF:		JP	C,J0546			; illegal function call error
		LD	E,A
		DEC	HL
		RST	S_CHRGTR
		JR	Z,J1FD2
J1FC7:		PUSH	DE
		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
		POP	DE
		CALL	C1F93
		JR	C,J1FBF
		LD	D,A
J1FD2:		LD	(DPPAGE),DE
		PUSH	HL
		CALL	C2980			; wait for vdp command to complete
		CALL	C06A8			; set active/display page
		POP	HL
		RET

; Subroutine S.WIDTHS
J1FDF:		JP	Z,J0546			; illegal function call error
		LD	A,(OLDSCR)
		AND	A
		LD	A,E
		JR	Z,J1FEE
		CP	32+1
		JP	NC,J0546		; illegal function call error
J1FEE:		CP	40+1
		JR	C,J1FF7
		CP	80+1
		JP	NC,J0546		; illegal function call error
J1FF7:		LD	A,(LINLEN)
		CP	E
		RET	Z
		LD	A,12
		RST	S_OUTDO			; clear screen

J1FFF:		LD	A,E
I2000:		LD	(LINLEN),A
		CALL	C2033
		LD	A,(OLDSCR)
		DEC	A
		LD	A,E
		JR	NZ,J2014
		LD	(LINL32),A
		LD	A,12
		RST	S_OUTDO			; clear screen
		RET

J2014:		LD	C,40+1
		LD	A,(LINL40)
		CP	C
		LD	A,E
		LD	(LINL40),A
		PUSH	AF
		LD	A,12
		RST	S_OUTDO			; clear screen
		POP	AF
		JR	C,J2028
		CP	C
		RET	NC
		LD	C,A
J2028:		CP	C
		RET	C
		PUSH	AF
		PUSH	HL
		XOR	A			; screenmode 0
		CALL	C09BF			; change screenmode, initialize pallette
		POP	HL
		POP	AF
		LD	E,A

; Subroutine initialize CLMLST
C2033:		SUB	14
		JR	NC,C2033
		ADD	A,28
		CPL
		INC	A
		ADD	A,E
		LD	(CLMLST),A
		RET

; Subroutine COLORSPRITE statement
J2040:		CALL	C0626			; MSX1 screenmode ?
		JP	C,J0546			; yep, illegal function call error
		RST	S_CHRGTR
		CP	'$'
		JR	Z,J2065
		LD	A,1FH
		CALL	C2318
		PUSH	HL
		CALL	C0775			; get spriteplane colortable
		EX	(SP),HL
		RST	S_SYNCHR
		DEFB	0EFH
		RST	S_GETBYT		; evaluate byte operand
		AND	A
		JP	M,J0546			; illegal function call error
		LD	BC,16
		EX	(SP),HL
		CALL	C0977			; fill VRAM (active page)
		POP	HL
		RET

J2065:		RST	S_SYNCHR
		DEFB	"$"
		LD	A,1FH
		CALL	C2318
		PUSH	HL
		CALL	C0775			; get spriteplane colortable
		EX	(SP),HL
		RST	S_SYNCHR
		DEFB	0EFH
		CALL	C05FC			; evaluate expression
		PUSH	HL
		CALL	C0614			; free temporary string with type check
		LD	A,(HL)
		CP	16+1
		JR	C,J2081
		LD	A,16
J2081:		INC	HL
		LD	E,(HL)
		INC	HL
		LD	D,(HL)
		POP	HL
		EX	(SP),HL
		AND	A
		LD	B,A
		EX	DE,HL
		CALL	NZ,C0F2B		; write string to VRAM
		POP	HL
		RET

; Subroutine S.PUTSPR
J208F:		RST	S_CHRGTR
		RST	S_GETBYT		; evaluate byte operand
		CP	32
		JP	NC,J0546		; illegal function call error
		LD	(BUF+2),A
		PUSH	HL
		PUSH	AF
		CALL	C0775			; get spriteplane colortable
		LD	(LINWRK+0),HL
		LD	DE,LINWRK+2
		LD	B,16
		CALL	C0F22			; read string from VRAM
		POP	AF
		PUSH	AF
		CALL	C076A			; get spriteplane address
		LD	(BUF+0),HL
		POP	AF
		LD	B,A
		LD	A,20H
		SUB	B
		LD	B,A
		LD	(BUF+3),A
		LD	DE,BUF+4
		PUSH	DE
		CALL	C0F22			; read string from VRAM
		POP	HL
		EX	(SP),HL
		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J20F5
		CALL	C059C			; evaluate complex graphic coordinatepair
		EX	(SP),HL
		LD	(HL),E
		INC	HL
		LD	A,B
		ADD	A,A
		LD	A,C
		LD	C,00H
		JR	NC,J20DA
		ADD	A,20H
		LD	C,80H
J20DA:		LD	(HL),A
		PUSH	HL
		LD	HL,LINWRK+2
I20DF:		LD	B,16
J20E1:		LD	A,(HL)
		AND	7FH
		OR	C
		LD	(HL),A
		INC	HL
		DJNZ	J20E1
		CALL	C217A
		POP	HL
		INC	HL
		EX	(SP),HL
		DEC	HL
		RST	S_CHRGTR
		POP	BC
		JR	Z,J212E
		PUSH	BC
J20F5:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J2119
		RST	S_GETBYT		; evaluate byte operand
		AND	A
		JP	M,J0546			; illegal function call error
		PUSH	HL
		LD	HL,LINWRK+2
		LD	C,A
		LD	B,16
J2107:		LD	A,(HL)
		AND	80H
		OR	C
		LD	(HL),A
		INC	HL
		DJNZ	J2107
		CALL	C217A
		POP	HL
		DEC	HL
		RST	S_CHRGTR
		POP	BC
		JR	Z,J212E
		PUSH	BC
J2119:		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
		CALL	C077F			; get spritesize
		LD	A,E
		JR	NC,J2129
		CP	64
		JP	NC,J0546		; illegal function call error
		ADD	A,A
		ADD	A,A
J2129:		EX	(SP),HL
		LD	(HL),A
		LD	B,H
		LD	C,L
		POP	HL
J212E:		PUSH	HL
		LD	A,(BUF+2)
		SUB	1FH
		JR	Z,J216A
		LD	HL,(LINWRK+0)
		LD	A,(BUF+3)
		LD	B,A
J213D:		LD	DE,16
		ADD	HL,DE
		PUSH	HL
		PUSH	BC
		LD	DE,LINWRK+2
		LD	B,16
		CALL	C0F22			; read string from VRAM
		CALL	C2187
		POP	BC
		POP	HL
		JR	Z,J2154
		DJNZ	J213D
J2154:		LD	A,(BUF+3)
		SUB	B
		JR	Z,J216A
		LD	B,A
		LD	HL,BUF+8
		LD	DE,(BUF+4)
J2162:		LD	(HL),E
		INC	HL
		LD	(HL),D
		INC	HL
		INC	HL
		INC	HL
		DJNZ	J2162
J216A:		INC	A
		ADD	A,A
		ADD	A,A
		LD	B,A
		LD	HL,BUF+4
		LD	DE,(BUF)
		CALL	C0F2B			; write string to VRAM
		POP	HL
		RET

C217A:		LD	HL,LINWRK+2
		LD	DE,(LINWRK+0)
		LD	B,16
		CALL	C0F2B			; write string to VRAM
		RET

; Subroutine check if CC bit of spritecolor is on
C2187:		LD	HL,LINWRK+2
		LD	B,16
J218C:		LD	A,(HL)
		AND	40H
		RET	NZ
		INC	HL
		DJNZ	J218C
		XOR	A
		RET

; Subroutine PUT KANJI statement
C2195:		DEC	B
		JP	M,J0540			; syntax error
		LD	A,(SCRMOD)
		CP	5
		JP	C,J0546			; illegal function call error
		RST	S_CHRGTR
		CP	','
		CALL	NZ,C059C		; evaluate complex graphic coordinatepair
		RST	S_SYNCHR
		DEFB	','
		CALL	C0608			; evaluate integer operand
		PUSH	DE
		CALL	C256F
		CALL	C2594
		DEC	HL
		RST	S_CHRGTR
		CP	','
		LD	A,00H
		JR	NZ,J21C2
		RST	S_CHRGTR
		RST	S_GETBYT		; evaluate byte operand
		CP	4-1
		JP	NC,J0546		; illegal function call error
J21C2:		POP	BC
		JP	J1049

; Subroutine S.VDP
J21C6:		LD	A,47
		CALL	C2318
		CP	8
		JR	C,J21DD
		JP	Z,J0546			; illegal function call error
		DEC	E
		DEC	A
		CP	18H
		JR	C,J21DD
		CP	20H
		JP	C,J0546			; illegal function call error
J21DD:		PUSH	DE
		RST	S_SYNCHR
		DEFB	0EFH
		RST	S_GETBYT		; evaluate byte operand
		POP	BC
		JP	J1F76

; Subroutine S.VDPF
J21E5:		RST	S_CHRGTR
		RST	S_SYNCHR
		DEFB	"("
		CALL	C0608			; evaluate integer operand
		RST	S_SYNCHR
		DEFB	")"
		PUSH	HL
		LD	A,D
		RLCA
		JR	C,J220E
		AND	A
		JR	NZ,J21F8
		LD	A,E
		CP	19H
J21F8:		JP	NC,J0546		; illegal function call error
		CP	09H
		JR	NC,J2204
		LD	HL,RG0SAV
		JR	J2207

J2204:		LD	HL,RG8SAV-8-1
J2207:		ADD	HL,DE
		LD	A,(HL)
J2209:		CALL	C05E4			; byte to DAC
		POP	HL
		RET

J220E:		EX	DE,HL
		CALL	C25DC
		EX	DE,HL
		LD	HL,9
		RST	S_DCOMPR
		JP	C,J0546			; illegal function call error
		LD	A,E
		CALL	C298B			; read vdp status register
		JR	J2209

; Subroutine S.BASE
J2220:		LD	A,13H
		CALL	C2318
		LD	D,00H
		PUSH	DE
		RST	S_SYNCHR
		DEFB	0EFH
		CALL	C0608			; evaluate integer operand
		CP	40H
		JP	NC,J0546		; illegal function call error
		EX	(SP),HL
		PUSH	HL
		LD	C,E
		LD	B,D
		POP	HL
		LD	A,L
		PUSH	AF
		ADD	HL,HL
		EX	DE,HL
		LD	HL,I226F
		ADD	HL,DE
		LD	A,C
		AND	(HL)
		JR	NZ,J2246
		INC	HL
		LD	A,B
		AND	(HL)
J2246:		JP	NZ,J0546		; illegal function call error
		LD	HL,TXTNAM
		ADD	HL,DE
		LD	(HL),C
		INC	HL
		LD	(HL),B
		POP	AF
		LD	E,-1
J2253:		INC	E
		SUB	5
		JR	NC,J2253
		LD	A,(SCRMOD)
		CP	E
		CALL	Z,C2261
		POP	HL
		RET

C2261:		DEC	A
		JP	M,C0B42			; SETTXT
		JP	Z,C0B9C			; SETT32
		DEC	A
		JP	Z,C0BD2			; SETGRP
		JP	C0C22			; SETMLT

I226F:		DEFW	003FFH
		DEFW	0003FH
		DEFW	007FFH
		DEFW	0007FH
		DEFW	007FFH
		DEFW	003FFH
		DEFW	0003FH
		DEFW	007FFH
		DEFW	0007FH
		DEFW	007FFH
		DEFW	003FFH
		DEFW	01FFFH
		DEFW	01FFFH
		DEFW	0007FH
		DEFW	007FFH
		DEFW	003FFH
		DEFW	0003FH
		DEFW	007FFH
		DEFW	0007FH
		DEFW	007FFH

; Subroutine S.BASEF
J2297:		RST	S_CHRGTR
		LD	A,44
		CALL	C2318
		CP	14H
		JR	NC,J22C2
		PUSH	HL
J22A2:		CP	02H
		JR	NZ,J22B2
		LD	A,(LINLEN)
		CP	40+1
		JR	C,J22B2
		LD	HL,1000H
		JR	J22BD

J22B2:		LD	HL,TXTNAM
		LD	D,00H
		ADD	HL,DE
		ADD	HL,DE
		LD	A,(HL)
		INC	HL
		LD	H,(HL)
		LD	L,A
J22BD:		CALL	C05DE			; convert to SGN and put in DAC
		POP	HL
		RET

J22C2:		PUSH	HL
		SUB	19H
		JR	NC,J22D3
		ADD	A,0FH
		LD	E,A
		CP	0DH
		JR	NZ,J22A2
		LD	HL,(D22DE)
		JR	J22BD

J22D3:		LD	D,00H
		LD	E,A
		LD	HL,I22E0
		ADD	HL,DE
		LD	H,(HL)
		LD	L,D
		JR	J22BD

D22DE:		DEFW	01E00H

I22E0:		DEFB	000H,000H,000H,076H,078H,000H,000H,000H
		DEFB	076H,078H,000H,000H,000H,0FAH,0F0H,000H
		DEFB	000H,000H,0FAH,0F0H

; Subroutine S.VPOKE
J22F4:		CALL	C0602			; evaluate address operand
		PUSH	DE
		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
		EX	(SP),HL
		CALL	C08CB			; write VRAM (active page)
		LD	HL,0
		CALL	C08D6			; read VRAM (active page)
		POP	HL
		RET

; Subroutine S.VPEEK
J2307:		CALL	C05B4			; convert address to integer
		CALL	C08D6			; read VRAM (active page)
		PUSH	AF
		LD	HL,0
		CALL	C08D6			; read VRAM (active page)
		POP	AF
		JP	C05E4			; byte to DAC

; Subroutine evaluate (operand) with max
; Input:  E=max value
C2318:		PUSH	AF
		RST	S_SYNCHR
		DEFB	"("
		RST	S_GETBYT		; evaluate byte operand
		POP	AF
		CP	E
		JP	C,J0546			; illegal function call error
		RST	S_SYNCHR
		DEFB	")"
		LD	A,E
		RET

; Subroutine GETYPR
; Remark: Only called from 2335, which is unused: UNUSED CODE
C2325:		LD	A,(VALTYP)
		CP	8
		JR	NC,J2331
		SUB	3
		OR	A
		SCF
		RET

J2331:		SUB	03H
		OR	A
		RET

; Subroutine convert to integer
; Remark: UNUSED CODE
Q2335:		CALL	C2325
		LD	HL,(DAC+2)
		RET	M
		JP	Z,J0552			; type mismatch error
		JP	J05F0			; convert float (SGN or DBL) to integer

; Subroutine S.SCOPY
J2342:		CP	0C5H
		JP	Z,J3676
		LD	(BUF),HL
		CP	'('
		JR	Z,J23C3
		CALL	C24F6
		LD	(BUF+4),DE
		LD	(BUF+6),BC
		JR	NC,J238C
		DEC	HL
		RST	S_CHRGTR
		JP	Z,J254D
		CP	0D9H
		JR	NZ,J236F
		RST	S_CHRGTR
		CP	'('
		JR	NZ,J2378
		XOR	A
		LD	(BUF+17),A
		JR	J2372

J236F:		CALL	C252B
J2372:		PUSH	HL
		LD	HL,J30A8		; BLTVD
		JR	J23A4

J2378:		CALL	C24F6
		JP	C,J254D
		LD	(BUF+8),DE
		LD	(BUF+10),BC
		PUSH	HL
		LD	HL,J306F		; BLTMD
		JR	J23BD

J238C:		DEC	HL
		RST	S_CHRGTR
		CP	0D9H
		JR	NZ,J239D
		RST	S_CHRGTR
		CP	'('
		JR	NZ,J23AB
		XOR	A
		LD	(BUF+17),A
		JR	J23A0

J239D:		CALL	C252B
J23A0:		PUSH	HL
		LD	HL,J2F42
J23A4:		LD	(BUF+2),HL		; BLTVM
		POP	HL
		JP	J24A9

J23AB:		CALL	C24F6
		JP	NC,J0540		; syntax error
		LD	(BUF+8),DE
		LD	(BUF+10),BC
		PUSH	HL
		LD	HL,J307C		; BLTDM
J23BD:		LD	(BUF+2),HL
		JP	J24E4

J23C3:		PUSH	HL
		LD	HL,J2EB9
		LD	(BUF+2),HL		; BLTVV
		POP	HL
		CALL	C059C			; evaluate complex graphic coordinatepair
		PUSH	BC
		PUSH	DE
		RST	S_SYNCHR
		DEFB	0F2H
		CALL	C05A2
		DEC	HL
		RST	S_CHRGTR
		CP	0D9H
		LD	A,(ACPAGE)
		JR	Z,J23E1
		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
J23E1:		CALL	C1F93
		JP	C,J0546			; illegal function call error
		LD	(BUF+7),A
		POP	DE
		POP	BC
		RST	S_SYNCHR
		DEFB	0D9H
		PUSH	HL
		CALL	C12C4
		LD	(BUF+4),BC
		LD	A,E
		LD	(BUF+6),A
		CALL	C25F1
		CALL	C12C4
		CALL	C25F1
		EX	AF,AF'
		XOR	A
		EX	AF,AF'
		CALL	C25D5
		JR	NC,J240F
		EX	AF,AF'
		OR	04H
		EX	AF,AF'
J240F:		INC	HL
		LD	(BUF+12),HL
		CALL	C25E6
		JR	NC,J241C
		EX	AF,AF'
		OR	08H
		EX	AF,AF'
J241C:		INC	HL
		LD	(BUF+14),HL
		EX	AF,AF'
		LD	(BUF+17),A
		EX	AF,AF'
		PUSH	BC
		PUSH	DE
		LD	BC,(BUF+12)
		LD	E,L
		LD	D,H
		CALL	C3376
		JP	C,J0546			; illegal function call error
		POP	DE
		POP	BC
		POP	HL
		DEC	HL
		RST	S_CHRGTR
		CP	'('
		JR	Z,J24A9
		CALL	C24F6
		LD	(BUF+8),DE
		LD	(BUF+10),BC
		JR	NC,J2450
		PUSH	HL
		LD	HL,J31B4		; BLTDV
		JP	J24A4

J2450:		PUSH	HL
		PUSH	BC
		PUSH	DE
		LD	BC,(BUF+12)
		LD	DE,(BUF+14)
		CALL	C3052
		LD	A,00H
		ADD	A,A
		LD	E,A
		CALL	C253B
		DEC	B
		JR	Z,J248D
		DEC	B
		JR	Z,J2480
		LD	BC,3
		ADD	HL,BC
		LD	A,E
		ADC	A,00H
		SRL	A
		RR	H
		RR	L
		SRL	A
		RR	H
		RR	L
		JR	J248D

J2480:		LD	BC,1
		ADD	HL,BC
		LD	A,E
		ADC	A,00H
		SRL	A
		RR	H
		RR	L
J248D:		POP	DE
		ADD	HL,DE
		JR	C,J249E
		NOP
		LD	DE,4
		ADD	HL,DE
		JR	C,J249E
		NOP
		POP	DE
		EX	DE,HL
		AND	A
		SBC	HL,DE
J249E:		JP	C,J0546			; illegal function call error
		LD	HL,J2EDF		; BLTMV
J24A4:		LD	(BUF+2),HL
		JR	J24DE

J24A9:		CALL	C05A2			; evaluate simple graphic coordinatepair
		PUSH	HL
		CALL	C12C4
		LD	(BUF+8),BC
		LD	A,E
		LD	(BUF+10),A
		POP	HL
		XOR	A
		LD	(BUF+11),A
		DEC	HL
		RST	S_CHRGTR
		LD	A,(ACPAGE)
		JR	Z,J24CE
		RST	S_SYNCHR
		DEFB	','
		CP	','
		LD	A,(ACPAGE)
		CALL	NZ,C060E		; evaluate byte operand
J24CE:		CALL	C1F93
		JP	C,J0546			; illegal function call error
		LD	(BUF+11),A
		CALL	C2594
		LD	(BUF+18),A
		PUSH	HL
J24DE:		CALL	C062C			; new style screenmode ?
		JP	C,J0546			; nope, illegal function call error
J24E4:		LD	HL,I24F0
		PUSH	HL
		LD	HL,(BUF+2)
		PUSH	HL
		LD	HL,BUF+4
		RET

I24F0:		POP	HL
		JP	C,J0546			; illegal function call error
		AND	A
		RET

; Subroutine evaluate string or arrayexpression
; Output: Cx set if string, Cx reset if array
C24F6:		PUSH	HL
		CALL	C05FC			; evaluate expression
		LD	A,(VALTYP)
		CP	3
		JR	Z,J2523
		POP	HL
		LD	A,1
		LD	(SUBFLG),A
		CALL	C05D8			; locate variable
		JP	NZ,J0546		; illegal function call error
		LD	(SUBFLG),A
		PUSH	HL
		LD	H,B
		LD	L,C
		EX	DE,HL
		ADD	HL,DE
		DEC	HL
		PUSH	HL
		LD	A,(BC)
		ADD	A,A
		LD	L,A
		LD	H,00H
		INC	BC
		ADD	HL,BC
		EX	DE,HL
		POP	BC
		POP	HL
		AND	A
		RET

J2523:		PUSH	HL
		CALL	C0614			; free temporary string with type check
		POP	HL
		POP	DE
		SCF
		RET

C252B:		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
		CP	3+1
		JP	NC,J0546		; illegal function call error
		ADD	A,A
		ADD	A,A
		LD	(BUF+17),A
		RST	S_SYNCHR
		DEFB	0D9H
		RET

C253B:		LD	B,1
		LD	A,(SCRMOD)
		CP	8
		RET	Z
		INC	B
		CP	5
		RET	Z
		CP	7
		RET	Z
		INC	B
		INC	B
		RET

J254D:		XOR	A
		LD	(SUBFLG),A
		LD	HL,(BUF)
		SCF
		RET

; Subroutine S.PSET
J2556:		PUSH	AF
		CALL	C05A2			; evaluate simple graphic coordinatepair
		POP	AF
		CALL	C2572
		CALL	C2594
		PUSH	HL
		CALL	C12C4
		JR	NC,J256D
		CALL	C1342
		CALL	C13B5
J256D:		POP	HL
		RET

; Subroutine S.ATRSCN
C256F:		LD	A,(FORCLR)

C2572:		PUSH	BC
		PUSH	DE
		LD	E,A
		CALL	C0620			; in textmode ?
		JP	C,J0546			; yep, illegal function call error
		DEC	HL
		RST	S_CHRGTR
		JR	Z,J2586
		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J2586
		RST	S_GETBYT		; evaluate byte operand
J2586:		LD	A,E
		PUSH	HL
		CALL	C13AD
		JP	C,J0546			; illegal function call error
		POP	HL
		POP	DE
		POP	BC
		JP	J1D3E

C2594:		LD	A,00H
		PUSH	DE
		LD	D,00H
		LD	E,A
		DEC	HL
		RST	S_CHRGTR
		JR	Z,J25CC
		RST	S_SYNCHR
		DEFB	','
		JP	Z,J054C			; missing operand error
		CP	','
		JR	Z,J25CC
		LD	E,A
		CP	'T'
		JR	Z,J25C4
		CP	0D9H
		JR	Z,J25BC
		INC	A
		JR	NZ,J25CB
		RST	S_CHRGTR
		RST	S_SYNCHR
		DEFB	08DH
		RST	S_SYNCHR
		DEFB	"D"
		LD	E,0F6H
		JR	J25C1

J25BC:		RST	S_CHRGTR
		RST	S_SYNCHR
		DEFB	"R"
		LD	E,0F7H
J25C1:		DEC	HL
		JR	J25C9

J25C4:		RST	S_CHRGTR
		JP	Z,J0540			; syntax error
		LD	E,A
J25C9:		LD	D,08H
J25CB:		RST	S_CHRGTR
J25CC:		LD	A,E
		CALL	C27B6
		JP	C,J0540			; syntax error
		POP	DE
		RET

C25D5:		LD	HL,(GXPOS)
		AND	A
		SBC	HL,BC
J25DB:		RET	NC

C25DC:		XOR	A
		SUB	L
		LD	L,A
		SBC	A,H
		SUB	L
		LD	H,A
		XOR	A
		SUB	1
		RET

C25E6:		LD	HL,(GYPOS)
		AND	A
		SBC	HL,DE
		JR	J25DB

C25EE:		CALL	C12C4

C25F1:		CALL	C2600

C25F4:		PUSH	HL
		PUSH	BC
		LD	HL,(GXPOS)
		EX	(SP),HL
		LD	(GXPOS),HL
		POP	BC
		POP	HL
		RET

C2600:		PUSH	HL
		LD	HL,(GYPOS)
		EX	DE,HL
		JR	J263A

J2607:		CALL	C059C			; evaluate complex graphic coordinatepair
		PUSH	BC
		PUSH	DE
		RST	S_SYNCHR
		DEFB	0F2H
		CALL	C05A2
		CALL	C256F
		POP	DE
		POP	BC
		DEC	HL
		RST	S_CHRGTR
		JR	Z,J263F
		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J263F
		RST	S_SYNCHR
		DEFB	"B"
		JR	Z,J2648
		CP	','
		JR	Z,J2648
		RST	S_SYNCHR
		DEFB	"F"
J262A:		CALL	C2594
		PUSH	HL
		CALL	C2898
J2631:		LD	HL,(GRPACX)
		LD	(GXPOS),HL
		LD	HL,(GRPACY)
J263A:		LD	(GYPOS),HL
		POP	HL
		RET

; Subroutine S.DOLINE
J263F:		CALL	C2594
		PUSH	HL
		CALL	C2823
		JR	J2631

; Subroutine S.BOXLIN
J2648:		CALL	C2594
		PUSH	HL
		CALL	C27E7
		JR	J2631

; Subroutine S.PAINT
J2651:		CALL	C059C			; evaluate complex graphic coordinatepair
		PUSH	BC
		PUSH	DE
		CALL	C256F
		LD	A,(ATRBYT)
		LD	E,A
		DEC	HL
		RST	S_CHRGTR
		JR	Z,J2664
		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
J2664:		LD	A,E
		AND	A
		CALL	C14C7
		JP	C,J0546			; illegal function call error
		POP	DE
		POP	BC

; entry point used by KUN BASIC
J266E:		PUSH	HL
		CALL	C27AD
		CALL	C1342
		LD	DE,1
		CALL	C277E
		JR	Z,J2691
		PUSH	HL
		CALL	C278F
		POP	DE
		ADD	HL,DE
		EX	DE,HL
		XOR	A
		CALL	C275B
		LD	A,40H
		CALL	C275B
		LD	B,0C0H
		JR	J26B3

J2691:		POP	HL
		RET

J2693:		LD	A,(INTFLG)
		OR	A
		CALL	NZ,C0589
		LD	A,(LOHDIR)
		OR	A
		JR	Z,J26AC
		LD	HL,(LOHADR)
		PUSH	HL
		LD	HL,(LOHMSK)
		PUSH	HL
		LD	HL,(LOHCNT)
		PUSH	HL
J26AC:		POP	DE
		POP	BC
		POP	HL
		LD	A,C
		CALL	C137D
J26B3:		LD	A,B
		LD	(PDIREC),A
		ADD	A,A
		JR	Z,J2691
		PUSH	DE
		JR	NC,J26C2
		CALL	C1414
		JR	J26C5

J26C2:		CALL	C13FC
J26C5:		POP	DE
		JR	C,J26AC
		CALL	C277E
		JP	Z,J26AC
		XOR	A
		LD	(LOHDIR),A
		CALL	C278F
		LD	E,L
		LD	D,H
		OR	A
		JR	Z,J26F4
		DEC	HL
		DEC	HL
		LD	A,H
		ADD	A,A
		JR	C,J26F4
		LD	(LOHCNT),DE
		CALL	C1376
		LD	(LOHADR),HL
		LD	(LOHMSK),A
		LD	A,(PDIREC)
		CPL
		LD	(LOHDIR),A
J26F4:		LD	HL,(MOVCNT)
		ADD	HL,DE
		EX	DE,HL
		CALL	C274F
		LD	HL,(CSAVEA)
		LD	A,(CSAVEM)
		CALL	C137D
J2705:		LD	HL,(SKPCNT)
		LD	DE,(MOVCNT)
		OR	A
		SBC	HL,DE
		JR	Z,J274C
		JR	C,J272F
		EX	DE,HL
		LD	B,1
		CALL	C277E
		JR	Z,J274C
		OR	A
		JR	Z,J2705
		EX	DE,HL
		LD	HL,(CSAVEA)
		LD	A,(CSAVEM)
		LD	C,A
		LD	A,(PDIREC)
		LD	B,A
		CALL	C2760
		JR	J2705

J272F:		CALL	C25DC
		DEC	HL
		DEC	HL
		LD	A,H
		ADD	A,A
		JR	C,J274C
		INC	HL
		PUSH	HL
		EX	DE,HL
		LD	HL,(CLOC)
		OR	A
		SBC	HL,DE
		LD	(CLOC),HL
		POP	DE
		LD	A,(PDIREC)
		CPL
		CALL	C275B
J274C:		JP	J2693

C274F:		LD	A,(LFPROG)
		LD	C,A
		LD	A,(RTPROG)
		OR	C
		RET	Z
		LD	A,(PDIREC)

C275B:		LD	B,A
		CALL	C1376
		LD	C,A

C2760:		EX	(SP),HL
		PUSH	BC
		PUSH	DE
		PUSH	HL
		LD	C,2
		PUSH	HL
		LD	HL,(STREND)
		LD	B,00H
		ADD	HL,BC
		ADD	HL,BC
		LD	A,088H			; LOW -120
		SUB	L
		LD	L,A
		LD	A,0FFH			; HIGH -120
		SBC	A,H
		LD	H,A
		JR	C,J277B
		ADD	HL,SP
		POP	HL
		RET	C
J277B:		JP	J0558			; out of memory error

C277E:		CALL	C14DB
		LD	(SKPCNT),DE
		LD	(MOVCNT),HL
		LD	A,H
		OR	L
		LD	A,C
		LD	(RTPROG),A
		RET

C278F:		CALL	C1376
		PUSH	HL
		PUSH	AF
		LD	HL,(CSAVEA)
		LD	A,(CSAVEM)
		CALL	C137D
		POP	AF
		POP	HL
		LD	(CSAVEA),HL
		LD	(CSAVEM),A
		CALL	C158F
		LD	A,C
		LD	(LFPROG),A
		RET

C27AD:		PUSH	HL
		CALL	C12C4
		JP	NC,J0546		; illegal function call error
		POP	HL
		RET

C27B6:		AND	A
		JR	Z,J27E2
		PUSH	BC
		LD	B,A
		CALL	C062C			; new style screenmode ?
		LD	A,B
		POP	BC
		RET	C			; nope, quit
		CP	0F8H
		JR	Z,J27E0
		CP	0F7H
		JR	Z,J27DD
		CP	0F6H
		JR	Z,J27DA
		CP	0C3H
		JR	Z,J27D7
		CP	0C2H
		SCF
		RET	NZ
		XOR	A
		DEFB	0C2H
J27D7:		LD	A,4
		DEFB	0C2H
J27DA:		LD	A,1
		DEFB	0C2H
J27DD:		LD	A,2
		DEFB	0C2H
J27E0:		LD	A,3
J27E2:		OR	D
		LD	(LOGOPR),A
		RET

; Subroutine S.NVBXLN
C27E7:		CALL	C25EE
		CALL	C25EE
		CALL	C25D5
		JR	Z,C2823
		CALL	C,C25F4
		CALL	C25E6
		JR	Z,C2823
		CALL	C,C2600
		LD	HL,(GYPOS)
		PUSH	HL
		PUSH	DE
		EX	DE,HL
		CALL	C2823
		POP	HL
		LD	(GYPOS),HL
		EX	DE,HL
		CALL	C2823
		POP	HL
		DEC	HL
		INC	DE
		LD	(GYPOS),HL
		LD	HL,(GXPOS)
		PUSH	BC
		LD	C,L
		LD	B,H
		CALL	C2823
		POP	HL
		LD	(GXPOS),HL
		LD	C,L
		LD	B,H

; Subroutine S.DOGRPH
C2823:		PUSH	BC
		PUSH	DE
		PUSH	HL
		LD	HL,(GYPOS)
		PUSH	HL
		LD	HL,(GXPOS)
		PUSH	HL
		CALL	C25EE
		CALL	C12C4
		CALL	C25E6
		CALL	C,C25F1
		PUSH	DE
		PUSH	HL
		EX	AF,AF'
		XOR	A
		EX	AF,AF'
		CALL	C25D5
		EX	DE,HL
		JR	NC,J2849
		EX	AF,AF'
		OR	04H
		EX	AF,AF'
J2849:		POP	HL
		RST	S_DCOMPR
		JR	C,J2852
		EX	DE,HL
		EX	AF,AF'
		OR	01H
		EX	AF,AF'
J2852:		EX	(SP),HL
		CALL	C2980			; wait for vdp command to complete
		LD	A,36
		DI
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer to 36, autoincrement
		LD	A,C
		OUT	(9BH),A
		LD	A,B
		OUT	(9BH),A
		LD	A,L
		OUT	(9BH),A
		LD	A,(ACPAGE)
		OUT	(9BH),A
		LD	A,E
		OUT	(9BH),A
		LD	A,D
		OUT	(9BH),A
		POP	HL
		LD	A,L
		OUT	(9BH),A
		LD	A,H
		OUT	(9BH),A
		LD	A,(ATRBYT)
		OUT	(9BH),A
		EX	AF,AF'
		OUT	(9BH),A
		LD	A,(LOGOPR)
		AND	0FH
		OR	70H
		OUT	(9BH),A
		EI
		POP	HL
		LD	(GXPOS),HL
		POP	HL
		LD	(GYPOS),HL
		POP	HL
		POP	DE
		POP	BC
		RET

; Subroutine S.NVBXFL
C2898:		CALL	C12C4
		LD	L,C
		LD	H,B
		LD	A,E
		CALL	C29A5			; write DX,DY registers
		CALL	C25F1
		CALL	C25EE
		EX	AF,AF'
		XOR	A
		EX	AF,AF'
		CALL	C25D5
		JR	NC,J28B3
		EX	AF,AF'
		OR	04H
		EX	AF,AF'
J28B3:		INC	HL
		PUSH	HL
		CALL	C25E6
		JR	NC,J28BE
		EX	AF,AF'
		OR	08H
		EX	AF,AF'
J28BE:		INC	HL
		POP	DE
		CALL	C29C4			; write NX,NY registers
		EX	AF,AF'
		CALL	C29DB			; write ARG register
		EX	AF,AF'
		LD	A,(ATRBYT)
		CALL	C29EB			; write colorregister
		LD	A,(LOGOPR)
		AND	0FH
		ADD	A,80H			; LMMV (logical move) command
		CALL	C29E3			; execute vdp command

C28D8:		LD	B,4
		JR	J28E6

C28DC:		LD	B,6
		JR	J28E6

C28E0:		LD	B,2
		JR	J28E6

C28E4:		LD	B,0
J28E6:		PUSH	BC
		CALL	C2925			; setup SX,SY and CLR
		POP	AF
		DI
		OUT	(99H),A
		LD	A,45+128
		OUT	(99H),A			; setup ARG
		LD	A,60H			; SRCH command
		OUT	(99H),A
		LD	A,46+128
		OUT	(99H),A			; execute command
		CALL	C2980			; wait for vdp command to complete
		AND	10H
		RET	Z
		LD	A,8
		DI
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister 8
		IN	A,(99H)
		LD	L,A
		LD	A,9
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister 9
		IN	A,(99H)
		PUSH	AF
		XOR	A
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister 0
		EI
		POP	AF
		AND	01H
		LD	H,A
		SCF
		RET

C2925:		PUSH	AF
		CALL	C2980			; wait for vdp command to complete
		LD	A,32
		DI
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer to 32, autoincrement
		LD	A,L
		OUT	(9BH),A			; SX low
		LD	A,H
		OUT	(9BH),A			; SX high
		LD	A,E
		OUT	(9BH),A			; SY low
		LD	A,(ACPAGE)
		OUT	(9BH),A			; SY high
		POP	AF
		OUT	(99H),A
		LD	A,44+128
		OUT	(99H),A			; CLR (data to VDP)
		EI
		RET

C2949:		PUSH	DE
		CALL	C29A0			; write SX,SY registers
		XOR	A
		CALL	C29DB			; write ARG register
		LD	A,40H			; POINT vdp command
		CALL	C29E3			; execute vdp command
		CALL	C2980			; wait for vdp command to complete
		LD	A,7
		CALL	C298B			; read vdp status register 7
		EI
		POP	DE
		RET

C2961:		CALL	C29A5			; write DX,DY registers
		LD	A,44
		DI
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer to 44, autoincrement
		LD	A,(ATRBYT)
		OUT	(9BH),A
		XOR	A
		OUT	(9BH),A
		LD	A,(LOGOPR)
		AND	0FH
		OR	50H
		OUT	(9BH),A
		EI
		RET

; Subroutine wait for vdp command to complete
C2980:		LD	A,2
		CALL	C298B			; read vdp status register 2
		EI
		RRCA				; vdp command executing ?
		JR	C,C2980			; yep, wait
		RLCA
		RET

; Subroutine S.VDPSTA
C298B:		DI
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister
		PUSH	HL
		POP	HL
		IN	A,(99H)			; read vdp statusregister
		PUSH	AF
		XOR	A
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister 0
		POP	AF
		RET

; Subroutine write SX,SY registers
C29A0:		PUSH	AF
		LD	A,32
		JR	J29A8

; Subroutine write DX,DY registers
C29A5:		PUSH	AF
		LD	A,36
J29A8:		PUSH	AF
		CALL	C2980			; wait for vdp command to complete
		DI
		POP	AF
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer
		LD	A,L
		OUT	(9BH),A
		LD	A,H
		OUT	(9BH),A
		POP	AF
		OUT	(9BH),A
		LD	A,(ACPAGE)
		OUT	(9BH),A
		EI
		RET

; Subroutine write NX,NY registers
C29C4:		LD	A,40
		DI
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer to 40, autoincrement
		LD	A,E
		OUT	(9BH),A
		LD	A,D
		OUT	(9BH),A
		LD	A,L
		OUT	(9BH),A
		LD	A,H
		OUT	(9BH),A
		EI
		RET

; Subroutine write ARG register
C29DB:		PUSH	AF
		DI
		OUT	(99H),A
		LD	A,45+128
		JR	J29F1

; Subroutine execute vdp command
C29E3:		PUSH	AF
		DI
		OUT	(99H),A
		LD	A,46+128
		JR	J29F1

; Subroutine write colorregister
C29EB:		PUSH	AF
		DI
		OUT	(99H),A
		LD	A,44+128
J29F1:		OUT	(99H),A
		EI
		POP	AF
		RET

; Initial VDP register values
I29F6:		DEFB	000H			; R#0: DG=0 (Digitize off), IE2=0 (lightpen int off) IE1=0 (VR int off), M5=0, M4=0, M3=0 (screen 1)
		DEFB	020H			; R#1: BLK=0 Enable display IE0=1 (HR int on), M1=0, M2=0 (screen 1), SZ=0 (8x8 sprites), MAG=0 (sprites not magnified)
		DEFB	000H			; R#2: A16=0, A15=0, A14=0, A13=0, A12=0, A11=0, A10=0
		DEFB	000H			; R#3: B13=0, B12=0, B11=0, B10=0, B9=0,	B8=0,	B7=0, B6=0
		DEFB	000H			; R#4: C16=0, C15=0, C14=0, C13=0, C12=0, C11=0
		DEFB	000H			; R#5: D14=0, D13=0, D12=0, D11=0, D10=0, D9=0,	D8=0, D7=0
		DEFB	000H			; R#6: E16=0, E15=0, E14=0, E13=0, E12=0, E11=0
		DEFB	000H			; R#7: TC3=0, TC2=0, TC1=0, TC0=0 (text color 0), BDC3=0,BDC2=0,BDC1=0,BDC0=0 (border color 0)

		DEFB	008H			; R#8: MS=0 (disable mouse), LP=0 (disable lightpen), TP=0 (color 0 is transparent), CBD=0 (colorbus not input), 
						;      VRS1=0, VRS0=0 (64 KB RAM chip), SPD=0 (sprites enabled), B/W=0 (blank/white off)
	IF INTHZ = 60
		DEFB	000H			; R#9: LN=0, SYM1=0, SYM0=0 (internal synchronisation), IL=0 (no interlace), EO=0 (no odd even display), NTSC=0 (NTSC), DCD=0 (dot clock direction output)
	ELSE
		DEFB	002H			; R#9: LN=0, SYM1=0, SYM0=0 (internal synchronisation), IL=0 (no interlace), EO=0 (no odd even display), NTSC=1 (PAL), DCD=0 (dot clock direction output)
	ENDIF
		DEFB	000H			; R#10: B16=0, B15=0, B14=0
		DEFB	000H			; R#11: D16=0, D15=0
		DEFB	000H			; R#12: C3=0, C2=0, C1=0, C0=0 (color 0), BC3=0, BC2=0, BC1=0, BC0=0 (back color 0)
		DEFB	000H			; R#13: ON3=0, ON2=0, ON1=0, ON0=0 (blink on ?), OF3=0, OF2=0, OF1=0, OF0=0 (blink off ?)
		DEFB	000H			; R#14: F16=0, F15=0, F14=0 (VRAM base access is lower 16 KB, TMS9918 compatibel)
		DEFB	000H			; R#15: RS3=0, RS2=0, RS1=0, RS0=0 (select status register 0, TMS9918 compatibel)
		DEFB	000H			; R#16: C3=0, C2=0, C1=0, C0=0 (color code 0)
		DEFB	000H			; R#17: AII=0 (autoincrement register), RC5=0, RC4=0, RC3=0, RC2=0, RC1=0, RC0=0 (control register 0)
		DEFB	000H			; R#18: dV3=0, dV2=0, dV1=0, dV0=0 (no vertical adjust), dH3=0, dH2=0, dH1=0, dH0=0 (no horizontal adjust)
		DEFB	000H			; R#19: IL7=0, IL6=0, IL5=0, IL4=0, IL3=0, IL2=0, IL1=0, IL0=0 (interrupt on line 0)
		DEFB	000H			; R#20: CBX5=0, CBX4=0, CBX3=0, CBX2=0, CBX1=0, CBX0=0 (Color burst value of phase 0)
		DEFB	03BH			; R#21: CBX5=1, CBX4=1, CBX3=1, CBX2=0, CBX1=1, CBX0=1 (Color burst value of phase 1/3)
		DEFB	005H			; R#22: CBX5=0, CBX4=0, CBX3=0, CBX2=1, CBX1=0, CBX0=1 (Color burst value of phase 2/3)
		DEFB	000H			; R#23: DO7=0, DO6=0, DO5=0, DO4=0, DO3=0, DO2=0, DO1=0, DO0=0 (display offset)

; Subroutine display msx startscreen
C2A0E:		CALL	C2A14
		JP	J1B6D

C2A14:		CALL	C2B40
		DI
		LD	A,55H
		LD	HL,0
		LD	BC,08000H
		CALL	C0977			; fill VRAM (active page)
		CALL	C2AFF
		LD	DE,0705H
		CALL	C2AC2			; write vdp register
		LD	DE,2D00H
		CALL	C2AC2			; write vdp register
		LD	D,2AH
		CALL	C2AC2			; write vdp register
		INC	D
		CALL	C2AC2			; write vdp register
		LD	D,27H
		CALL	C2AC2			; write vdp register
		LD	E,37H
		LD	HL,I2BF6
		EXX
		LD	HL,I2B6F
		EXX
		LD	DE,0160H
		CALL	C2AC2			; write vdp register
		LD	A,0F0H
		LD	(BUF),A
J2A55:		DI
		CALL	C1034			; wait for VR to pass
		LD	A,(BUF)
		LD	B,A
		LD	C,23
		CALL	C0647			; write vdp register 23 (vertical scroll)
		LD	A,B
		INC	A
		LD	(BUF),A
		EI
		LD	A,3
		PUSH	AF
		LD	BC,002DH
		LD	D,26H
		CALL	C2AC2			; write vdp register
		EXX
		LD	B,8
		LD	C,(HL)
		INC	HL
		EXX
J2A79:		CALL	C2AD8			; setup vdp dx,dy
		LD	A,(HL)
		INC	HL
		CP	0FEH
		JR	Z,J2AB9
		PUSH	HL
		PUSH	BC
		LD	BC,001A6H
		JR	NC,J2A8C
		LD	B,0
		LD	C,A
J2A8C:		CALL	C2AD4			; setup vdp nx,ny
		POP	HL
		ADD	HL,BC
		LD	C,L
		LD	B,H
		POP	HL
		POP	AF
		XOR	03H
		CALL	C2AE3			; setup vdp color
		PUSH	AF
		CALL	C2AEC
		EXX
		SLA	C
		DJNZ	J2AA7
		LD	B,8
		LD	C,(HL)
		INC	HL
J2AA7:		EXX
		JR	NC,J2A79
		DEC	BC
		CALL	C2AD8			; setup vdp dx,dy
		INC	BC
		LD	A,2
		CALL	C2AE3			; setup vdp color
		CALL	C2AFA
		JR	J2A79

J2AB9:		INC	E
		POP	AF
		LD	A,(HL)
		CP	0FEH
		JR	NZ,J2A55
		EI
		RET

; Subroutine write vdp register
C2AC2:		PUSH	BC
		PUSH	AF
		DI
		LD	C,99H
		OUT	(C),E
		LD	A,D
		OR	80H
		EX	(SP),HL
		EX	(SP),HL
		OUT	(C),A
		EI
		POP	AF
		POP	BC
		RET

; Subroutine setup vdp nx,ny
C2AD4:		LD	D,40
		JR	J2ADA

; Subroutine setup vdp dx,dy
C2AD8:		LD	D,36
J2ADA:		PUSH	DE
		LD	E,C
		CALL	C2AC2			; write vdp register
		LD	E,B
		INC	D
		JR	J2AE7

; Subroutine setup vdp color
C2AE3:		PUSH	DE
		LD	E,A
		LD	D,44
J2AE7:		CALL	C2AC2			; write vdp register
		POP	DE
		RET

; Subroutine 
C2AEC:		PUSH	DE
		LD	E,70H			; Line command
J2AEF:		LD	D,46
		CALL	C2AC2			; write vdp register
		CALL	C2980			; wait for vdp command to complete
		DI
		POP	DE
		RET

C2AFA:		PUSH	DE
		LD	E,50H			; Pset command
		JR	J2AEF

C2AFF:		LD	A,2
		CALL	C1CF9			; select RP-501 bank 2
		LD	B,11
		CALL	C1CD2			; read RP-501 register 11
		RLCA
		RLCA
		AND	0CH
		LD	C,A
		LD	B,00H
		LD	HL,I2B5F
		ADD	HL,BC
		LD	C,(HL)
		INC	HL
		LD	B,(HL)
		PUSH	BC
		INC	HL
		LD	E,(HL)
		INC	HL
		LD	D,(HL)
		EX	DE,HL
		LD	DE,1000H
		CALL	C2AC2			; write vdp register
		CALL	C2B33
		POP	HL
		CALL	C2B33
		LD	HL,0444H
		CALL	C2B33
		LD	HL,0777H

C2B33:		PUSH	BC
		LD	C,9AH
		DI
		OUT	(C),L
		EX	(SP),HL
		EX	(SP),HL
		OUT	(C),H
		EI
		POP	BC
		RET

C2B40:		LD	B,8
		LD	HL,I2B4F
J2B45:		LD	D,(HL)
		INC	HL
		LD	E,(HL)
		INC	HL
		CALL	C2AC2			; write vdp register
		DJNZ	J2B45
		RET

I2B4F:		DEFB	00H,08H
		DEFB	01H,20H
		DEFB	08H,2AH

	IF INTHZ = 60
		DEFB	09H,00H
		ELSE
		DEFB	09H,02H
	ENDIF

		DEFB	02H,1FH
		DEFB	05H,0F7H
		DEFB	0BH,00H
		DEFB	06H,0EH

I2B5F:		DEFB	07H,00H,00H,00H,20H,04H,27H,02H
		DEFB	72H,02H,56H,00H,70H,05H,70H,00H

I2B6F:		DEFB	00H,00H,00H,00H,00H,00H,00H,00H
		DEFB	00H,6EH,00H,9FH,00H,6FH,00H,9FH
		DEFB	00H,6FH,00H,9FH,00H,6FH,00H,9FH
		DEFB	00H,6FH,00H,9FH,00H,6FH,00H,9FH
		DEFB	00H,6FH,00H,0FFH,00H,07H,00H,0FCH
		DEFB	07H,00H,0F7H,00H,07H,00H,0F3H,00H
		DEFB	07H,00H,0F7H,00H,07H,00H,0FFH,00H
		DEFB	07H,00H,0FFH,00H,0FH,00H,0FCH,3CH
		DEFB	0FCH,3CH,0FCH,3CH,87H,0C0H,7BH,0C0H
		DEFB	87H,0C0H,7BH,0C0H,86H,0C0H,7BH,0C0H
		DEFB	86H,0C0H,7AH,0C0H,86H,0C0H,78H,0C0H
		DEFB	86H,90H,7AH,0F0H,86H,0F0H,7FH,0C0H
		DEFB	83H,0C0H,7FH,0C0H,0D7H,0C0H,2FH,0C0H
		DEFB	0D7H,0C0H,2FH,0C0H,0D7H,0C0H,2FH,0C0H
		DEFB	0D7H,0C0H,2FH,0C0H,0D7H,0C0H,2FH,0C0H
		DEFB	0D7H,0C0H,2FH,0C0H,0D5H,80H,00H,00H
		DEFB	00H,00H,00H,00H,00H,00H,00H

I2BF6:		DEFB	0FFH
		DEFB	0FEH,0FFH,0FEH,0FFH,0FEH,0FFH,0FEH,0FFH
		DEFB	0FEH,0FFH,0FEH,0FFH,0FEH,0FFH,0FEH,0FFH
		DEFB	0FEH,44H,1DH,1DH,1CH,2FH,68H,2BH
		DEFB	28H,22H,0FEH,44H,1DH,1CH,1EH,2AH
		DEFB	6DH,29H,29H,22H,0FEH,43H,1FH,1BH
		DEFB	1EH,26H,73H,25H,29H,24H,0FEH,43H
		DEFB	1FH,1AH,20H,22H,77H,23H,29H,25H
		DEFB	0FEH,42H,21H,19H,20H,1FH,7CH,1FH
		DEFB	29H,27H,0FEH,42H,21H,18H,22H,1BH
		DEFB	80H,1DH,29H,28H,0FEH,41H,23H,17H
		DEFB	22H,19H,84H,1AH,28H,2AH,0FEH,41H
		DEFB	23H,16H,24H,17H,86H,17H,29H,2BH
		DEFB	0FEH,40H,25H,15H,24H,15H,8AH,14H
		DEFB	28H,2DH,0FEH,40H,25H,14H,26H,13H
		DEFB	8CH,11H,29H,2EH,0FEH,3FH,27H,13H
		DEFB	26H,12H,8FH,0EH,29H,2FH,0FEH,3FH
		DEFB	27H,12H,28H,10H,91H,0CH,28H,31H
		DEFB	0FEH,3EH,29H,11H,28H,0FH,94H,08H
		DEFB	29H,32H,0FEH,3EH,2AH,0FH,2AH,0DH
		DEFB	96H,06H,28H,34H,0FEH,3DH,2BH,0EH
		DEFB	2BH,0CH,99H,02H,29H,35H,0FEH,3DH
		DEFB	2CH,0DH,2CH,0BH,0C2H,37H,0FEH,3CH
		DEFB	2DH,0CH,2DH,0AH,26H,4CH,50H,38H
		DEFB	0FEH,3CH,2EH,0BH,2EH,09H,23H,51H
		DEFB	4CH,3AH,0FEH,3BH,2FH,0AH,2FH,09H
		DEFB	21H,54H,4AH,3BH,0FEH,3BH,30H,09H
		DEFB	30H,08H,20H,57H,46H,3DH,0FEH,3AH
		DEFB	31H,08H,31H,08H,21H,57H,44H,3EH
		DEFB	0FEH,3AH,32H,07H,32H,07H,23H,57H
		DEFB	41H,3FH,0FEH,39H,33H,06H,33H,07H
		DEFB	26H,55H,3EH,41H,0FEH,39H,34H,05H
		DEFB	34H,07H,47H,35H,3BH,42H,0FEH,38H
		DEFB	35H,04H,35H,07H,4BH,32H,38H,44H
		DEFB	0FEH
		DEFB	38H,36H,03H,36H,07H,4EH,30H,35H
		DEFB	45H,0FEH,37H,37H,02H,37H,08H,50H
		DEFB	2EH,32H,47H,0FEH,37H,71H,08H
		DEFB	52H,2DH,2FH,48H,0FEH,36H,72H,09H
		DEFB	54H,2BH,2CH,4AH,0FEH,36H,73H,09H
		DEFB	55H,2BH,29H,4BH,0FEH,35H,74H,0AH
		DEFB	55H,29H,2AH,4BH,0FEH,35H,75H,0BH
		DEFB	55H,25H,2DH,4AH,0FEH,34H,76H,0CH
		DEFB	55H,23H,30H,48H,0FEH,34H,1FH,01H
		DEFB	37H,01H,1FH,0DH,54H,20H,33H,47H
		DEFB	0FEH,33H,20H,02H,36H,02H,1EH,10H
		DEFB	52H,1EH,36H,45H,0FEH,33H,1FH,03H
		DEFB	35H,03H,1FH,12H,50H,1BH,39H,44H
		DEFB	0FEH,32H,20H,04H,34H,04H,1EH,15H
		DEFB	4EH,19H,3CH,42H,0FEH,32H,1FH,05H
		DEFB	33H,05H,1FH,18H,4AH,18H,3EH,41H
		DEFB	0FEH,31H,20H,06H,32H,06H,1EH,1CH
		DEFB	47H,15H,42H,3FH,0FEH,31H,1FH,07H
		DEFB	31H,07H,1FH,3CH,26H,14H,44H,3EH
		DEFB	0FEH,30H,20H,08H,30H,08H,1EH,40H
		DEFB	22H,12H,48H,3CH,0FEH,30H,1FH,09H
		DEFB	2FH,09H,1FH,41H,20H,11H,4AH,3BH
		DEFB	0FEH,2FH,20H,0AH,2EH,0AH,1EH,41H
		DEFB	20H,0FH,4EH,39H,0FEH,2FH,1FH,0BH
		DEFB	2DH,0BH,1FH,40H,20H,0EH,28H,01H
		DEFB	27H,38H,0FEH,2EH,20H,0CH,2CH,0CH
		DEFB	1EH,3EH,22H,0CH,29H,04H,27H,36H
		DEFB	0FEH,2EH,1FH,0DH,2BH,0DH,1FH,39H
		DEFB	26H,0BH,29H,06H,27H,35H,0FEH,2DH
		DEFB	20H,0EH,2AH,0EH,7DH,0AH,28H,0AH
		DEFB	27H,33H,0FEH,2DH,1FH,0FH,29H,0FH
		DEFB	7CH,09H,29H,0CH,27H,32H,0FEH,2CH
		DEFB	20H,10H,28H,10H,7BH,08H,28H,10H
		DEFB	27H,30H,0FEH,2CH,1FH,11H,27H,11H
		DEFB	7AH,07H,29H,12H,27H,2FH,0FEH,2BH
		DEFB	1FH,13H,25H,13H,78H,07H,28H,16H
		DEFB	27H,2DH,0FEH,2BH,1FH,13H,25H,13H
		DEFB	77H,06H,29H,18H,27H,2CH,0FEH,2AH
		DEFB	1FH,15H,23H,15H,75H,06H,28H,1CH
		DEFB	27H,2AH,0FEH,2AH,1FH,15H,23H,15H
		DEFB	74H,06H,27H,1FH,27H,29H,0FEH,29H
		DEFB	1FH,17H,21H,17H,71H,06H,28H,22H
		DEFB	27H,27H,0FEH,29H,1FH,17H,21H,17H
		DEFB	70H,06H,27H,25H,27H,26H,0FEH,28H
		DEFB	1FH,19H,1FH,19H,6DH,06H,28H,28H
		DEFB	27H,24H,0FEH,28H,1FH,19H,1FH,19H
		DEFB	6AH,08H,27H,2BH,27H,23H,0FEH,27H
		DEFB	1FH,1BH,1DH,1BH,66H,09H,28H,2EH
		DEFB	27H,21H,0FEH,27H,1FH,1BH,1DH,1BH
		DEFB	63H,0BH,27H,31H,27H,20H,0FEH,26H
		DEFB	1FH,1DH,1BH,1DH,5FH,0CH,28H,34H
		DEFB	27H,1EH,0FEH,26H,1FH,1DH,1BH,1DH
		DEFB	5BH,0EH,28H,37H,27H,1DH,0FEH,0FFH
		DEFB	0FEH,0FFH,0FEH,0FFH,0FEH,0FFH,0FEH,0FFH
		DEFB	0FEH,0FFH,0FEH,0FFH,0FEH,0FFH,0FEH,0FFH
		DEFB	0FEH,0FEH


; ******************************************
; Begin Section Blit
; ******************************************


; Subroutine S.BLTVV (blit vram-vram)
J2EB9:		LD	BC,(BUF+12)
		LD	DE,(BUF+14)
		CALL	C3376
		RET	C
		CALL	C2980			; wait for vdp command to complete
		LD	A,32			; register 32, autoincrement
		CALL	C336E			; set indirect vdp register pointer
		LD	HL,BUF+4
		LD	BC,0E9BH
		OTIR
		LD	A,(HL)
		AND	0FH
		OR	90H			; LMMM (logical move) command
		OUT	(C),A
		EI
		AND	A
		RET

; Subroutine S.BLTMV (blit vram-ram)
J2EDF:		CALL	C3320
		LD	A,(SCRMOD)
		SUB	6
		JR	C,J2EFA
		JR	Z,J2F1B
		DEC	A
		JR	Z,J2EFA
J2EEE:		CALL	C3354
		LD	(HL),A
		INC	HL
		DEC	BC
		LD	A,C
		OR	B
		JP	NZ,J2EEE
		RET

J2EFA:		LD	D,2
		LD	(HL),0
J2EFE:		CALL	C2F13
		CALL	C3354
		OR	(HL)
		LD	(HL),A
		DEC	BC
		LD	A,C
		OR	B
		JR	Z,J2F11
		DEC	D
		JR	NZ,J2EFE
		INC	HL
		JR	J2EFA

J2F11:		DEC	D
		RET	Z

C2F13:		LD	A,(HL)
		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
		LD	(HL),A
		AND	A
		RET

J2F1B:		LD	D,4
		LD	(HL),00H
J2F1F:		LD	A,(HL)
		ADD	A,A
		ADD	A,A
		LD	(HL),A
		CALL	C3354
		OR	(HL)
		LD	(HL),A
		DEC	BC
		LD	A,C
		OR	B
		JR	NZ,J2F31
		OR	E
		JR	Z,J2F37
		DEC	E
J2F31:		DEC	D
		JR	NZ,J2F1F
		INC	HL
		JR	J2F1B

J2F37:		DEC	D
		RET	Z
		LD	A,(HL)
J2F3A:		ADD	A,A
		ADD	A,A
		DEC	D
		JR	NZ,J2F3A
		LD	(HL),A
		AND	A
		RET

; Subroutine S.BLTVM (blit ram-vram)
J2F42:		CALL	C32C9
		RET	C
		CALL	C2980			; wait for vdp command to complete
		LD	A,32			; register 32, autoincrement
		CALL	C336E			; set indirect vdp register pointer
		PUSH	BC
		LD	HL,BUF+4
		LD	BC,0E9BH
		OTIR
		POP	BC
		LD	A,(HL)
		AND	0FH
		OR	0B0H			; LMMC (logical move) command
		POP	HL
		PUSH	AF
		LD	A,(SCRMOD)
		SUB	6
		JR	C,J2F92
		JR	Z,J2FD4
		DEC	A
		JR	Z,J2F92
		CALL	C336C			; set indirect vdp register pointer to 44, non-autoincrement
		LD	A,(BUF+74)
		LD	E,A
		AND	A
		CALL	NZ,C33A2
		LD	A,(HL)
		OUT	(9BH),A
		POP	AF
		OUT	(99H),A
		LD	A,46+128
		OUT	(99H),A			; execute command
		EI
J2F81:		DEC	BC
		LD	A,C
		OR	B
		RET	Z
		INC	HL
		LD	A,E
		AND	A
		CALL	NZ,C33A2
		JR	C,J2F81
		LD	A,(HL)
		OUT	(9BH),A
		JR	J2F81

J2F92:		CALL	C336C			; set indirect vdp register pointer to 44, non-autoincrement
		LD	D,0FH
		LD	A,(BUF+74)
		LD	E,A
		AND	A
		CALL	NZ,C33A2
		LD	A,(HL)
		RRCA
		RRCA
		RRCA
		RRCA
		AND	D
		OUT	(9BH),A
		POP	AF
		OUT	(99H),A
		LD	A,46+128
		OUT	(99H),A			; execute command
		EI
J2FAF:		DEC	BC
		LD	A,C
		OR	B
		RET	Z
		LD	A,E
		AND	A
		CALL	NZ,C33A2
		JR	C,J2FBE
		LD	A,(HL)
		AND	D
		OUT	(9BH),A
J2FBE:		INC	HL
		DEC	BC
		LD	A,C
		OR	B
		RET	Z
		LD	A,E
		AND	A
		CALL	NZ,C33A2
		JR	C,J2FAF
		LD	A,(HL)
		RRCA
		RRCA
		RRCA
		RRCA
		AND	D
		OUT	(9BH),A
		JR	J2FAF

J2FD4:		CALL	C336C			; set indirect vdp register pointer to 44, non-autoincrement
		LD	D,3
		LD	A,(BUF+74)
		AND	A
		CALL	NZ,C33A2
		LD	A,(HL)
		RRCA
		RRCA
		AND	D
		OUT	(9BH),A
		POP	AF
		OUT	(99H),A
		LD	A,46+128
		OUT	(99H),A			; execute command
		EI
J2FEE:		DEC	BC
		LD	A,C
		OR	B
		JP	NZ,J2FF7
		OR	E
		RET	Z
		DEC	E
J2FF7:		LD	A,(BUF+74)
		AND	A
		CALL	NZ,C33A2
		JR	C,J3008
		LD	A,(HL)
		RRCA
		RRCA
		RRCA
		RRCA
		AND	D
		OUT	(9BH),A
J3008:		DEC	BC
		LD	A,C
		OR	B
		JP	NZ,J3011
		OR	E
		RET	Z
		DEC	E
J3011:		LD	A,(BUF+74)
		AND	A
		CALL	NZ,C33A2
		JR	C,J3020
		LD	A,(HL)
		RRCA
		RRCA
		AND	D
		OUT	(9BH),A
J3020:		DEC	BC
		LD	A,C
		OR	B
		JP	NZ,J3029
		OR	E
		RET	Z
		DEC	E
J3029:		LD	A,(BUF+74)
		AND	A
		CALL	NZ,C33A2
		JR	C,J3036
		LD	A,(HL)
		AND	D
		OUT	(9BH),A
J3036:		DEC	BC
		LD	A,C
		OR	B
		JP	NZ,J303F
		OR	E
		RET	Z
		DEC	E
J303F:		INC	HL
		LD	A,(BUF+74)
		AND	A
		CALL	NZ,C33A2
		JR	C,J2FEE
		LD	A,(HL)
		RLCA
		RLCA
		AND	D
		OUT	(9BH),A
		JP	J2FEE

; Subroutine multiply
C3052:		LD	HL,0
		LD	A,16
		AND	A
J3058:		PUSH	AF
		ADD	HL,HL
		JR	NC,J305F
		POP	AF
		SCF
		PUSH	AF
J305F:		EX	DE,HL
		ADD	HL,HL
		EX	DE,HL
		JR	NC,J306A
		ADD	HL,BC
		JR	NC,J306A
		POP	AF
		SCF
		PUSH	AF
J306A:		POP	AF
		DEC	A
		JR	NZ,J3058
		RET

; Subroutine S.BLTMD (blit disk to ram)



J306F:		PUSH	HL
		CALL	C32BB
		POP	HL
		INC	HL
		INC	HL
		INC	HL
		INC	HL
		LD	E,0			; open file
		JR	J3087

; Subroutine S.BLTDM (blit ram to disk)
J307C:		PUSH	HL
		INC	HL
		INC	HL
		INC	HL
		INC	HL
		CALL	C32BB
		POP	HL
		LD	E,1			; create file
J3087:		PUSH	DE
		LD	C,(HL)
		INC	HL
		LD	B,(HL)
		INC	HL
		LD	E,(HL)
		INC	HL
		LD	D,(HL)
		EX	DE,HL
		AND	A
		SBC	HL,BC
		INC	HL
		POP	DE
		PUSH	DE
		CALL	C33C7
		POP	DE
		DEC	E
		PUSH	AF
		CALL	NZ,C3444
		POP	AF
		CALL	Z,C3433
		CALL	C3457
		AND	A
		RET

; Subroutine S.BLTVD (blit disk to vram)
J30A8:		PUSH	HL
		CALL	C32BB
		LD	HL,0
		LD	E,L			; open file
		CALL	C33C7
		CALL	C3444
		LD	(BUF+23),HL
		LD	DE,(BUF+19)
		POP	HL
		LD	(HL),E
		INC	HL
		LD	(HL),D
		DEC	HL
		CALL	C32C9
		JR	C,J30F3
		CALL	C2980			; wait for vdp command to complete
		LD	A,32			; register 32, autoincrement
		CALL	C336E			; set indirect vdp register pointer
		PUSH	BC
		LD	HL,BUF+4
		LD	BC,0E9BH
		OTIR
		POP	BC
		LD	A,(HL)
		AND	0FH
		OR	0B0H			; LMMC (logical move) command
		LD	HL,I30F2
		EX	(SP),HL
		PUSH	AF
		LD	A,(SCRMOD)
		SUB	6
		JR	C,J311C
		JP	Z,J315C
		DEC	A
		JR	Z,J311C
		JR	J30F9

I30F2:		AND	A
J30F3:		PUSH	AF
		CALL	C3457
		POP	AF
		RET

J30F9:		CALL	C336C			; set indirect vdp register pointer to 44, non-autoincrement
		CALL	C339D
		LD	A,(HL)
		OUT	(9BH),A
		POP	AF
		OUT	(99H),A
		LD	A,46+128
		OUT	(99H),A			; execute command
		EI
J310A:		DEC	BC
		LD	A,C
		OR	B
		RET	Z
		INC	HL
		CALL	C3279
		CALL	C339D
		JR	C,J310A
		LD	A,(HL)
		OUT	(9BH),A
		JR	J310A

J311C:		CALL	C336C			; set indirect vdp register pointer to 44, non-autoincrement
		LD	D,00H
		CALL	C339D
		LD	A,(HL)
		RRCA
		RRCA
		RRCA
		RRCA
		AND	0FH
		OUT	(9BH),A
		POP	AF
		OUT	(99H),A
		LD	A,46+128
		OUT	(99H),A			; execute command
		EI
J3135:		INC	D
		DEC	BC
		LD	A,C
		OR	B
		JR	NZ,J313E
		OR	E
		RET	Z
		DEC	E
J313E:		CALL	C3279
		LD	A,D
		RRCA
		LD	A,(HL)
		INC	HL
		JR	C,J314C
		DEC	HL
		RRCA
		RRCA
		RRCA
		RRCA
J314C:		PUSH	AF
		CALL	C339D
		JR	C,J3159
		POP	AF
		AND	0FH
		OUT	(9BH),A
		JR	J3135

J3159:		POP	AF
		JR	J3135

J315C:		CALL	C336C			; set indirect vdp register pointer to 44, non-autoincrement
		LD	D,00H
		CALL	C339D
		LD	A,(HL)
		RRCA
		RRCA
		AND	03H
		OUT	(9BH),A
		POP	AF
		OUT	(99H),A
		LD	A,46+128
		OUT	(99H),A			; execute command
		EI
J3173:		INC	D
		DEC	BC
		LD	A,C
		OR	B
		JR	NZ,J317C
		OR	E
		RET	Z
		DEC	E
J317C:		CALL	C3279
		LD	A,D
		AND	03H
		JR	Z,J3197
		DEC	A
		JR	Z,J319F
		DEC	A
		JR	Z,J319A
		CALL	C339D
		JR	C,J3194
		LD	A,(HL)
		AND	03H
		OUT	(9BH),A
J3194:		INC	HL
		JR	J3173

J3197:		LD	A,(HL)
		JR	J31A2

J319A:		LD	A,(HL)
		RRCA
		RRCA
		JR	J31A4

J319F:		LD	A,(HL)
		RLCA
		RLCA
J31A2:		RLCA
		RLCA
J31A4:		PUSH	AF
		CALL	C339D
		JR	C,J31B1
		POP	AF
		AND	03H
		OUT	(9BH),A
		JR	J3173

J31B1:		POP	AF
		JR	J3173

; Subroutine S.BLTDV (blit vram to disk)
J31B4:		PUSH	HL
		INC	HL
		INC	HL
		INC	HL
		INC	HL
		PUSH	HL
		CALL	C32BB
		LD	HL,0
		LD	E,1			; create file
		CALL	C33C7
		LD	DE,(BUF+19)
		POP	HL
		LD	(HL),E
		INC	HL
		LD	(HL),D
		POP	HL
		CALL	C3320
		LD	A,(SCRMOD)
		SUB	6
		JR	C,J31ED
		JR	Z,J3227
		DEC	A
		JR	Z,J31ED
J31DD:		CALL	C3292
		CALL	C3354
		LD	(HL),A
		INC	HL
		DEC	BC
		LD	A,C
		OR	B
		JR	NZ,J31DD
		JP	J3270

J31ED:		LD	D,00H
J31EF:		CALL	C3292
		LD	A,7
		DI
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister 7
		PUSH	HL
		POP	HL
		LD	A,D
		RRCA
		IN	A,(99H)
		JR	NC,J3208
		OR	(HL)
		LD	(HL),A
		INC	HL
		JR	J320D

J3208:		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
		LD	(HL),A
J320D:		XOR	A
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister 0
		EI
		INC	D
		DEC	BC
		LD	A,C
		OR	B
		JR	NZ,J31EF
		OR	E
		JR	Z,J3221
		DEC	E
		JR	J31EF

J3221:		LD	A,D
		RRCA
		JR	C,J3271
		JR	J3270

J3227:		LD	D,00H
J3229:		CALL	C3292
		LD	A,7
		DI
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister 7
		PUSH	HL
		POP	HL
		LD	A,D
		AND	03H
		JR	Z,J3249
		DEC	A
		JR	Z,J324F
		DEC	A
		IN	A,(99H)
		JR	Z,J3253
		OR	(HL)
		LD	(HL),A
		INC	HL
		JR	J3257

J3249:		IN	A,(99H)
		RRCA
		RRCA
		JR	J3256

J324F:		IN	A,(99H)
		RLCA
		RLCA
J3253:		RLCA
		RLCA
		OR	(HL)
J3256:		LD	(HL),A
J3257:		XOR	A
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister 0
		EI
		INC	D
		DEC	BC
		LD	A,C
		OR	B
		JR	NZ,J3229
		OR	E
		JR	Z,J326B
		DEC	E
		JR	J3229

J326B:		LD	A,D
		AND	03H
		JR	NZ,J3271
J3270:		DEC	HL
J3271:		CALL	C32B0
		CALL	C3457
		AND	A
		RET

; Subroutine __________________________



C3279:		PUSH	DE
		PUSH	HL
		LD	HL,(BUF+19)
		LD	DE,(BUF+23)
		ADD	HL,DE
		EX	DE,HL
		POP	HL
		RST	S_DCOMPR
		POP	DE
		RET	C
		PUSH	BC
		PUSH	DE
		CALL	C3444
		LD	(BUF+23),HL
		JR	J32AA

C3292:		PUSH	DE
		PUSH	HL
		LD	HL,(BUF+19)
		LD	DE,(BUF+21)
		ADD	HL,DE
		EX	DE,HL
		POP	HL
		RST	S_DCOMPR
		POP	DE
		RET	C
		LD	HL,(BUF+21)
		PUSH	BC
		PUSH	DE
		CALL	C3436
		AND	A
J32AA:		POP	DE
		POP	BC
		LD	HL,(BUF+19)
		RET

C32B0:		LD	DE,(BUF+19)
		AND	A
		SBC	HL,DE
		INC	HL
		JP	C3436

; Subroutine parse filespec and check if diskdrive
C32BB:		LD	A,(HL)
		INC	HL
		LD	H,(HL)
		LD	L,A
		CALL	C05BA			; evaluate filespecification
		LD	A,D
		CP	9
		RET	C
		JP	J052E			; bad filename error

C32C9:		LD	HL,(BUF+4)
		LD	C,(HL)
		INC	HL
		LD	B,(HL)
		INC	HL
		LD	E,(HL)
		INC	HL
		LD	D,(HL)
		INC	HL
		CALL	C3376
		RET	C
		PUSH	HL
		PUSH	BC
		PUSH	DE
		LD	HL,(BUF+8)
		LD	A,(BUF+17)
		AND	04H
		JR	NZ,J32ED
		EX	DE,HL
		CALL	C3390
		AND	A
		SBC	HL,DE
		DEFB	03EH
J32ED:		INC	HL
		LD	(BUF+66),HL
		LD	(BUF+68),HL
		LD	D,B
		LD	E,C
		EX	DE,HL
		SBC	HL,DE
		JR	NC,J32FE
		LD	HL,0
J32FE:		LD	(BUF+70),HL
		LD	(BUF+72),HL
		LD	A,H
		OR	L
		LD	(BUF+74),A
		POP	DE
		POP	BC
		LD	(BUF+12),BC
		LD	(BUF+14),DE
		CALL	C3052
		LD	B,H
		LD	C,L
		LD	A,00H
		ADC	A,A
		LD	E,A
		AND	A
		POP	HL
		EX	(SP),HL
		JP	(HL)

C3320:		CALL	C2980			; wait for vdp command to complete
		LD	A,32			; register 32, autoincrement
		CALL	C336E			; set indirect vdp register pointer
		LD	HL,BUF+4
		LD	BC,0E9BH
		OTIR
		LD	A,0A0H			; LMCM (logical move) command
		OUT	(C),A
		EI
		LD	BC,(BUF+12)
		LD	DE,(BUF+14)
		LD	HL,(BUF+8)
		LD	(HL),C
		INC	HL
		LD	(HL),B
		INC	HL
		LD	(HL),E
		INC	HL
		LD	(HL),D
		INC	HL
		PUSH	HL
		CALL	C3052
		LD	B,H
		LD	C,L
		LD	A,00H
		ADC	A,A
		LD	E,A
		POP	HL
		RET

; Subroutine read vdp colorregister (status register 7)
C3354:		DI
		LD	A,7
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister 7
		PUSH	HL
		POP	HL
		IN	A,(99H)
		PUSH	AF
		XOR	A
		OUT	(99H),A
		LD	A,15+128
		OUT	(99H),A			; select vdp statusregister 0
		POP	AF
		EI
		RET

; Subroutine set VDP controlregister to colorregister
C336C:		LD	A,44+128		; register 44, non-autoincrement

; Subroutine set VDP controlregister
C336E:		DI
		OUT	(99H),A
		LD	A,17+128
		OUT	(99H),A			; set indirect vdp register pointer
		RET

; Subroutine validate NX and NY
; Input:  BC=NX,DE=NY
C3376:		LD	A,B
		OR	C
		SCF
		RET	Z
		LD	A,D
		OR	E
		SCF
		RET	Z
		PUSH	HL
		PUSH	DE
		LD	E,C
		LD	D,B
		CALL	C3390
		RST	S_DCOMPR
		POP	DE
		POP	HL
		RET	C
		PUSH	HL
		LD	HL,212
		RST	S_DCOMPR
		POP	HL
		RET

; Subroutine get maximum X
C3390:		LD	HL,256
		LD	A,(SCRMOD)
		AND	06H
		CP	06H
		RET	NZ
		INC	H
		RET

C339D:		LD	A,(BUF+74)
		AND	A
		RET	Z

C33A2:		PUSH	HL
		LD	HL,(BUF+68)
		LD	A,L
		OR	H
		JR	Z,J33B0
		DEC	HL
		LD	(BUF+68),HL
		POP	HL
		RET

J33B0:		LD	HL,(BUF+72)
		DEC	HL
		LD	A,L
		OR	H
		JR	NZ,J33C1
		LD	HL,(BUF+66)
		LD	(BUF+68),HL
		LD	HL,(BUF+70)
J33C1:		LD	(BUF+72),HL
		POP	HL
		SCF
		RET

; Subroutine open file
; Input:  HL=size (0 specifies free basic space), BC=start
C33C7:		LD	A,(H_PHYD+0)		; disk system available ?
		CP	0C9H
		JP	Z,J0546			; nope, illegal function call error
		PUSH	BC
		PUSH	HL
		LD	HL,BUF+25
		PUSH	HL
		LD	(HL),D			; driveid
		LD	A,E
		AND	A
		PUSH	AF
		EX	DE,HL
		INC	DE
		LD	HL,FILNAM
		LD	BC,11
		LDIR				; filename
		XOR	A
		LD	B,37-11-1
J33E6:		LD	(DE),A
		INC	DE
		DJNZ	J33E6			; clear rest of the FCB
		POP	AF
		POP	DE
		PUSH	AF
		LD	C,0FH			; _FOPEN (open file)
		JR	Z,J33F3
		LD	C,16H			; _FMAKE (create file)
J33F3:		CALL	C345C			; BDOS
		INC	A			; error ?
		JR	NZ,J3401		; nop, continue
		POP	AF
		JP	Z,J055E			; file not found error
		LD	E,43H			; too many files error
		JR	J3454

J3401:		LD	HL,1
		LD	(BUF+39),HL
		POP	AF
		POP	HL
		POP	DE
		LD	A,H
		OR	L
		JR	NZ,J3428
		LD	HL,-512
		ADD	HL,SP
		JR	NC,J3421
		LD	DE,(STREND)
		AND	A
		SBC	HL,DE
		JR	C,J3421
		LD	A,H
		AND	A
		JR	NZ,J3428
J3421:		LD	DE,(NULBUF)
		LD	HL,256
J3428:		LD	(BUF+19),DE
		LD	(BUF+21),HL
		LD	C,1AH			; _SETDTA
		JR	C345C			; BDOS

; Subroutine write whole buffer to file
C3433:		LD	HL,(BUF+21)

; Subroutine write to file
; Input:  HL=size
C3436:		LD	DE,BUF+25
		LD	C,26H			; _WRBLK
		CALL	C345C			; BDOS
		AND	A
		RET	Z
		LD	E,42H			; disk full error
		JR	J3454

; Subroutine read from file
C3444:		LD	HL,(BUF+21)
		LD	DE,BUF+25
		LD	C,27H			; _RDBLK
		CALL	C345C			; BDOS
		LD	A,L
		OR	H
		RET	NZ
		LD	E,37H			; input past end error
J3454:		JP	J05C6			; BASIC error

; Subroutine close file
C3457:		LD	DE,BUF+25
		LD	C,10H			; _FCLOSE (close file)
						; BDOS

; Subroutine BDOS with mainrom invoked
C345C:		LD	IX,M7FFD
		JP	C0262


; ******************************************
; End Section Blit
; ******************************************


; ******************************************
; Begin Section Mouse/Trackball/Lightpen
; ******************************************

C3463:		LD	B,0
		AND	7FH
		LD	C,A
		SBC	HL,BC
		RET

; Subroutine read lightpen status
J346B:		IN	A,(0BAH)
		CPL
		AND	08H
		RET	Z
		LD	A,0FFH			; switch pressed
		RET

; Subroutine read lightpen
J3474:		LD	HL,XSAVE+1
		XOR	A			; assume invalid data
		BIT	7,(HL)
		RET	Z
		RES	7,(HL)
		IN	A,(0BAH)
		AND	07H
		LD	D,A
		IN	A,(0B8H)
		LD	L,A
		LD	H,00H
		IN	A,(0B9H)
		SRL	D
		RRA
		LD	E,A
		RL	H
		PUSH	HL
		PUSH	DE
		LD	BC,-3
		ADD	HL,BC
		EX	DE,HL
		LD	C,0EEH
		ADD	HL,BC
		LD	A,E
		OUT	(0B8H),A
		RR	D
		ADC	HL,HL
		LD	A,L
		OUT	(0B9H),A
		LD	A,H
		OUT	(0BAH),A
		POP	DE
		POP	HL
		IN	A,(0BAH)
		AND	20H
		RET	Z			; nope, quit with invalid data
		LD	A,(YSAVE+1)
		CALL	C3463
		LD	BC,212-1
		CALL	C34C7
		LD	A,L
		LD	(YSAVE),A
		EX	DE,HL
		LD	A,(XSAVE+1)
		CALL	C3463
		LD	BC,255

C34C7:		LD	A,H
		RLA
		JR	C,J34D4
		PUSH	HL
		SBC	HL,BC
		POP	HL
		JR	C,J34D6
		LD	L,C
		LD	H,B
		DEFB	0DAH
J34D4:		LD	L,0
J34D6:		LD	A,L
		LD	(XSAVE),A
		LD	A,0FFH			; flag valid data
		RET

; Subroutine S.NEWPAD
J34DD:		CP	8
		JR	Z,J3474			; yep, read lightpen (function 0)
		CP	11
		JR	Z,J346B			; yep, read lightpen status (function 3)
		LD	E,0FH
		LD	BC,000BFH
		LD	HL,010EFH
		CP	12
		JR	Z,J3509			; read mouse/trackball port A (function 0)
		LD	BC,040FFH
		LD	HL,020DFH
		CP	16
		JR	Z,J3509			; read mouse/trackball port B (function 0)
		AND	03H			; functionnumber
		SUB	2			; function 2 ?
		LD	A,(XSAVE)
		RET	M			; function 1, return X value
		LD	A,(YSAVE)
		RET	Z			; function 2, return Y value
		XOR	A			; function 3, return status (OK)
		RET

; Subroutine read mouse/trackball
J3509:		DI
		CALL	C357F			; pulse low
		CALL	C359A			; read 4 inputbits (used in case of a trackball ?)
		PUSH	AF
		CALL	C3585			; pulse high
		CALL	C3591			; wait ? and read 4 inputbits (DX7-DX4)
		PUSH	AF
		CALL	C357F			; pulse low
		CALL	C3597			; wait ? and read 4 inputbits (DX3-DX0)
		PUSH	AF
		CALL	C3585			; pulse high
		CALL	C3597			; wait ? and read 4 inputbits (DY7-DY4)
		PUSH	AF
		CALL	C357F			; pulse low
		CALL	C3597			; wait ? and read 4 inputbits (DY3-DY0)
		PUSH	AF

	IFNDEF FS5500
		CALL	C3585			; pulse high
	ELSE
		CALL	C3F8B
	ENDIF

		CALL	C3591			; wait ? and read 4 inputbits (DX7-DX4 for the second time)
		PUSH	AF
		CALL	C357F			; pulse low
		CALL	C35AA			; wait ? (DX3-DX0 are not read)
		CALL	C3585			; pulse high
		CALL	C35AA			; wait ? (DY7-DY4 are not read)
		CALL	C357F			; pulse low
		EI
		POP	AF
		POP	HL
		POP	DE
		POP	BC
		XOR	08H
		SUB	02H
		CP	0DH
		JR	C,J355D
		POP	AF
		CALL	C3577
		LD	(YSAVE),A
		POP	AF
		CALL	C3577
		JR	J3571

J355D:		LD	A,D
		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
		OR	H
		NEG
		LD	(YSAVE),A
		POP	AF
		POP	DE
		ADD	A,A
		ADD	A,A
		ADD	A,A
		ADD	A,A
		OR	B
		NEG
J3571:		LD	(XSAVE),A
		LD	A,0FFH
		RET

; Subroutine convert trackballdata to offset
C3577:		XOR	08H
		BIT	3,A
		RET	Z
		OR	0F0H
		RET

; Subroutine reset puls bit
C357F:		CALL	C35A2
		AND	L
		JR	J3589

; Subroutine set puls bit
C3585:		CALL	C35A2
		OR	H
J3589:		PUSH	AF
		LD	A,E
		OUT	(0A0H),A
		POP	AF
		OUT	(0A1H),A
		RET

; Subroutine wait until ticks has passed since counter reset and read 4 inputbits
C3591:		CALL	C35AA
		CALL	C35AA

C3597:		CALL	C35AA

C359A:		LD	A,0EH
		CALL	C35A3
		AND	0FH
		RET

; Subroutine read psg register with masks
; Input:  E = register, C=AND mask, B=OR mask
C35A2:		LD	A,E

; Subroutine read psg register with masks
; Input:  A = register, C=AND mask, B=OR mask
C35A3:		OUT	(0A0H),A
		IN	A,(0A2H)
		AND	C
		OR	B
		RET

C35AA:
	IFNDEF	FS5500
		EX	(SP),HL
		EX	(SP),HL
		EX	(SP),HL
		EX	(SP),HL
	ELSE
		NOP
		NOP
		NOP
		NOP
	ENDIF
		RET


; ******************************************
; End Section Mouse/Trackball/Lightpen
; ******************************************


; Subroutine SET VIDEO statement
C35AF:		CP	','
		JR	Z,J35EF
		RST	S_GETBYT		; evaluate byte operand
		CP	4
		JR	NC,J3635
		AND	A
		JR	Z,J35C2
		CP	1
		JR	Z,J35C2
		LD	E,0
		DEC	A
J35C2:		PUSH	DE
		RRCA
		RRCA
		LD	D,A
		RRCA
		RRCA
		LD	E,A
		LD	A,(RG9SAV)
		AND	0CFH
		OR	E
		LD	C,9
		CALL	C36B4			; write vdp register 9
		LD	A,D
		XOR	0C0H
		LD	E,A			; b7,b6
		LD	A,3FH			; leave b5-b0
		CALL	C3661
		POP	DE
		LD	A,E
		RRCA
		RRCA
		RRCA
		LD	E,A
		LD	A,(RG8SAV)
		AND	0DFH
		OR	E
		CALL	C36B2			; write vdp register 8
		DEC	HL
		RST	S_CHRGTR
		RET	Z
J35EF:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J3600
		CALL	C366C			; evaluate intensity parameter
		LD	A,0DFH			; leave b7,b6, b4-b0
		CALL	C3661
		DEC	HL
		RST	S_CHRGTR
		RET	Z
J3600:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J3617
		CALL	C366C			; evaluate color direction parameter
		RRC	E
		LD	A,(RG8SAV)
		AND	0EFH
		OR	E
		CALL	C36B2			; write vdp register 8
		DEC	HL
		RST	S_CHRGTR
		RET	Z
J3617:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J362C
		CALL	C366C			; evaluate synchronization parameter
		RLCA
		RLCA
		RLCA
		LD	E,A			; b3
		LD	A,0F7H			; leave b7-b4, b2-b0
		CALL	C3661
		DEC	HL
		RST	S_CHRGTR
		RET	Z
J362C:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J3644

	IF CNTRY = 0
		RST	S_GETBYT		; evaluate byte operand
		CP	4
	ELSE
		CALL	C3CCD
	ENDIF

J3635:		JP	NC,J0546		; illegal function call error
		CPL
		AND	03H
		LD	E,A			; b1, b0
		LD	A,0FCH			; leave b7-b2
		CALL	C3661
		DEC	HL
		RST	S_CHRGTR
		RET	Z
J3644:		RST	S_SYNCHR
		DEFB	','
		CP	','
		JR	Z,J3658
		CALL	C366C			; evaluate external videoinput parameter
		RLCA
		RLCA
		LD	E,A			; b2
		LD	A,0FBH			; leave b7-b3, b1-b0
		CALL	C3661
		DEC	HL
		RST	S_CHRGTR
		RET	Z
J3658:		RST	S_SYNCHR
		DEFB	','
		CALL	C366C			; evaluate external videooutput parameter
		RRC	E			; b4
		LD	A,0EFH			; leave b7-b5, b3-b0

; Subroutine update digitizehardware setting
; Input:  A=AND mask,E=OR mask
C3661:		PUSH	HL
		LD	HL,AVCSAV
		AND	(HL)
		OR	E
		LD	(HL),A
		OUT	(0F7H),A
		POP	HL
		RET

; Subroutine evaluate on/off parameter
; Output: Zx set and A=0,E=0 when off, Zx reset and A=1,E=020H when on
C366C:		RST	S_GETBYT		; evaluate byte operand
		CP	1+1
		JR	NC,J3635
		AND	A
		RET	Z
		LD	E,20H
		RET

; Subroutine COPY SCREEN statement
J3676:		RST	S_CHRGTR
		JR	Z,J3686
		CP	','
		JR	Z,J3686
		CALL	C366C
		DEC	A
		JR	NZ,J3686
		LD	E,0
		DEFB	1
J3686:		LD	E,2
		DEC	HL
		RST	S_CHRGTR
		LD	A,0FFH
		JR	Z,J3693
		PUSH	DE
		RST	S_SYNCHR
		DEFB	','
		RST	S_GETBYT		; evaluate byte operand
		POP	DE
J3693:		LD	D,A
		LD	A,(RG7SAV)
		PUSH	AF			; save vdp register 7
		LD	A,D
		CALL	C36AE			; write vdp register 7
		CALL	C36B8
		OR	40H
		CALL	C36B4			; write vdp register 0
		CALL	C36B8
		EI
		AND	0BFH
		CALL	C36B4			; write vdp register 0
		POP	AF
						; restore vdp register 7 and quit

; Subroutine write vdp register 7
C36AE:		LD	C,7
		JR	C36B4			; write vdp register 7


; Subroutine write vdp register 8
C36B2:		LD	C,8

; Subroutine write vdp register
C36B4:		LD	B,A
		JP	C0647			; write vdp register

C36B8:		LD	A,2
		CALL	C298B			; read vdp status register 2
		AND	40H			; in VR ?
		JR	Z,C36B8			; nope, wait
J36C1:		LD	A,2
		CALL	C298B			; read vdp status register 2
		BIT	6,A			; VR passed ?
		JR	NZ,J36C1		; nope, wait
		OR	E
		AND	02H
		JR	Z,C36B8
		LD	A,(RG0SAV)
		LD	C,0			; vdp register 0
		RET

; ******************************************
; Begin Section memory disk
; ******************************************

; memory disk uses 32Kb RAM at 00000H-7FFFH, which is normaly unusable in MSX-Basic
; 0000-0001	size used by memory disk (also start of the unused area)
; 0002-007F	not used
; 0080-00FF	"FAT", 128 entries of 1 byte
; 0100-02FF	"DIR", 32 entries of 16 bytes
; 0300-		"DATA", clusters of 256 bytes

; DIRECTORY ENTRY FORMAT
; +0	filename (ascii)
; +11	start cluster
; +12	end cluster
; +13	file open flag
; +14	filesize

; Subroutine BASIC DEVICE handler
C36D5:		EI
		INC	A			; device inquiry ?
		JR	NZ,J36DF		; nope, handle MEM device operation
J36D9:		CALL	C392C			; check if MEM or MEM0 devicename
		RET	Z			; yep, quit (devicename recognized)
		SCF
		RET				; flag devicename not recognized

J36DF:		PUSH	HL
		LD	HL,I36EE-1
		ADD	A,L
		LD	L,A
		JR	NC,J36E8
		INC	H
J36E8:		LD	A,(HL)
		INC	HL
		LD	H,(HL)
		LD	L,A
		EX	(SP),HL
		RET

I36EE:		DEFW	C3973			; open (mem device)
		DEFW	C39FA			; close (mem device)
		DEFW	C38B9			; random i/o (mem device), bad file mode error
		DEFW	C3A1E			; sequential output (mem device)
		DEFW	C3A7C			; sequential input (mem device)
		DEFW	C3898			; loc (mem device)
		DEFW	C38A3			; lof (mem device)
		DEFW	C38B6			; eof (mem device)
		DEFW	C38D7			; fpos (mem device)
		DEFW	C3AC8			; putback (mem device)

; Subroutine CALL MEMINI statement
C3702:		DEC	HL
		RST	S_CHRGTR		; end of statement ?
		LD	DE,07FFFH
		JR	Z,J3714			; yep, use 32767
		RST	S_SYNCHR
		DEFB	"("			; check for (
		CALL	C0608			; evaluate integer operand
		JP	M,J0546			; >32767, illegal function call error
		RST	S_SYNCHR
		DEFB	")"			; check for )
		RET	NZ			; not end of statement, quit (which generates syntax error)
J3714:		PUSH	HL
		PUSH	DE
		LD	A,(SLTWRK+0)
		AND	A			; memory disk already used ?
		JR	Z,J3722			; nope, search RAM
		BIT	5,A			; memory disk disabled ?
		JR	NZ,J3777		; yep, illegal function call error
		JR	J377A			; skip search RAM

J3722:		LD	HL,EXPTBL
		LD	BC,4*256+0		; four primary slots, start at 00XXH
		XOR	A			; begin at primary slot 0
J3729:		AND	03H
		OR	(HL)			; if slot expanded, start at secondary slot 0
J372C:		PUSH	BC
		PUSH	HL
		LD	H,C			; high byte of memory to check
J372F:		LD	L,10H			; low byte memory to check
J3731:		PUSH	AF
		CALL	C057D			; read from slot
		CPL
		LD	E,A			; inverted value
		POP	AF
		PUSH	DE
		PUSH	AF
		CALL	C0583			; write to slot
		POP	AF
		POP	DE
		PUSH	AF
		PUSH	DE
		CALL	C057D			; read from slot
		POP	BC
		LD	B,A
		LD	A,C
		CPL
		LD	E,A			; orginal value
		POP	AF
		PUSH	AF
		PUSH	BC
		CALL	C0583			; write to slot
		POP	BC
		LD	A,C
		CP	B			; is memory location writeable ?
		JR	NZ,J3766		; nope, try next slot
		POP	AF
		DEC	L			; checked XX01H-XX10H ?
		JR	NZ,J3731		; nope, continue
		INC	H
		INC	H
		INC	H
		INC	H			; next Kbyte
		JP	P,J372F			; not checked all lower 32KB RAM, continue
		POP	HL
		POP	HL			; cleanup stack
		LD	(SLTWRK+0),A		; slotid RAM, ramdisk not initialized, ramdisk enabled
		JR	J377A

J3766:		POP	AF
		POP	HL
		POP	BC
		AND	A			; slot expanded ?
		JP	P,J3773			; nope, next primary slot
		ADD	A,4			; next secondary slot
		CP	90H			; all secondary slots checked ?
		JR	C,J372C			; nope, next secondary slot
J3773:		INC	HL
		INC	A
		DJNZ	J3729			; next primary slot
						; no RAM found

J3777:		JP	J0546			; illegal function call error

J377A:		BIT	6,A			; memory disk initialized ?
		JR	Z,J3795			; nope, skip open file check
		LD	IX,0100H
		LD	B,32			; 32 directory entries
J3784:		CALL	C3AD8			; read directory entry
		LD	A,(LINWRK+13)
		AND	A			; file open ?
		JP	NZ,J3BF9		; yep, file still open error
		LD	DE,16
		ADD	IX,DE
		DJNZ	J3784			; next entry
J3795:		POP	HL
		XOR	A
		LD	DE,1023
		SBC	HL,DE
		JR	C,J3804			; ramdisk size <1023, no ramdisk
		INC	H			; round up
		PUSH	HL
		LD	B,128			; 128 FAT entries
		LD	C,A			; FAT entry 0
		DEC	A			; 0FFH (end of FAT chain) = cluster allocated
J37A4:		INC	C
		CALL	C3AEC			; update FAT entry content
		DJNZ	J37A4			; next entry
		LD	B,H			; number of clusters
		XOR	A			; 000H (cluster free)
		LD	C,A			; FAT entry 0
J37AD:		INC	C
		CALL	C3AEC			; update FAT entry content
		DJNZ	J37AD			; next entry
		LD	HL,LINWRK
		LD	DE,LINWRK+1
		LD	C,16-1
		LD	(HL),B
		LDIR				; clear directory entry
		LD	IX,0100H
		LD	B,32			; 32 directory entries
J37C4:		CALL	C3AD9			; write directory entry
		LD	DE,16
		ADD	IX,DE
		DJNZ	J37C4			; next entry
		LD	HL,SLTWRK+0
		SET	6,(HL)			; memory disk initialized
		POP	HL
		LD	L,B
		PUSH	HL
		CALL	C05CC			; print number
		CALL	C388F			; string to interpreter output (string direct after call)
		DEFB	" bytes allocated",13,10
		DEFB	0
		POP	HL
		INC	H
		INC	H
		INC	H
		LD	(LINWRK+0),HL		; size ?
		LD	HL,LINWRK
		LD	DE,0000H
		LD	BC,2
		CALL	C3B6A			; transfer from/to memory disk
		POP	HL
		RET

J3804:		XOR	A
		LD	(SLTWRK+0),A		; memory disk not initialized, enabled, search RAM
		CALL	C388F			; string to interpreter output (string direct after call)
		DEFB	"No RAM disk",13,10
		DEFB	0
		POP	HL
		RET

; Subroutine CALL MFILES statement
C381B:		CALL	C3BFD			; check if memory disk initialized
		CALL	C05AE			; fresh line to interpreter output
		PUSH	HL
		LD	IX,0100H
		LD	BC,32*256+0		; 32 directory entries, 0 files found
J3829:		CALL	C3AD8			; read directory entry
		LD	HL,LINWRK
		LD	A,(HL)
		AND	A			; entry unused ?
		JR	Z,J3861			; yep, skip
		INC	C			; increase files found
		PUSH	IX
		PUSH	BC
		LD	B,8
		CALL	C3889			; string to interpreter output
		LD	A,(HL)
		CP	' '			; file has extension ?
		JR	Z,J3843
		LD	A,'.'			; yep, print '.'
J3843:		RST	S_OUTDO
		LD	B,3
		CALL	C3889			; string to interpreter output
		LD	A,(LINLEN)
		LD	B,A			; screen width
		LD	A,(TTYPOS)
		AND	A			; at start of line ?
		JR	Z,J385E			; yep, skip does an other fit
		ADD	A,8+3+1
		CP	B			; does an other fit ?
		JR	NC,J385B
		LD	A,' '
		RST	S_OUTDO			; yep, space to interpreter
J385B:		CALL	NC,C05AE		; nope, fresh line to interpreter output
J385E:		POP	BC
		POP	IX
J3861:		LD	DE,16
		ADD	IX,DE
		DJNZ	J3829			; next entry
		LD	A,C
		OR	A
		JP	Z,J055E			; file not found error
		CALL	C05AE			; fresh line to interpreter output
		CALL	C38D7			; fpos (mem device)
		CALL	C05CC			; print number
		CALL	C388F			; string to interpreter output (string direct after call)
		DEFB	" bytes free",13,10
		DEFB	0
		POP	HL
		RET

; Subroutine string to interpreter output
C3889:		LD	A,(HL)
		INC	HL
		RST	S_OUTDO
		DJNZ	C3889
		RET


; Subroutine string to interpreter output (string direct after call)
C388F:		EX	(SP),HL
		LD	A,(HL)
		INC	HL
		EX	(SP),HL
		AND	A
		RET	Z
		RST	S_OUTDO
		JR	C388F

; Subroutine loc (mem device)
C3898:		PUSH	HL
		POP	IY
		LD	H,(IY+5)		; relative cluster
		LD	L,(IY+6)		; position in buffer
		JR	J38E6			; return current position in file in DAC

; Subroutine lof (mem device)
C38A3:		CALL	C3ACF			; read directory entry of I/O channel
		PUSH	HL
		POP	IY
		LD	A,(HL)			; file mode
		CP	1			; sequential input ?
		LD	HL,(LINWRK+14)
		JR	Z,J38E6			; yep, return file size in DAC
		LD	L,(IY+6)		; current file size + position in buffer
		JR	J38E6			; return in DAC

; Subroutine eof (mem device)
C38B6:		LD	A,(HL)			; file mode
		CP	1			; sequential input ?
C38B9:		LD	E,3DH
		JP	NZ,J3C05		; nope, bad file mode error
		PUSH	HL
		POP	IY
		CALL	C3ACF			; read directory entry of I/O channel
		LD	L,(IY+6)		; position in buffer
		LD	H,(IY+5)		; relative cluster
		LD	DE,(LINWRK+14)		; file size
		RST	S_DCOMPR
		LD	HL,-1
		JR	NC,J38E6		; at end or past end, return -1 in DAC
		INC	HL
		JR	J38E6			; return 0 in DAC


; Subroutine fpos (mem device)
C38D7:		LD	BC,125*256+1		; 125 FAT entries, start at 1
		LD	HL,0
J38DD:		CALL	C3AEB			; get FAT entry content
		JR	NZ,J38E3		; entry used, next
		INC	H			; entry unused, +256
J38E3:		INC	C
		DJNZ	J38DD			; next entry
						; return number of free clusters*256 = free space in bytes

J38E6:		LD	(DAC+2),HL
		LD	A,2
		LD	(VALTYP),A		; integer type
		RET

; Subroutine CALL MKILL statement
C38EF:		CALL	C3911			; check for ( , evaluate filespecification and check for memory disk device
		RST	S_SYNCHR
		DEFB	")"			; check for )
		CALL	C3BF1			; check for filename and check if not open

; Subroutine delete file
C38F7:		LD	(LINWRK+0),A		; directory entry not used
		CALL	C3AD9			; write directory entry
		LD	A,(LINWRK+11)
		AND	A			; file has a starting cluster ?
		RET	Z			; nope, quit
J3902:		LD	C,A
		CALL	C3AEB			; get FAT entry content
		PUSH	AF
		XOR	A			; free cluster
		CALL	C3AEC			; update FAT entry content
		POP	AF
		CP	0FFH			; end of FAT chain ?
		JR	NZ,J3902		; nope, next in FAT chain
		RET


; Subroutine check for ( , evaluate filespecification and check for memory disk device
C3911:		RST	S_SYNCHR
		DEFB	"("			; check for (
		CALL	C3BFD			; check if memory disk initialized

; Subroutine evaluate filespecification and check for memory disk device
C3916:		CALL	C05BA			; evaluate filespecification
		RET	Z
		LD	A,D
		CP	0FCH			; internal device ?
		JR	NC,J3929		; yep, bad filename error
		CP	8+1			; diskdrive device ?
		JR	C,J3929			; yep, bad filename error
		PUSH	HL
		CALL	C392C			; check if MEM or MEM0 devicename
		POP	HL
		RET	Z			; yep, quit
J3929:		JP	J052E			; bad filename error


; Subroutine check if MEM or MEM0 devicename
C392C:		LD	HL,PROCNM
		LD	DE,I3941
J3932:		LD	A,(DE)
		CP	(HL)
		JR	NZ,J393C
		INC	HL
		INC	DE
		OR	A
		JR	NZ,J3932
		RET

J393C:		SUB	'0'
		RET	NZ
		OR	(HL)
		RET

I3941:		DEFB	"MEM0"
		DEFB	0

; Subroutine CALL MNAME statement
C3946:		CALL	C3911			; check for ( , evaluate filespecification and check for memory disk device
		CALL	C3BF1			; check for filename and check if not open
		PUSH	IX
		RST	S_SYNCHR
		DEFB	"A"
		RST	S_SYNCHR
		DEFB	"S"			; check for AS
		CALL	C3916			; evaluate filespecification and check for memory disk device
		RST	S_SYNCHR
		DEFB	")"			; check for )
		CALL	C3B0D			; find directory entry
		LD	E,41H
		JP	Z,J3C05			; found, file already exists error
		POP	IX
		CALL	C3AD8			; read directory entry
J3964:		PUSH	HL
		LD	DE,LINWRK
		LD	HL,FILNAM
		LD	BC,11
		LDIR				; change filename
		POP	HL
		JR	J3995			; write directory entry and quit

; Subroutine open (mem device)
C3973:		CALL	C3BFD			; check if memory disk initialized
		CALL	C3B0D			; find directory entry
		PUSH	AF
		LD	A,E			; file mode
		CP	4			; random i/o ?
		JP	Z,J0534			; yep, sequential i/o only error
		CP	2			; sequential output ?
		JR	Z,J3998			; yep, handle
		CP	8			; sequential append ?
		JR	Z,J39D1			; yep, handle

		; open for sequential input
		POP	AF
		JR	NZ,J39D2		; sequential input and file not found, file not found error
		OR	A			; file open ?
		JR	NZ,J39D6		; yep, file already open error
		CALL	C3BD6			; initialize I/O channel

; Subroutine flag file is open, write directory entry
C3991:		DEC	A
		LD	(LINWRK+13),A		; set file open flag
J3995:		JP	C3AD9			; write directory entry

		; open for sequential output
J3998:		POP	AF
		JR	NZ,J39A1		; file not found, skip delete
		OR	A			; file open ?
		JR	NZ,J39D6		; yep, file already open error
		CALL	C38F7			; delete file
J39A1:		LD	IX,0100H
		LD	B,32			; 32 directory entries
J39A7:		CALL	C3AD8			; read directory entry
		LD	A,(LINWRK+0)
		AND	A			; entry free ?
		JR	Z,J39BC			; yep, use it
		LD	DE,16
		ADD	IX,DE
		DJNZ	J39A7			; next entry
		LD	E,43H
		JP	J3C05			; too many files error

J39BC:		LD	E,2			; sequential output
		CALL	C3BD6			; initialize I/O channel
		LD	HL,LINWRK
		LD	B,16
J39C6:		LD	(HL),A
		INC	HL
		DJNZ	J39C6			; clear directory entry
		DEC	A
		LD	(LINWRK+13),A		; set file open flag
		JP	J3964			; copy filename to directory entry, write directory entry and quit

		; open for sequential append
J39D1:		POP	AF
J39D2:		JP	NZ,J055E		; file not found, file not found error
		OR	A			; file open ?
J39D6:		JP	NZ,J053A		; yep, file already open error
		CALL	C3BD6			; initialize I/O channel
		LD	HL,(LINWRK+14)		; file size
		LD	(IY+5),H		; relative cluster
		LD	(IY+6),L		; position in buffer
		LD	L,A
		LD	(LINWRK+14),HL
		CALL	C3991			; flag file is open, write directory entry
		LD	A,(LINWRK+11)
		AND	A			; file has a starting cluster ?
		RET	Z			; nope, quit
		LD	A,(LINWRK+12)		; last cluster of file
		LD	(IY+7),A		; current cluster
		JP	C3B51			; read cluster from memory disk and quit

; Subroutine close (mem device)
C39FA:		CALL	C3ACF			; read directory entry of I/O channel
		XOR	A
		LD	(LINWRK+13),A		; reset flag file open
		CALL	C3AD9			; write directory entry
		LD	A,(HL)			; file mode
		CP	1			; sequential input ?
		RET	Z			; yep, quit
		PUSH	HL
		POP	IY
		LD	A,(IY+6)
		AND	A			; buffer empty ?
		RET	Z			; yep, quit
		CALL	C3B52			; write cluster to memory disk
		LD	HL,(LINWRK+14)		; file size
		LD	L,(IY+6)		; position in buffer
		LD	(LINWRK+14),HL		; update file size
		JR	J3A7A			; write directory entry and quit

; Subroutine sequential output (mem device)
C3A1E:		PUSH	HL
		POP	IY
		LD	A,(IY+6)
		AND	A			; buffer full ?
		JR	NZ,J3A5D		; nope, put in buffer
		PUSH	HL
		PUSH	BC
		CALL	C3ACF			; read directory entry of I/O channel
		LD	BC,125*256+1		; 125 FAT entries, start at 1
J3A2F:		CALL	C3AEB			; get FAT entry content
		JR	Z,J3A3C			; free FAT entry, use it
		INC	C
		DJNZ	J3A2F			; next entry
		LD	E,42H
		JP	J3C05			; ramdisk full error

J3A3C:		LD	A,(LINWRK+11)
		AND	A			; file has a starting cluster ?
		LD	A,C
		JR	NZ,J3A46		; yep, skip
		LD	(LINWRK+11),A		; nope, now it has
J3A46:		LD	C,(IY+7)		; current cluster
		CALL	NZ,C3AEC		; file had a starting cluster, update FAT entry content (extend FAT chain)
		LD	(LINWRK+12),A		; update last cluster of file
		LD	(IY+7),A		; new current cluster
		LD	C,A
		LD	A,0FFH			; end of FAT chain
		CALL	C3AEC			; update FAT entry content
		CALL	C3AD9			; write directory entry
		POP	BC
		POP	HL
J3A5D:		LD	DE,9
		ADD	HL,DE			; I/O channel buffer
		LD	E,(IY+6)		; position in buffer (relative)
		ADD	HL,DE
		LD	(HL),C			; put in buffer
		INC	(IY+6)			; update position in buffer
		RET	NZ			; buffer not full, quit
		CALL	C3B52			; write cluster to memory disk
		INC	(IY+5)			; update relative cluster
		PUSH	IY
		POP	HL
		CALL	C3ACF			; read directory entry of I/O channel
		LD	HL,LINWRK+14+1
		INC	(HL)			; update file size
J3A7A:		JR	C3AD9			; write directory entry

; Subroutine sequential input (mem device)
C3A7C:		PUSH	HL
		PUSH	HL
		POP	IY
		CALL	C3ACF			; read directory entry of I/O channel
		LD	L,(IY+6)		; position in buffer
		LD	H,(IY+5)		; relative cluster
		LD	DE,(LINWRK+14)		; file size
		RST	S_DCOMPR
		POP	HL
		CCF
		RET	C			; at end of file or past end of file, quit with error
		LD	A,(IY+3)
		AND	A			; backup character ?
		LD	(IY+3),0		; not anymore
		RET	NZ			; yep, return backup character
		LD	A,(IY+6)
		AND	A			; buffer empty ?
		JR	Z,J3AAF			; yep, fill buffer
		LD	BC,9
		ADD	HL,BC			; I/O channel buffer
		LD	C,A
		ADD	HL,BC
		LD	A,(HL)			; get from buffer
		INC	(IY+6)			; update position in buffer
		RET	NZ			; not end of buffer, quit
		INC	(IY+5)			; next relative cluster
		RET

J3AAF:		LD	A,(IY+7)
		AND	A			; has a current cluster ?
		LD	C,A
		LD	A,(LINWRK+11)		; file starting cluster
		CALL	NZ,C3AEB		; yep, get FAT entry content
		LD	(IY+7),A		; update current cluster
		CALL	C3B51			; read cluster from memory disk
		LD	A,(IY+9)		; get from buffer
		INC	(IY+6)			; update position in buffer
		AND	A
		RET

; Subroutine putback (mem device)
C3AC8:		PUSH	HL
		POP	IY
		LD	(IY+3),C		; backup character
		RET

; Subroutine read directory entry of I/O channel
; Input:  HL = I/O channel
C3ACF:		INC	HL
		LD	C,(HL)
		INC	HL
		LD	B,(HL)			; directory entry pointer
		PUSH	BC
		POP	IX
		DEC	HL
		DEC	HL

; Subroutine read directory entry
; Input:  IX = directory entry
; Output: LINWRK
C3AD8:		DEFB	0F6H			; OR xx, skip next instruction

; Subroutine write directory entry
; Input:  IX = directory entry, LINWRK
C3AD9:		SCF
		PUSH	HL
		PUSH	DE
		PUSH	BC
		PUSH	IX
		POP	HL
		LD	DE,LINWRK
		JR	NC,J3AE6
		EX	DE,HL
J3AE6:		LD	BC,16
		JR	J3B02

; Subroutine get FAT entry content
; Input:  C = FAT entry number (1 based)
; Output: A = content
C3AEB:		DEFB	0F6H			; OR xx, skip next instruction

; Subroutine update FAT entry content
; Input:  C = FAT entry, A = content
C3AEC:		SCF
		PUSH	HL
		PUSH	DE
		PUSH	BC
		PUSH	AF
		LD	HL,0080H-1
		LD	B,0
		ADD	HL,BC
		LD	DE,LINWRK+16
		POP	AF
		JR	NC,J3AFF
		EX	DE,HL
		LD	(HL),A
J3AFF:		LD	BC,1
J3B02:		CALL	C3B6A			; transfer from/to memory disk
		POP	BC
		POP	DE
		POP	HL
		LD	A,(LINWRK+16)
		AND	A
		RET

; Subroutine find directory entry
; Input:  FILNAM = filename
; Output: Zx set if found, A = file open flag
C3B0D:		PUSH	HL
		PUSH	DE
		LD	DE,FILNAM
		LD	B,8+3
J3B14:		LD	A,(DE)
		CP	'a'
		JR	C,J3B20
		CP	'z'+1
		JR	NC,J3B20
		AND	0DFH			; upcase
		LD	(DE),A
J3B20:		INC	DE
		DJNZ	J3B14
		LD	IX,0100H
		LD	C,32			; 32 directory entries
J3B29:		CALL	C3AD8			; read directory entry
		LD	HL,LINWRK
		LD	A,(HL)
		AND	A			; entry unused ?
		JR	NZ,J3B3E		; nope, check filename
J3B33:		LD	DE,16
		ADD	IX,DE
		DEC	C
		JR	NZ,J3B29		; next entry
		INC	C			; Zx reset (not found)
		JR	J3B4B

J3B3E:		LD	DE,FILNAM
		LD	B,8+3
J3B43:		LD	A,(DE)
		CP	(HL)
		JR	NZ,J3B33		; filename not equal, next entry
		INC	DE
		INC	HL
		DJNZ	J3B43
J3B4B:		POP	DE
		POP	HL
		LD	A,(LINWRK+13)		; file open flag
		RET

; Subroutine read cluster from memory disk
; Input:  IY = I/O channel pointer, (IY+7) = cluster
C3B51:		DEFB	0F6H			; OR xx, skip next instruction

; Subroutine write cluster to memory disk
; Input:  IY = I/O channel pointer, (IY+7) = cluster
C3B52:		SCF
		PUSH	AF
		PUSH	IY
		POP	HL
		LD	BC,9
		ADD	HL,BC			; I/O channel buffer
		LD	E,00H			; LOW 0300H
		LD	A,(IY+7)		; current cluster
		ADD	A,02H			; (HIGH 0300H)-1
		LD	D,A
		POP	AF
		JR	C,J3B67
		EX	DE,HL
J3B67:		LD	BC,256

; Subroutine transfer from/to memory disk
; Input:  HL = source address, DE = destination address, BC = size
C3B6A:		LD	A,H
		AND	A			; is source memory disk ?
		JP	P,J3B70			; yep,
		LD	A,D
J3B70:		CP	40H			; to or from page 0 ?
		EXX
		LD	HL,D_FFFF
		LD	A,(SLTWRK+0)
		LD	D,A			; slotid ram of memory disk
		JR	C,J3BB7			; yep, use helper routine in mainrom
		RRCA
		RRCA
		AND	0C0H
		LD	C,A			; OR mask page 3 primary slot
		RRCA
		RRCA
		RRCA
		RRCA
		OR	C
		LD	C,A			; OR mask page 3 and 1 primary slot
		IN	A,(0A8H)
		LD	B,A
		AND	33H			; clear page 3 and 1
		OR	C
		LD	E,A
		DI
		OUT	(0A8H),A		; select primary slot memory disk RAM in page 3 and 1
		LD	A,D
		AND	0CH
		LD	D,A			; secondary slot
		LD	A,(HL)
		CPL
		LD	C,A
		AND	0F3H			; clear page 1
		OR	D
		LD	(HL),A			; select secondary slot page 1
		LD	A,E
		AND	3FH			; clear page 3
		LD	D,A
		LD	A,B			; saved primary slot register
		AND	0C0H			; clear page 2,1 and 0
		OR	D
		EI
		OUT	(0A8H),A		; select primary slot memory disk RAM in page 1
		EXX
		LDIR				; copy data
		EXX
		LD	A,E
		DI
		OUT	(0A8H),A		; select primary slot memory disk RAM in page 3 and 1
		LD	A,C
		LD	(HL),A			; restore secondary slot
		LD	A,B
		EI
		OUT	(0A8H),A		; restore primary slot
		EXX
		RET

J3BB7:		RRCA
		RRCA
		AND	0C0H			; OR mask page 3 primary slot
		LD	C,A
		LD	A,D
		AND	03H			; primary slot memory disk RAM
		OR	C			; OR mask page 3 and 0 primary slot memory disk RAM
		LD	C,A
		IN	A,(0A8H)
		LD	B,A			; save primary slot register
		AND	3CH			; clear page 3 and 0
		OR	C
		LD	E,A
		LD	A,D
		RRCA
		RRCA
		AND	03H			; secondary slot memory disk RAM
		LD	D,A
		LD	A,E
		DI
		CALL	M51D7			; transfer via helper routine in main rom
		EI
		EXX
		RET

; Subroutine initialize I/O channel
; Input:  E = file mode, HL = I/O channel pointer, IX = directory entry pointer
C3BD6:		PUSH	HL
		POP	IY
		PUSH	IX
		POP	BC
		LD	(IY+1),C
		LD	(IY+2),B		; directory entry pointer
		XOR	A
		LD	(IY+3),A		; no backup character
		LD	(IY+5),A		; relative cluster = 0
		LD	(IY+6),A		; position in buffer = 0
		LD	(PTRFIL),HL		; redirect BASIC interpreter I/O to I/O channel
		LD	(HL),E			; save file open mode
		RET

; Subroutine check for filename and check if not open
C3BF1:		CALL	C3B0D			; find directory entry
		JP	NZ,J055E		; not found, file not found error
		AND	A			; file open ?
		RET	Z			; nope, quit

J3BF9:		LD	E,40H
		JR	J3C05			; file still open error

; Subroutine check if memory disk initialized
C3BFD:		LD	A,(SLTWRK+0)
		BIT	6,A			; memory disk initialized ?
		RET	NZ			; yep, quit
		LD	E,46H			; ramdisk offline error

J3C05:		XOR	A
		LD	(NLONLY),A		; reset loading BASIC program, leave I/O channels open flag
		PUSH	DE
		CALL	C05C0			; close I/O channel 0
		POP	DE
		JP	J05C6			; BASIC error

; ******************************************
; End Section Ramdisk
; ******************************************

; Subroutine extended errorhandler
; Input:  HL = errortext pointer, E=errorcode
; Output: HL = errortext pointer, E=errorcode

I3C11:		LD	BC,I3C34
J3C14:		LD	A,(BC)
		INC	BC
		OR	A
		RET	Z
		SUB	E
		JR	Z,J3C1F
		INC	BC
		INC	BC
		JR	J3C14

J3C1F:		LD	L,C
		LD	H,B
		LD	E,(HL)
		INC	HL
		LD	D,(HL)
		LD	HL,BUF
		PUSH	HL
		LD	(HL),A
J3C29:		INC	DE
		INC	HL
		LD	A,(DE)
		LD	(HL),A
		OR	A
		JR	NZ,J3C29
		POP	HL
		LD	E,1
		RET

I3C34:		DEFB	03DH
		DEFW	I3C47-1
		DEFB	040H
		DEFW	I3C55-1
		DEFB	041H
		DEFW	I3C65-1
		DEFB	042H
		DEFW	I3C79-1
		DEFB	43H
		DEFW	I3C87-1
		DEFB	46H
		DEFW	I3C96-1
		DEFB	0

I3C47:		DEFB	"Bad file mode"
		DEFB	0

I3C55:		DEFB	"File still open"
		DEFB	0

I3C65:		DEFB	"File already exists"
		DEFB	0

I3C79:		DEFB	"RAM disk full"
		DEFB	0

I3C87:		DEFB	"Too many files"
		DEFB	0

I3C96:		DEFB	"RAM disk offline"
		DEFB	0


; *************************************
; PUTCHR and KYKLOK
; *************************************

	IF KEYTYP = 0

; Japanese version
; routine to handle special keys (non letter) in SHIFT KANA mode
J3CA7:		LD	HL,KANAMD
		INC	(HL)
		DEC	(HL)			; ANSI layout ?
		LD	B,8
		LD	DE,I3F4E
		LD	HL,I3F3E
		JR	Z,J3CB9			; yep,
		LD	HL,I3F46
J3CB9:		CP	(HL)
		JR	Z,J3CC6
		INC	HL
		INC	DE
J3CBE:		DJNZ	J3CB9
J3CC0:		CALL	C3ED1			; put in keyboardbuffer
		JP	J3DC7			; end vowel

J3CC6:		LD	A,(DE)
		JR	J3CC0

; Subroutine S.PUTCHR
J3CC9:		PUSH	AF
		CALL	C3EE7			; generate keyclick
		POP	AF
		JR	Z,J3CD5			; generated by a functionkey, put in keyboardbuffer and quit
		LD	HL,MODE
		BIT	0,(HL)			; in SHIFT KANA mode ?
J3CD5:		JP	Z,C3ED1			; nope, put in keyboardbuffer and quit
		CP	'A'
		JR	C,J3CA7
		CP	'Z'+1
		JR	C,J3CEC			; upcase letter, flag SHIFT KANA upcase
		CP	'a'
		JR	C,J3CA7
		CP	'z'+1
		JR	NC,J3CA7
		RES	7,(HL)			; lowercase letter, flag SHIFT KANA lowercase
		JR	J3CEE

J3CEC:		SET	7,(HL)
J3CEE:		CP	'a'			; upcase letter ?
		JR	C,J3CF4
		SUB	20H			; nope, make uppercase
J3CF4:		LD	D,A			; save character
		CP	'x'
		JR	Z,J3D15
		LD	A,(CHRCNT)
		OR	A			; already in a vowel ?
		JR	NZ,J3D1A		; yep,
		LD	A,D
		CALL	C3E2F
		LD	A,D
		JP	NZ,C3EB6
		CALL	C3E20
		CP	80H			; >= 80H ?
		RET	NC			; yep, quit
		LD	A,D
		LD	(ROMA+0),A
		LD	A,1
		JR	J3D55			; 1 character in vowel

J3D15:		LD	A,0B0H
		JP	J3CC0

J3D1A:		DEC	A			; has 1 character already ?
		LD	A,D
		JR	NZ,J3D57		; nope, 2 characters
J3D1E:		LD	E,A
		CALL	C3E2F
		LD	A,(ROMA+0)
		JR	Z,J3D2C
		CALL	C3E3D
		JR	J3D38			; end vowel

J3D2C:		CP	E
		JR	NZ,J3D3F
		CP	'N'
		JR	NZ,J3D3B
		LD	A,0FDH
		CALL	C3EBC			; put in keyboardbuffer (case conversion)
J3D38:		JP	J3DC7			; end vowel

J3D3B:		LD	A,8FH
		JR	J3D4D			; put in keyboardbuffer (case conversion)

J3D3F:		CP	'N'
		LD	A,E
		JR	NZ,J3D50
		CP	'Y'
		JR	Z,J3D50
		LD	(ROMA+0),A
		LD	A,0FDH
J3D4D:		JP	C3EBC			; put in keyboardbuffer (case conversion)

J3D50:		LD	(ROMA+1),A
		LD	A,2			; 2 characters in vowel
J3D55:		JR	J3DC8

J3D57:		LD	HL,ROMA
		LD	D,(HL)
		INC	HL
		LD	B,(HL)
		LD	C,A
		PUSH	BC
		CALL	C3E2F
		POP	BC
		JR	NZ,J3D72
		LD	A,B
		LD	(ROMA+0),A
		LD	A,1
		LD	(CHRCNT),A
		LD	A,C
		JP	J3D1E

J3D72:		LD	A,B
		CP	'Y'
		JR	NZ,J3D9D
		LD	A,D
		CP	'C'
		JR	Z,J3D90
		CP	'L'
		JR	Z,J3DC1
		CP	'F'
		JR	Z,J3D99
		CP	'W'
		JR	Z,J3DF2
		LD	E,"I"
		PUSH	BC
		CALL	C3E3D
		JR	J3D96

J3D90:		LD	A,0E1H
J3D92:		PUSH	BC
		CALL	C3EBC			; put in keyboardbuffer (case conversion)
J3D96:		POP	BC
		JR	J3DC1

J3D99:		LD	A,0ECH
		JR	J3D92

J3D9D:		LD	A,B
		CP	'H'
		JR	NZ,J3DDC
		LD	A,D
		CP	'C'
		JR	Z,J3DB7
		CP	'D'
		JR	Z,J3DCC
		CP	'T'
		JR	Z,J3DD5
		CP	'S'
		JR	NZ,J3DF2
		LD	A,9CH
		JR	J3DB9

J3DB7:		LD	A,0E1H
J3DB9:		CALL	C3EBC			; put in keyboardbuffer (case conversion)
J3DBC:		LD	A,C
		CP	'I'
		JR	Z,J3DC7			; end vowel
J3DC1:		LD	A,"J"
		LD	E,C
		CALL	C3EB9
J3DC7:		XOR	A
J3DC8:		LD	(CHRCNT),A
		RET

J3DCC:		LD	A,0E3H
		CALL	C3EBC			; put in keyboardbuffer (case conversion)
		LD	A,0DEH
		JR	J3DD7

J3DD5:		LD	A,0E3H
J3DD7:		CALL	C3EBC			; put in keyboardbuffer (case conversion)
		JR	J3DC1

J3DDC:		LD	A,D
		CP	'T'
		JR	NZ,J3DF2
		LD	A,B
		CP	'S'
		JR	NZ,J3DF2
		LD	A,C
		CP	"U"
		JR	NZ,J3DF2
		LD	A,0E2H
		CALL	C3EBC			; put in keyboardbuffer (case conversion)
		JR	J3DC7			; end vowel

J3DF2:		LD	A,B
		LD	E,C
		CALL	C3E3D
		JR	J3DC7			; end vowel

C3DF9:		CALL	C3E29
		LD	B,A
		ADD	A,A
		ADD	A,A
		ADD	A,B
		LD	D,A
		LD	A,E
		CALL	C3E2F
		ADD	A,D
		LD	L,A
		LD	A,D
		CP	0FH
		JR	NC,J3E10
		LD	A,L
		SUB	70H
		RET

J3E10:		CP	23H
		JR	NC,J3E18
		LD	A,L
		SUB	30H
		RET

J3E18:		LD	A,L
		SUB	23H
		LD	BC,I3F71
		JR	J3E23

C3E20:		LD	BC,I3F56-040H
J3E23:		LD	L,A
		LD	H,0
		ADD	HL,BC
		LD	A,(HL)
		RET

C3E29:		CALL	C3E20
		AND	0FH
		RET

C3E2F:		CALL	C3E20
		SUB	14H
		CP	1			; is it 014H ?
		JR	C,J3E3B
		CP	06H
		RET	C
J3E3B:		XOR	A
		RET

C3E3D:		LD	D,A
		CALL	C3E20
		CP	0BH			; >= 0BH ?
		JR	NC,J3E48		; yep,
		LD	A,D
		JR	J3E8A

J3E48:		CP	21H
		JR	C,J3E58
		CP	26H
		JR	NC,J3E58
		LD	A,D
		CALL	C3EB9

C3E54:		LD	A,0DEH
		JR	C3EBC			; put in keyboardbuffer (case conversion)

J3E58:		LD	A,D
		CP	"P"
		JR	NZ,J3E67
		LD	A,"H"
		CALL	C3EB9
		LD	A,0DFH
		JP	C3EBC			; put in keyboardbuffer (case conversion)

J3E67:		CP	'J'
		JR	NZ,J3E79
		LD	A,9CH
		PUSH	DE
		CALL	C3EBC			; put in keyboardbuffer (case conversion)
		CALL	C3E54
		POP	DE
		LD	C,E
		JP	J3DBC

J3E79:		CP	'F'
		JR	NZ,J3E8C
		LD	A,0ECH
		PUSH	DE
		CALL	C3EBC			; put in keyboardbuffer (case conversion)
J3E83:		POP	DE
		LD	A,E
		CP	"U"
		RET	Z
J3E88:		LD	A,"L"
J3E8A:		JR	C3EB9

J3E8C:		CP	'Y'
		JR	NZ,J3E96
		CALL	C3EB9
		LD	A,E
		JR	J3EA2

J3E96:		CP	'W'
		JR	NZ,J3EA7
		CALL	C3EB9
		LD	A,E
		CP	'I'
		JR	Z,J3E88
J3EA2:		CP	"E"
		RET	NZ
		JR	J3E88

J3EA7:		CP	"V"
		JR	NZ,J3EB7
		LD	A,"U"
		PUSH	DE
		CALL	C3EB6
		CALL	C3E54
		JR	J3E83

C3EB6:		LD	E,A
J3EB7:		LD	A,"Q"

C3EB9:		CALL	C3DF9

; Subroutine put in keyboardbuffer (case conversion)
C3EBC:		LD	HL,MODE
		BIT	7,(HL)			; upcase SHIFT KANA ?
		JR	Z,C3ED1			; nope, put in keyboardbuffer and quit
		CP	0E0H
		JR	C,J3ECB
		SUB	20H
		JR	C3ED1			; put in keyboardbuffer and quit

J3ECB:		CP	0A0H
		JR	NC,C3ED1		; put in keyboardbuffer and quit
		ADD	A,20H
						; put in keyboardbuffer and quit

; Subroutine put in keyboardbuffer
C3ED1:		LD	HL,(PUTPNT)
		LD	(HL),A
		INC	HL
		LD	A,L
		CP	(KEYBUF+40) AND 255
		JR	NZ,J3EDE
		LD	HL,KEYBUF
J3EDE:		LD	A,(GETPNT)
		CP	L
		RET	Z
		LD	(PUTPNT),HL
		RET

; Subroutine generate keyclick
C3EE7:		LD	A,(CLIKSW)
		AND	A
		RET	Z
		LD	A,(CLIKFL)
		AND	A
		RET	NZ
		LD	A,0FH
		LD	(CLIKFL),A
		DI
		OUT	(0ABH),A
		LD	A,10
J3EFB:		DEC	A
		JR	NZ,J3EFB
		LD	A,0EH
		OUT	(0ABH),A
		EI
		RET

; Subroutine S.KYKLOK
J3F04:		LD	HL,MODE
		BIT	0,(HL)			; in SHIFT KANA mode ?
		JR	Z,J3F10			; nope,
		RES	0,(HL)			; leave SHIFT KANA mode
		XOR	A
		JR	J3F29			; KANA led off

J3F10:		LD	A,(KANAST)
		INC	A			; in KANA mode ?
		JR	Z,J3F26			; yep, leave KANA mode and KANA led off
I3F16:		LD	A,(NEWKEY+6)
		RRCA				; SHIFT also pressed ?
		JR	C,J3F24			; nope, switch to KANA mode and KANA led on
		XOR	A
		LD	(CHRCNT),A
		SET	0,(HL)			; switch to SHIFT KANA mode
		JR	J3F29			; KANA led off

J3F24:		LD	A,0FFH
J3F26:		LD	(KANAST),A
J3F29:		PUSH	AF
		LD	A,0FH
		OUT	(0A0H),A
		IN	A,(0A2H)
		AND	7FH
		LD	B,A
		POP	AF
		OR	A
		LD	A,80H
		JR	Z,J3F3A
		XOR	A
J3F3A:		OR	B
		OUT	(0A1H),A
		RET

; ANSI layout
I3F3E:		DEFB	"*:]_?>{}"

; JIS layout
I3F46:		DEFB	"\@>?[<{}"

; replacement table
I3F4E:		DEFB	0B0H,0DEH,0A1H,0A5H,0DFH,0A4H,0A2H,0A3H

I3F56:		DEFB	00AH	; unused entry
		DEFB	015H	; A
		DEFB	025H	; B
		DEFB	028H	; C
		DEFB	023H	; D
		DEFB	018H	; E
		DEFB	040H	; F
		DEFB	021H	; G
		DEFB	005H	; H
		DEFB	016H	; I
		DEFB	04BH	; J
		DEFB	001H	; K
		DEFB	00AH	; L
		DEFB	006H	; M
		DEFB	004H	; N
		DEFB	019H	; O
		DEFB	028H	; P
		DEFB	080H	; Q
		DEFB	008H	; R
		DEFB	002H	; S
		DEFB	003H	; T
		DEFB	017H	; U
		DEFB	040H	; V
		DEFB	049H	; W
		DEFB	08CH	; X
		DEFB	047H	; Y
		DEFB	022H	; Z

I3F71:		DEFB	0FDH,0F4H,092H,0F5H,092H,0F6H,0F7H,0F8H
		DEFB	0F9H,0FAH,0FBH,0FCH,093H,093H,093H,086H
		DEFB	087H,088H,089H,08AH,08BH,08CH,088H,08DH
		DEFB	08AH,08EH

	ENDIF

	IF KEYTYP != 0
	IF DATFMT = 1
C3CA7:		CALL	C1947
		LD	BC,-8
		ADD	HL,BC
		LD	D,H
		LD	E,L
		LD	B,(HL)
		INC	HL
		LD	C,(HL)
		INC	HL
		BIT	4,(HL)
		RET	NZ
		INC	HL
		PUSH	BC
		LD	BC,5
		LDIR
		POP	BC
		INC	DE
		EX	DE,HL
		LD	(HL),B
		INC	HL
		LD	(HL),C
		RET
	ENDIF
	IF DATFMT = 2
C3CA7:		CALL	C1947
		DEC	HL
		LD	C,(HL)
		DEC	HL
		LD	B,(HL)
		DEC	HL
		BIT	4,(HL)
		RET	NZ
J3CB2:		LD	DE,-5
		ADD	HL,DE
		LD	D,(HL)
		LD	(HL),B
		INC	HL
		LD	E,(HL)
		LD	(HL),C
		LD	BC,5
J3CBE:		ADD	HL,BC
		LD	(HL),D
J3CC0:		INC	HL
		LD	(HL),E
		RET
	ENDIF

C3CC3:		LD	(AVCSAV),A
		XOR	A
		OUT	(0F8H),A
		LD	(D_FFF8),A
J3CCC:		RET

C3CCD:		RST	S_GETBYT
J3CCE:		CP	16
J3CD0:		RET	NC
		PUSH	AF
		RRCA
		RRCA
		AND	03H
		LD	E,A
		LD	A,0FCH
		CALL	C3CE1
		POP	AF
		AND	03H
		SCF
		RET

C3CE1:		PUSH	HL
J3CE2:		LD	HL,D_FFF8
		AND	(HL)
		OR	E
		LD	(HL),A
		OUT	(0F8H),A
		POP	HL
		RET

I3CEC:		DEFB	00AH			; id
		DEFB	000H			; adjust X
		DEFB	000H			; adjust Y
		DEFB	000H			; no interlace, screen 0
		DEFB	37 % 16
		DEFB	37 / 16			; screen width
		DEFB	00FH			; foreground color
		DEFB	004H			; background color
		DEFB	004H			; border color
		DEFB	003H			; cassette speed 1200, printer mode msx, key click on, function keys on
		DEFB	001H			; beep tone 0, volume 1
		DEFB	000H			; title color
	IFNDEF	HBF9P
		DEFB	003H			; native code
	ELSE
		DEFB	CNTRY
	ENDIF
		DEFB	0FFH


; International version, officially not needed
; Subroutine S.PUTCHR
J3CC9:		PUSH	AF
		CALL	X3E78			; keyclick
		POP	AF

C3E62:		LD	HL,(PUTPNT)
		LD	(HL),A
		INC	HL
		LD	A,L
KEYLOW		EQU	(KEYBUF+40) % 256
		CP	KEYLOW
		JR	NZ,J3E6F
		LD	HL,KEYBUF
J3E6F:		LD	A,(GETPNT)
		CP	L
		RET	Z
		LD	(PUTPNT),HL
		RET

; Subroutine S.KYKLOK
J3F04:		LD	A,(NEWKEY+6)
		LD	E,A
		OR	0FEH			; only SHIFT status remains
		BIT	4,E
		JR	NZ,X3E96		; CODE not pressed,
		AND	0FDH
X3E96:		CPL
		INC	A
		LD	(KANAST),A		; dead status

; Subroutine keyclick
X3E78:		LD	A,(CLIKSW)
		AND	A
		RET	Z
		LD	A,(CLIKFL)
		AND	A
		RET	NZ
		LD	A,0FH
		LD	(CLIKFL),A
		DI
		OUT	(0ABH),A
		LD	A,0AH
X3E8C:		DEC	A
		JR	NZ,X3E8C		; wait 0.05 ms (in Z80 mode!)
		LD	A,0EH
		OUT	(0ABH),A
		EI
		RET

	ENDIF

; *************************************
; End of PUTCHR and KYKLOK
; *************************************

	IFDEF FS5500
C3F8B:		LD	D,48
J3F8D:		DEC	D
		JR	NZ,J3F8D
		JP	C3585
	ENDIF

		DEFS	04000H-$,0

